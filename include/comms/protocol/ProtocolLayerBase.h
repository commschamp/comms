...
using ProtocolLayerBase =
    comms::frame::FrameLayerBase<TField, TNextLayer, TDerived, TOptions...>;

/// @brief Same as @ref comms::frame::toFrameLayerBase()
/// @deprecated Use @ref comms::frame::toFrameLayerBase() instead
template <typename TField, typename TNextLayer, typename TDerived,
          typename... TOptions>
auto toProtocolLayerBase(
    ProtocolLayerBase<TField, TNextLayer, TDerived, TOptions...> &layer)
    -> decltype(comms::frame::toFrameLayerBase(layer)) {
...
template <typename TField, typename TNextLayer, typename TDerived,
          typename... TOptions>
constexpr auto toFrameLayerBase(
    const ProtocolLayerBase<TField, TNextLayer, TDerived, TOptions...> &layer)
    -> decltype(comms::frame::toFrameLayerBase(layer)) {
  return comms::frame::toFrameLayerBase(layer);
}
...
inline auto
missingSize(std::size_t &val) -> decltype(comms::frame::missingSize(val)) {
...
inline auto
msgIndex(std::size_t &val) -> decltype(comms::frame::msgIndex(val)) {
...
auto msgPayload(TIter &iter, std::size_t &len)
...
#define COMMS_PROTOCOL_LAYERS_ACCESS_INNER(...)                                \
  COMMS_FRAME_LAYERS_ACCESS_INNER(__VA_ARGS__)
...
#define COMMS_PROTOCOL_LAYERS_NAMES_INNER(...)                                 \
  COMMS_FRAME_LAYERS_NAMES_INNER(__VA_ARGS__)
...
#define COMMS_PROTOCOL_LAYERS_ACCESS_OUTER(...)                                \
  COMMS_FRAME_LAYERS_ACCESS_OUTER(__VA_ARGS__)
...
#define COMMS_PROTOCOL_LAYERS_NAMES_OUTER(...)                                 \
  COMMS_FRAME_LAYERS_NAMES_OUTER(__VA_ARGS__)

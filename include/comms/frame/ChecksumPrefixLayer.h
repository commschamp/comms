...
/// @details This protocol layer handles checksum value that usually precedes
/// the
...
///     the
///         @b ChecksumPrefixLayer will invoke @b read operation of inner
///         (wrapped) layers and only if it is successful, it will calculate and
///         verify the checksum value. Usage of @ref
///         comms::option::def::FrameLayerVerifyBeforeRead modifies the default
///         behaviour by forcing the checksum verification prior to invocation
///         of @b read operation in the wrapped layer(s).
...
class ChecksumPrefixLayer
    : public comms::frame::details::ChecksumPrefixLayerBase<
          TField, TCalc, TNextLayer, TOptions...> {
  using BaseImpl =
      comms::frame::details::ChecksumPrefixLayerBase<TField, TCalc, TNextLayer,
                                                     TOptions...>;
...
public:
  ...
  doRead(Field &field, TMsg &msg, TIter &iter, std::size_t size,
    ...
    auto fieldLen =
        static_cast<std::size_t>(std::distance(beforeFieldReadIter, iter));
    return readInternal(field, msg, iter, size - fieldLen,
                        std::forward<TNextLayerReader>(nextLayerReader),
                        VerifyTag(), extraValues...);
  }
  ...
  /// @details Should be called when @ref doWrite() returns
  ...
  comms::ErrorStatus doUpdate(Field &field, TIter &iter, std::size_t size,
    ...
    static_assert(!std::is_void<MsgPtr>::value,
                  "Please use update() overload that accepts message object as "
                  "its first parameter");

    auto *msgPtr = static_cast<typename MsgPtr::element_type *>(nullptr);
    return fieldUpdateInternal(msgPtr, checksumIter, fromIter, iter, size,
                               field);
  }
...
protected:
  ...
  /// @deprecated Override @ref comms::frame::ChecksumLayer::doReadField()
  /// "doReadField()" instead
  template <typename TMsg, typename TIter>
  comms::ErrorStatus readField(const TMsg *msgPtr, Field &field, TIter &iter,
                               std::size_t len) {
  ...
  /// @deprecated Override @ref comms::frame::ChecksumLayer::doWriteField()
  /// "doWriteField()" instead
  template <typename TMsg, typename TIter>
  comms::ErrorStatus writeField(const TMsg *msgPtr, const Field &field,
                                TIter &iter, std::size_t len) const {
  ...
  /// @param[in, out] iter Iterator used for reading data, expected to be
  /// advanced
  /// @param[in] len Length of the output buffer
  /// @param[out] checksumValid Indication of whether the return checksum is
  /// valid,
  ///     must be populated.
  /// @return The checksum value.
  /// @note May be non-static in the extending class, but needs to be const.
  template <typename TMsg, typename TIter>
  static auto calculateChecksum(const TMsg *msg, TIter &iter, std::size_t len,
                                bool &checksumValid) -> decltype(TCalc()(iter,
                                                                         len)) {
  ...
  static auto
  getChecksumFromField(const Field &field) -> decltype(field.getValue()) {
    return field.getValue();
  }
  ...
  ///     is required. The default implementation is:
  ///     @code
  ...
  /// @note May be non-static in the extending class. In case of non-static,
  /// must be const
...
private:
  ...
  ErrorStatus verifyRead(Field &field, TMsg &msg, TIter &iter, std::size_t size,
    ...
    auto checksum =
        thisObj.calculateChecksum(msgPtr, fromIter, size, checksumValid);
  ...
  ErrorStatus readVerify(Field &field, TMsg &msg, TIter &iter, std::size_t size,
    ...
    bool checksumValid = false;
    auto checksum =
        thisObj.calculateChecksum(msgPtr, fromIter, len, checksumValid);
  ...
  ErrorStatus writeInternalRandomAccess(Field &field, const TMsg &msg,
    ...
    bool checksumValid = false;
    auto checksum =
        thisObj.calculateChecksum(&msg, fromIter, len, checksumValid);
    ...
    auto checksumEs =
        thisObj.writeField(&msg, field, checksumIter, checksumLen);
  ...
  template <typename TMsg, typename TIter>
  ErrorStatus fieldUpdateInternal(const TMsg *msgPtr, TIter checksumIter,
                                  TIter from, TIter to, std::size_t size,
                                  Field &field) const {
    ...
    bool checksumValid = false;
    auto checksum = thisObj.calculateChecksum(msgPtr, from, len, checksumValid);
...
namespace details {
template <typename T> struct ChecksumPrefixLayerCheckHelper {
...
template <typename T> constexpr bool isChecksumPrefixLayer() {

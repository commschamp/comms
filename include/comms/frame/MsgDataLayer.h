...
/// are:
///     @li @ref comms::option::def::FieldType - Use this option to override
///     default field type.
/// @headerfile comms/frame/MsgDataLayer.h
template <typename... TOptions> class MsgDataLayer {
...
public:
  ...
  ThisLayer &thisLayer() { return *this; }

  /// @brief Get "const" access to this layer object.
  const ThisLayer &thisLayer() const { return *this; }
  ...
  /// @brief All fields of the remaining transport layers, contains only @ref
  /// Field.
  ...
  static constexpr bool canSplitRead() { return true; }
  ...
  static ErrorStatus read(TMsg &msg, TIter &iter, std::size_t size,
                          TExtraValues... extraValues) {
    using MsgType = typename std::decay<decltype(msg)>::type;
    using Tag = typename comms::util::
        Conditional<comms::isMessageBase<MsgType>()>::template Type<
            DirectOpTag<>,
            typename comms::util::Conditional<comms::isMessage<MsgType>()>::
                template Type<InterfaceOpTag<>,
                              typename comms::util::LazyShallowConditional<
                                  std::is_pointer<MsgType>::value>::
                                  template Type<PointerOpTag, OtherOpTag>>>;

    if (setPayloadRequiredInternal(extraValues...)) {
      auto fromIter = iter;
      auto es = readInternal(msg, iter, size, Tag(), extraValues...);
      setMsgPayloadInternal(
          fromIter, static_cast<std::size_t>(std::distance(fromIter, iter)),
          extraValues...);
      return es;
    }
  ...
  template <typename TMsg, typename TIter, typename... TExtraValues>
  static ErrorStatus readUntilData(TMsg &msg, TIter &iter, std::size_t size,
                                   TExtraValues...) {
  ...
  /// @return @ref comms::ErrorStatus::Success;
  template <typename TMsg, typename TIter, typename... TExtraValues>
  static ErrorStatus readFromData(TMsg &msg, TIter &iter, std::size_t size,
                                  TExtraValues... extraValues) {
  ...
  static ErrorStatus readFieldsCached(TAllFields &allFields, TMsg &msg,
    ...
    static_assert(
        ...
        "Caching read from non random access iterators are not supported at "
        "this moment.");
  ...
  template <typename TAllFields, typename TMsg, typename TIter,
            typename... TExtraValues>
  static ErrorStatus readUntilDataFieldsCached(TAllFields &allFields, TMsg &msg,
                                               TIter &iter, std::size_t size,
                                               TExtraValues...) {
  ...
  /// @details Expected to be called by the privous layers to properly
  ///     finalise read operation after the call to @ref
  ///     readUntilDataFieldsCached();
  /// @return @ref comms::ErrorStatus::Success;
  template <typename TAllFields, typename TMsg, typename TIter,
            typename... TExtraValues>
  static ErrorStatus readFromDataFeildsCached(TAllFields &allFields, TMsg &msg,
                                              TIter &iter, std::size_t size,
                                              TExtraValues... extraValues) {
  ...
  ///     @ref comms::option::def::FieldsImpl option, the function calls @b
  ///     doWrite non-virtual function defined by comms::MessageBase (see
  ///     comms::MessageBase::doWrite) or redefined by the actual message
  ///     itself. Otherwise, TMsg type is expected to be the used interface
  ///     which allows polymorphic write functionality. It must define @b
  ///     write() member function which will be called.
  ...
  /// @return Status of the write operation.
  template <typename TAllFields, typename TMsg, typename TIter>
  static ErrorStatus writeFieldsCached(TAllFields &allFields, const TMsg &msg,
                                       TIter &iter, std::size_t size) {
    static_assert(comms::util::IsTuple<TAllFields>::Value,
                  "Expected TAllFields to be tuple.");
  ...
  /// @return Status of the update operation.
  template <typename TMsg, typename TIter>
  static comms::ErrorStatus update(const TMsg &msg, TIter &iter,
                                   std::size_t size) {
    static_cast<void>(msg);
  ...
  static ErrorStatus updateFieldsCached(TAllFields &allFields, TIter &iter,
                                        std::size_t size) {
    static_cast<void>(allFields);
    std::advance(iter, size);
    return comms::ErrorStatus::Success;
  ...
  /// @return Status of the update operation.
  template <typename TAllFields, typename TMsg, typename TIter>
  static ErrorStatus updateFieldsCached(TAllFields &allFields, const TMsg &msg,
                                        TIter &iter, std::size_t size) {
    static_cast<void>(allFields);
    static_cast<void>(msg);
    std::advance(iter, size);
  ...
  /// @return 0.
  static constexpr std::size_t length() { return 0U; }
  ...
  static constexpr std::size_t length(const TMsg &msg) {
    ...
    using Tag = typename comms::util::LazyShallowConditional<
        ...
        template Type<MsgDirectLengthTag, MsgHasLengthTag>;
...
private:
  ...
  template <typename... TParams> using OtherOpTag = comms::details::tag::Tag7<>;

  template <typename TMsg, typename TIter>
  static ErrorStatus
  writeWithFieldCachedInternal(Field &field, const TMsg &msg, TIter &iter,
                               std::size_t size,
  ...
  template <typename TMsg, typename TIter>
  static ErrorStatus writeWithFieldCachedInternal(Field &field, const TMsg &msg,
                                                  TIter &iter, std::size_t size,
  ...
  template <typename TMsg, typename TIter>
  static ErrorStatus
  writeWithFieldCachedRandomAccess(Field &field, const TMsg &msg, TIter &iter,
                                   std::size_t size) {
    ...
    auto writtenCount =
        static_cast<std::size_t>(std::distance(dataReadIter, iter));
  ...
  static ErrorStatus
  writeWithFieldCachedOutput(Field &field, const TMsg &msg,
  ...
  static std::size_t getMsgLength(const TMsg &msg,
                                  MsgHasLengthTag<TParams...>) {
    using MsgType = typename std::decay<decltype(msg)>::type;
    static_assert(MsgType::hasLength(),
                  "Message interface must define length()");
    return msg.length();
  }

  template <typename TMsg, typename... TParams>
  static constexpr std::size_t getMsgLength(const TMsg &msg,
                                            MsgDirectLengthTag<TParams...>) {
    using MsgType = typename std::decay<decltype(msg)>::type;
  ...
  template <typename TMsg, typename... TParams>
  static constexpr std::size_t getMsgLength(const TMsg &,
                                            MsgNoLengthTag<TParams...>) {
    return 0U;
  }

  template <typename TMsg, typename TIter, typename... TExtraValues>
  static ErrorStatus readInternalPolymorphic(TMsg &msg, TIter &iter,
                                             std::size_t size,
                                             TExtraValues... extraValues) {
    using MsgType = typename std::decay<decltype(msg)>::type;
    ...
    static_assert(
        std::is_convertible<IterType, typename MsgType::ReadIterator>::value,
        "The provided iterator is not convertible to ReadIterator defined by "
        "Message class");

    using ReadIter = typename std::add_lvalue_reference<
        typename MsgType::ReadIterator>::type;
    ...
    if ((result == ErrorStatus::NotEnoughData) &&
      ...
      using Tag = typename comms::util::LazyShallowConditional<
          MsgType::hasLength()>::template Type<MsgHasLengthTag, MsgNoLengthTag>;
  ...
  template <typename TMsg, typename TIter, typename... TExtraValues>
  static ErrorStatus readInternalDirect(TMsg &msg, TIter &iter,
                                        std::size_t size,
                                        TExtraValues... extraValues) {
    using MsgType = typename std::decay<decltype(msg)>::type;
  ...
  template <typename TMsg, typename TIter, typename... TExtraValues>
  static ErrorStatus readInternal(TMsg &msg, TIter &iter, std::size_t size,
                                  DirectOpTag<>, TExtraValues... extraValues) {
  ...
  template <typename TMsg, typename TIter, typename... TExtraValues>
  static ErrorStatus readInternal(TMsg &msg, TIter &iter, std::size_t size,
                                  PointerOpTag<>, TExtraValues... extraValues) {
    return read(*msg, iter, size, extraValues...);
  }

  template <typename TMsg, typename TIter, typename... TExtraValues>
  static ErrorStatus readInternal(TMsg &msg, TIter &iter, std::size_t size,
                                  OtherOpTag<>, TExtraValues... extraValues) {
    using MsgType = typename std::decay<decltype(msg)>::type;
    static_assert(comms::details::hasElementType<MsgType>(),
                  "Unsupported type of message object, expected to be either "
                  "message itself, raw pointer or smart pointer");
    return read(*msg, iter, size, extraValues...);
  }

  template <typename TMsg, typename TIter, typename... TParams>
  static ErrorStatus writeInternal(const TMsg &msg, TIter &iter,
                                   std::size_t size, DirectOpTag<TParams...>) {
  ...
  static ErrorStatus writeInternal(const TMsg &msg, TIter &iter,
    ...
    static_assert(
        std::is_convertible<IterType, typename MsgType::WriteIterator>::value,
        "The provided iterator is not convertible to WriteIterator defined by "
        "Message class");

    using WriteIter = typename std::add_lvalue_reference<
        typename MsgType::WriteIterator>::type;
  ...
  template <typename TMsg, typename TIter, typename... TParams>
  static ErrorStatus writeInternal(const TMsg &msg, TIter &iter,
                                   std::size_t size, PointerOpTag<TParams...>) {
    return write(*msg, iter, size);
  }

  template <typename TMsg, typename TIter, typename... TParams>
  static ErrorStatus writeInternal(const TMsg &msg, TIter &iter,
                                   std::size_t size, OtherOpTag<TParams...>) {
    using MsgType = typename std::decay<decltype(msg)>::type;
    static_assert(comms::details::hasElementType<MsgType>(),
                  "Unsupported type of message object, expected to be either "
                  "message itself, raw pointer or smart pointer");
  ...
  static constexpr bool missingSizeRequiredInternal() { return false; }

  template <typename... TExtraValues>
  static bool missingSizeRequiredInternal(details::MissingSizeRetriever<>,
                                          TExtraValues...) {
    return true;
  }
  ...
  static bool missingSizeRequiredInternal(T, TExtraValues... extraValues) {
    static_assert(!details::isMissingSizeRetriever<T>(),
  ...
  template <typename... TExtraValues>
  static void
  updateMissingSizeInternal(std::size_t val,
  ...
  template <typename T, typename... TExtraValues>
  static void updateMissingSizeInternal(std::size_t val, T retriever,
                                        TExtraValues... extraValues) {
  ...
  static constexpr bool setPayloadRequiredInternal() { return false; }

  template <typename TIter, typename... TExtraValues>
  static bool setPayloadRequiredInternal(details::MsgPayloadRetriever<TIter>,
                                         TExtraValues...) {
    return true;
  }
  ...
  static bool setPayloadRequiredInternal(T, TExtraValues... extraValues) {
    static_assert(!details::isMsgPayloadRetriever<T>(),
  ...
  static void
  setMsgPayloadInternal(TIter iter, std::size_t len,
  ...
  static void setMsgPayloadInternal(TIter iter, std::size_t len, T retriever,
                                    TExtraValues... extraValues) {
    static_cast<void>(retriever);
    static_assert(!details::isMsgPayloadRetriever<
                      typename std::decay<decltype(retriever)>::type>(),
                  "Mustn't be message payload retriever");
    setMsgPayloadInternal(iter, len, extraValues...);
  }
...
namespace details {
template <typename T> struct MsgDataLayerCheckHelper {
...
template <typename T> constexpr bool isMsgDataLayer() {
  return details::MsgDataLayerCheckHelper<T>::Value;
}

template <typename... TExtraOpts>
constexpr MsgDataLayer<TExtraOpts...> &
toFrameLayerBase(MsgDataLayer<TExtraOpts...> &layer) {
  return layer;
}

template <typename... TExtraOpts>
constexpr const MsgDataLayer<TExtraOpts...> &

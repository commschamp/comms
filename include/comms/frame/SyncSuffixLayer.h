...
namespace frame {
/// @brief Frame layer that uses "sync" field as a suffix (after message
/// payload) to all the
///        preceding data written by other (next) layers.
/// @details The main purpose of this layer is to provide a constant
/// synchronisation
...
/// @tparam TField Type of the field that is used as to represent checksum
/// value.
/// @tparam TNextLayer Next transport layer in frame.
/// @tparam TOptions Extending functionality options. Supported options are:
///     @li  @ref comms::option::ExtendingClass - Use this option to provide a
///     class
///         name of the extending class, which can be used to extend existing
///         functionality. See also @ref page_custom_checksum_layer tutorial
///         page.
///     @li @ref comms::option::def::FrameLayerVerifyBeforeRead - By default,
///     the
///         @b SyncSuffixLayer will invoke @b read operation of inner (wrapped)
///         layers and only if it is successful. Usage of @ref
///         comms::option::def::FrameLayerVerifyBeforeRead modifies the default
///         behaviour by forcing the field value verification prior to
///         invocation of @b read operation in the wrapped layer(s).
///     @li @ref comms::option::def::FrameLayerSeekField - By default, the
///         @b SyncSuffixLayer will invoke @b read operation of inner (wrapped)
///         layers and only if it is successful. Usage of @ref
///         comms::option::def::FrameLayerSeekField modifies the default
///         behaviour by forcing layer to seek the field in the buffer (until
///         the successful read and verification) prior to invocation of @b read
///         operation in the wrapped layer(s).
/// @headerfile comms/frame/SyncSuffixLayer.h
/// @extends comms::frame::FrameLayerBase
template <typename TField, typename TNextLayer, typename... TOptions>
class SyncSuffixLayer
    : public comms::frame::details::SyncSuffixLayerBase<TField, TNextLayer,
                                                        TOptions...> {
  using BaseImpl =
      comms::frame::details::SyncSuffixLayerBase<TField, TNextLayer,
                                                 TOptions...>;
  using ParsedOptionsInternal =
      details::SyncSuffixLayerOptionsParser<TOptions...>;

public:
  /// @brief Type of the field object used to read/write checksum value.
  using Field = typename BaseImpl::Field;

  /// @brief Type of the escape field provided via @ref
  /// comms::option::def::FrameLayerSeekField option.
  ...
  ///     If the call returns comms::ErrorStatus::Success, it reads and verifies
  ///     the field value. If the field's value is as expected,
  ...
  ErrorStatus doRead(Field &field, TMsg &msg, TIter &iter, std::size_t size,
                     TNextLayerReader &&nextLayerReader,
                     TExtraValues... extraValues) {
    using IterType = typename std::decay<decltype(iter)>::type;
    static_assert(
        std::is_same<typename std::iterator_traits<IterType>::iterator_category,
                     std::random_access_iterator_tag>::value,
        "The read operation is expected to use random access iterator");

    using SeekTag = typename comms::util::LazyShallowConditional<
        ParsedOptionsInternal::HasSeekField>::template Type<SeekFieldTag,
                                                            InstantOpTag>;

    return readInternal(field, msg, iter, size,
                        std::forward<TNextLayerReader>(nextLayerReader),
                        SeekTag(), extraValues...);
  }

  /// @brief Customized write functionality, invoked by @ref
  /// comms::frame::FrameLayerBase::write() "write()".
  /// @details The function will call the write() function of the next layer and
  /// then will write its sync field value.
  ...
  ErrorStatus doWrite(Field &field, const TMsg &msg, TIter &iter,
    ...
    return writeInternal(field, msg, iter, size,
...
protected:
  ...
  /// @details Default implementation compares read field with default
  /// constructed Field type. @n
  ...
  static bool verifyFieldValue(const Field &field) { return field == Field(); }

  /// @brief Verify the validity of the escapte field provided via @ref
  /// comms::option::def::FrameLayerSeekField option.
  /// @details Default implementation compares read field with default
  /// constructed type. @n
  ///     May be overridden by the extending class in case
  ...
  static void prepareFieldForWrite(Field &field) { static_cast<void>(field); }
...
private:
  ...
  template <typename TMsg, typename TIter, typename TReader,
            typename... TExtraValues>
  ErrorStatus verifyRead(Field &field, TMsg &msg, TIter &iter, std::size_t size,
    ...
    static_assert(Field::minLength() == Field::maxLength(),
                  "Cannot verify variable length suffix before reading rest of "
                  "the message frame");
  ...
  template <typename TMsg, typename TIter, typename TReader,
            typename... TExtraValues>
  ErrorStatus readVerify(Field &field, TMsg &msg, TIter &iter, std::size_t size,
                         TReader &&nextLayerReader,
                         TExtraValues... extraValues) {
    auto fromIter = iter;
  ...
  template <typename TMsg, typename TIter, typename TReader,
            typename... TExtraValues>
  ErrorStatus seekFieldRead(Field &field, TMsg &msg, TIter &iter,
                            std::size_t size, TReader &&nextLayerReader,
                            TExtraValues... extraValues) {
    auto &thisObj = BaseImpl::thisLayer();
    auto *msgPtr = BaseImpl::toMsgPtr(msg);
  ...
  template <typename TMsg, typename TIter, typename TReader,
            typename... TExtraValues>
  ErrorStatus readInternal(Field &field, TMsg &msg, TIter &iter,
                           std::size_t size, TReader &&nextLayerReader,
                           SeekFieldTag<>, TExtraValues... extraValues) {
    return seekFieldRead(field, msg, iter, size,
  ...
  ErrorStatus readInternal(Field &field, TMsg &msg, TIter &iter,
                           std::size_t size, TReader &&nextLayerReader,
                           InstantOpTag<>, TExtraValues... extraValues) {
    using VerifyTag = typename comms::util::LazyShallowConditional<
        ParsedOptionsInternal::HasVerifyBeforeRead>::
        template Type<VerifyBeforeReadTag, VerifyAfterReadTag>;

    return readInternal(field, msg, iter, size,
                        std::forward<TReader>(nextLayerReader), VerifyTag(),
                        extraValues...);
  }
  ...
  ErrorStatus readInternal(Field &field, TMsg &msg, TIter &iter,
                           std::size_t size, TReader &&nextLayerReader,
                           VerifyBeforeReadTag<>, TExtraValues... extraValues) {
    return verifyRead(field, msg, iter, size,
                      std::forward<TReader>(nextLayerReader), extraValues...);
  }
  ...
  ErrorStatus readInternal(Field &field, TMsg &msg, TIter &iter,
                           std::size_t size, TReader &&nextLayerReader,
                           VerifyAfterReadTag<>, TExtraValues... extraValues) {
    return readVerify(field, msg, iter, size,
                      std::forward<TReader>(nextLayerReader), extraValues...);
  }

  template <typename TMsg, typename TIter, typename TWriter>
  ErrorStatus writeInternalRandomAccess(Field &field, const TMsg &msg,
  ...
  template <typename TMsg, typename TIter, typename TWriter>
  ErrorStatus writeInternalOutput(Field &field, const TMsg &msg, TIter &iter,
                                  std::size_t size,
                                  TWriter &&nextLayerWriter) const {
    auto &thisObj = BaseImpl::thisLayer();
  ...
  ErrorStatus writeInternal(Field &field, const TMsg &msg, TIter &iter,
    ...
    return writeInternalRandomAccess(field, msg, iter, size,
                                     std::forward<TWriter>(nextLayerWriter));
  }
  ...
  ErrorStatus writeInternal(Field &field, const TMsg &msg, TIter &iter,
    ...
    return writeInternalOutput(field, msg, iter, size,
                               std::forward<TWriter>(nextLayerWriter));
  }

  template <typename TMsg, typename TIter>
  ErrorStatus fieldUpdateInternal(const TMsg *msgPtr, TIter from, TIter to,
                                  std::size_t size, Field &field) const {
    ...
    auto checksum =
        thisObj.calculateSyncSuffix(msgPtr, from, len, checksumValid);
  ...
  bool fieldEscapedInternal(TIter from, TIter to, EscapeSupportedTag<>) {
    ...
    while (from != to) {
      ...
      while (true) {
        ...
        if ((es == comms::ErrorStatus::Success) && (iterTmp == to) &&
  ...
  template <typename TIter> bool fieldEscapedInternal(TIter from, TIter to) {
    using EscTag = typename comms::util::LazyShallowConditional<std::is_same<
        EscField, void>::value>::template Type<NoEscapeTag, EscapeSupportedTag>;

    return fieldEscapedInternal(from, to, EscTag());
  }

  static_assert(!(hasVerifyBeforeRead() && hasSeekField()),
                "Usage of both comms::option::def::FrameLayerVerifyBeforeRead "
                "and comms::option::def::FrameLayerSeekField at the same time "
                "are not allowed");
};

namespace details {
template <typename T> struct SyncSuffixLayerCheckHelper {
...
template <typename T> constexpr bool isSyncSuffixLayer() {

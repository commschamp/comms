...
class TransportValueLayerBaseHelper {
  ...
  using TopBase = comms::frame::FrameLayerBase<
      ...
      typename ParsedOptionsInternal::template ForceReadUntilDataSplitIfNeeded<
          TNextLayer>>;

  using AdaptedPseudoBase =
...
template <typename TField, std::size_t TIdx, typename TNextLayer,
          typename... TOptions>
using TransportValueLayerBase =
    typename TransportValueLayerBaseHelper<TField, TIdx, TNextLayer,
                                           TOptions...>::Base;

...
class ChecksumPrefixLayerBaseHelper {
  using ParsedOptionsInternal =
      comms::frame::details::ChecksumLayerOptionsParser<TOptions...>;
  using ActualLayer =
  ...
  using TopBase = comms::frame::FrameLayerBase<
      ...
      typename ParsedOptionsInternal::template SuppressForVerifyBeforeRead<
...
template <typename TField, typename TCalc, typename TNextLayer,
          typename... TOptions>
using ChecksumPrefixLayerBase =
    typename ChecksumPrefixLayerBaseHelper<TField, TCalc, TNextLayer,
                                           TOptions...>::Base;

...
COMMS_MSVC_WARNING_DISABLE(
    4189) // Disable erroneous initialized but not referenced variable warning
...
/// @details The main purpose of this layer is to process the message ID
/// information.
///     Holds instance of @ref comms::MsgFactory as its private member and uses
///     it to create message(s) with the required ID.
...
///     that this frame must be able to read() as well as create (using
///     createMsg()).
/// @tparam TNextLayer Next transport layer type.
/// @tparam TOptions Default functionality extension options. Supported options
/// are:
///     @li @ref comms::option::def::ExtendingClass - Use this option to provide
///     a class
///         name of the extending class, which can be used to extend existing
///         functionality. See also @ref page_custom_id_layer tutorial page.
///     @li @ref comms::option::app::MsgFactory - Override default message
///     factory class.
///         The overriding class is expected to have the same public interface
///         as @ref comms::MsgFactory.
///     @li @ref comms::option::app::MsgFactoryTempl - Override default message
///     factory class.
///         The overriding class is expected to have the same template
///         parameters as well as public interface as @ref comms::MsgFactory.
///     @li All the options supported by the @ref comms::MsgFactory. All the
///     options
...
template <typename TField, typename TMessage, typename TAllMessages,
          typename TNextLayer, typename... TOptions>
class MsgIdLayer
    : public comms::frame::details::MsgIdLayerBase<
          TField, TMessage, TAllMessages, TNextLayer, TOptions...> {
  ...
  static_assert(TMessage::hasMsgIdType(),
                ...
                "Use comms::option::def::MsgIdType option in message interface "
                "type definition.");

  using BaseImpl =
      comms::frame::details::MsgIdLayerBase<TField, TMessage, TAllMessages,
                                            TNextLayer, TOptions...>;
...
public:
  ...
  using MsgFactory =
      typename ParsedOptionsInternal::template MsgFactory<TMessage,
                                                          TAllMessages>;
  ...
  /// @brief Compile time inquiry of whether custom message factory class has
  /// been
  ///   provided via @ref comms::option::app::MsgFactory or @ref
  ///   comms::option::app::MsgFactoryTempl options.
  ...
  ///     generate appropriate (or validate provided) message object based on
  ///     the read ID and forward the read() request to the next layer. If the
  ///     message object cannot be generated (the message type is not provided
  ///     inside @b TAllMessages template parameter), but the @ref
  ///     comms::option::app::SupportGenericMessage option has beed used, the
  ///     @ref comms::GenericMessage may be generated instead.@n
  ///     @b NOTE, that @b msg parameter can be either reference to a smart
  ///     pointer, which will hold allocated object, or to previously allocated
  ///     object itself. In case of the latter, the function will compare read
  ///     and expected message ID value and will return @ref
  ///     comms::ErrorStatus::InvalidMsgId in case of mismatch.
  ...
  ///     @ref comms::frame::FrameLayerBase::readFieldsCached()
  ///     "readFieldsCached()"). Need to passed on as variadic arguments to the
  ///     @b nextLayerReader.
  /// @return Status of the operation.
  /// @pre @b msg parameter, in case of being a smart pointer, doesn't point to
  /// any object:
  ...
  comms::ErrorStatus
  doRead(Field &field, TMsg &msg, TIter &iter, std::size_t size,
         TNextLayerReader &&nextLayerReader, TExtraValues... extraValues) {
    ...
    auto fieldLen =
        static_cast<std::size_t>(std::distance(beforeReadIter, iter));

    using Tag = typename comms::util::LazyShallowConditional<
        comms::isMessage<typename std::decay<decltype(msg)>::type>()>::
        template Type<DirectOpTag, PointerOpTag>;

    return doReadInternal(field, msg, iter, size - fieldLen,
  ...
  ///     @ref comms::option::def::StaticNumIdImpl option to specify its numeric
  ///     ID), its defined @b doGetId() member function (see
  ///     comms::MessageBase::doGetId()) non virtual function is called.
  ///     Otherwise polymorphic @b getId() member function is used to retrieve
  ///     the message ID information, which means the message interface class
  ///     must use @ref comms::option::app::IdInfoInterface option to define
  ...
  /// @param[in] msg Reference to message object, actual class of which must be
  /// a descendant of the @ref comms::Message.
  ...
  ErrorStatus doWrite(Field &field, const TMsg &msg, TIter &iter,
    ...
    thisObj.prepareFieldForWrite(getMsgId(msg, IdRetrieveTag<MsgType>()), msg,
                                 field);
    ...
    using Tag = typename comms::util::LazyShallowConditional<
        ...
        MsgType::hasWrite()>::template Type<DirectOpTag, StaticBinSearchOpTag>;

    return writeInternal(field, msg, iter, size - field.length(),
  ...
  /// @param[out] reason Failure reason in case creation has failed. May be
  /// nullptr.
...
protected:
  ...
  ///     class or message object itself (depending on how doRead() was
  ///     invoked).
  /// @note May be non-static in the extending class
  ...
  ///     May be overridden by the extending class if some complex functionality
  ///     is required.
  /// @param[in] id ID of the message
  ...
  /// @note May be non-static in the extending class
  template <typename TMsg>
  static void prepareFieldForWrite(MsgIdParamType id, const TMsg &msg,
                                   Field &field) {
    static_cast<void>(msg);
    static_cast<void>(field);
    field.setValue(id);
...
private:
  ...
  template <typename TMsg>
  using IdRetrieveTag =
      typename comms::util::LazyShallowConditional<comms::isMessageBase<
          TMsg>()>::template Type<DirectOpTag, PolymorphicOpTag>;
  ...
  using IdParamTag = typename comms::util::LazyShallowConditional<
      std::is_base_of<MsgIdType, TId>::value>::template Type<IdParamAsIsTag,
                                                             IdParamCastTag>;
  ...
  public:
    ...
    ReadRedirectionHandler(TIter &iter, std::size_t size,
                           TNextLayerReader &&nextLayerReader,
                           TExtraValues... extraValues)
        : m_iter(iter), m_size(size),
          m_nextLayerReader(std::forward<TNextLayerReader>(nextLayerReader)),
          m_extraValues(extraValues...) {}

    template <typename TMsg> RetType handle(TMsg &msg) {
      static_assert(comms::isMessageBase<TMsg>(),
  ...
  private:
    template <typename TMsg> RetType handleInternal(TMsg &msg, std::tuple<> &) {
    ...
    RetType handleInternal(TMsg &msg, std::tuple<T0, T1, T2> &extraValues) {
      return m_nextLayerReader.read(
          msg, m_iter, m_size, std::get<0>(extraValues),
          std::get<1>(extraValues), std::get<2>(extraValues));
    }
    ...
    template <typename TMsg, typename T0, typename T1, typename T2, typename T3,
              typename T4>
    RetType handleInternal(TMsg &msg,
                           std::tuple<T0, T1, T2, T3, T4> &extraValues) {
      return m_nextLayerReader.read(
          msg, m_iter, m_size, std::get<0>(extraValues),
          std::get<1>(extraValues), std::get<2>(extraValues),
          std::get<3>(extraValues), std::get<4>(extraValues));
    }

    template <typename TMsg, typename T0, typename T1, typename T2, typename T3,
              typename T4, typename T5>
    RetType handleInternal(TMsg &msg,
                           std::tuple<T0, T1, T2, T3, T4, T5> &extraValues) {
      return m_nextLayerReader.read(
          msg, m_iter, m_size, std::get<0>(extraValues),
          std::get<1>(extraValues), std::get<2>(extraValues),
          std::get<3>(extraValues), std::get<4>(extraValues),
  ...
  template <typename TIter, typename TNextLayerReader, typename... TExtraValues>
  ReadRedirectionHandler<TIter, TNextLayerReader, TExtraValues...>
  makeReadRedirectionHandler(TIter &iter, std::size_t size,
                             TNextLayerReader &&nextLayerReader,
                             TExtraValues... extraValues) {
    return ReadRedirectionHandler<TIter, TNextLayerReader, TExtraValues...>(
        iter, size, std::forward<TNextLayerReader>(nextLayerReader),
        extraValues...);
  }
  ...
  public:
    ...
    WriteRedirectionHandler(TIter &iter, std::size_t size,
                            TNextLayerWriter &&nextLayerWriter)
        : m_iter(iter), m_size(size),
          m_nextLayerWriter(std::forward<TNextLayerWriter>(nextLayerWriter)) {}

    template <typename TMsg> RetType handle(const TMsg &msg) {
      static_assert(comms::isMessageBase<TMsg>(),
                    "Expected to be a valid message object");
  ...
  template <typename TIter, typename TNextLayerWriter>
  WriteRedirectionHandler<TIter, TNextLayerWriter>
  makeWriteRedirectionHandler(TIter &iter, std::size_t size,
                              TNextLayerWriter &&nextLayerWriter) {
    return WriteRedirectionHandler<TIter, TNextLayerWriter>(
        iter, size, std::forward<TNextLayerWriter>(nextLayerWriter));
  }

  template <typename TMsg, typename... TParams>
  static MsgIdParamType getMsgId(const TMsg &msg,
                                 PolymorphicOpTag<TParams...>) {
    using MsgType = typename std::decay<decltype(msg)>::type;
    static_assert(
    ...
    static_assert(
        MsgType::hasGetId(),
        "The message interface class must expose polymorphic ID retrieval "
        "functionality, "
  ...
  static constexpr MsgIdParamType getMsgId(const TMsg &msg,
    ...
    static_assert(
        ...
        "comms::option::app::HasDoGetId option to mark the class as providing "
        "one");
    return msg.doGetId();
  }
  ...
  template <typename TMsg, typename TIter, typename TNextLayerReader,
            typename... TExtraValues>
  comms::ErrorStatus doReadInternalDirect(Field &field, TMsg &msg, TIter &iter,
                                          std::size_t size,
                                          TNextLayerReader &&nextLayerReader,
                                          TExtraValues... extraValues) {
    static_assert(
        (!details::hasMsgIndexRetriever<TExtraValues...>()) ||
            (comms::details::allMessagesAreStrongSorted<AllMessages>()),
        "Message type index cannot be retrieved with direct message object "
        "invocation");
    auto &thisObj = BaseImpl::thisLayer();
  ...
  template <typename TMsg, typename TIter, typename TNextLayerReader,
            typename... TExtraValues>
  comms::ErrorStatus
  doReadInternal(MsgIdParamType id, unsigned idx, TMsg &msg, TIter &iter,
                 std::size_t size, TNextLayerReader &&nextLayerReader,
                 PolymorphicOpTag<>, TExtraValues... extraValues) {
    static_cast<void>(id);
  ...
  template <typename TMsg, typename TIter, typename TNextLayerReader,
            typename... TExtraValues>
  comms::ErrorStatus
  doReadInternal(Field &field, TMsg &msg, TIter &iter, std::size_t size,
                 TNextLayerReader &&nextLayerReader, DirectOpTag<>,
                 TExtraValues... extraValues) {
    return doReadInternalDirect(field, msg, iter, size,
                                std::forward<TNextLayerReader>(nextLayerReader),
                                extraValues...);
  }
  ...
  comms::ErrorStatus
  doReadInternal(Field &field, TMsg &msg, TIter &iter, std::size_t size,
                 TNextLayerReader &&nextLayerReader, PointerOpTag<>,
                 TExtraValues... extraValues) {
    using MsgType = typename std::decay<decltype(msg)>::type;
    static_assert(comms::details::hasElementType<MsgType>(),
                  "Unsupported type of message object, expected to be either "
                  "message itself or smart pointer");

    using MsgElementType = typename MsgType::element_type;

    using Tag = typename comms::util::LazyShallowConditional<
        MsgElementType::hasRead()>::template Type<PolymorphicOpTag,
    ...
    while (true) {
      ...
      thisObj.beforeRead(field, *msg);
      es = doReadInternal(id, idx, msg, iter, size,
                          std::forward<TNextLayerReader>(nextLayerReader),
                          Tag(), extraValues...);
    ...
    using GenericMsgTag = typename comms::util::LazyShallowConditional<
        MsgFactory::hasGenericMessageSupport()>::template Type<HasGenericMsgTag,
                                                               NoGenericMsgTag>;

    return createAndReadGenericMsgInternal(
        field, idx, msg, iter, size,
        std::forward<TNextLayerReader>(nextLayerReader), es, GenericMsgTag(),
        extraValues...);
  }
  ...
  doReadInternal(MsgIdParamType id, unsigned idx, TMsg &msg, TIter &iter,
                 std::size_t size, TNextLayerReader &&nextLayerReader,
                 StaticBinSearchOpTag<>, TExtraValues... extraValues) {
    auto handler = makeReadRedirectionHandler(
        iter, size, std::forward<TNextLayerReader>(nextLayerReader),
        extraValues...);
    return comms::dispatchMsgStaticBinSearch<AllMessages>(id, idx, *msg,
                                                          handler);
  }

  template <typename TId, typename... TParams>
  MsgPtr createMsgInternalTagged(TId &&id, unsigned idx,
                                 CreateFailureReason *reason,
                                 IdParamAsIsTag<TParams...>) {
  ...
  MsgPtr createMsgInternal(TId &&id, unsigned idx,
    ...
    return createMsgInternalTagged(std::forward<TId>(id), idx, reason,
                                   IdParamTag<IdType>());
  }

  template <typename TId, typename... TParams>
  MsgPtr createGenericMsgInternalTagged(TId &&id, unsigned idx,
                                        IdParamAsIsTag<TParams...>) {
  ...
  MsgPtr createGenericMsgInternal(TId &&id, unsigned idx) {
    using IdType = typename std::decay<decltype(id)>::type;
    return createGenericMsgInternalTagged(std::forward<TId>(id), idx,
                                          IdParamTag<IdType>());
  }
  ...
  comms::ErrorStatus createAndReadGenericMsgInternal(
      const Field &field, unsigned msgIdx, TMsg &msg, TIter &iter,
      std::size_t size, TNextLayerReader &&nextLayerReader,
      comms::ErrorStatus es, NoGenericMsgTag<>, TExtraValues...) {
    static_cast<void>(field);
  ...
  comms::ErrorStatus createAndReadGenericMsgInternal(
      const Field &field, unsigned msgIdx, TMsg &msg, TIter &iter,
      std::size_t size, TNextLayerReader &&nextLayerReader,
      comms::ErrorStatus es, HasGenericMsgTag<>, TExtraValues... extraValues) {
    ...
    using Tag = typename comms::util::LazyShallowConditional<
        GenericMsgType::hasRead()>::template Type<PolymorphicOpTag,
                                                  DirectOpTag>;

    return readGenericMsg(msg, iter, size,
                          std::forward<TNextLayerReader>(nextLayerReader),
                          Tag(), extraValues...);
  }
  ...
  template <typename TMsg, typename TIter, typename TNextLayerReader,
            typename... TExtraValues>
  comms::ErrorStatus readGenericMsg(TMsg &msg, TIter &iter, std::size_t size,
                                    TNextLayerReader &&nextLayerReader,
                                    PolymorphicOpTag<>,
                                    TExtraValues... extraValues) {
    return nextLayerReader.read(msg, iter, size, extraValues...);
  ...
  template <typename TId> std::size_t msgCountInternal(TId &&id) {
    using IdType = typename std::decay<decltype(id)>::type;
  ...
  template <typename TMsg, typename TIter, typename TNextLayerWriter,
            typename... TParams>
  ErrorStatus writeInternal(Field &field, const TMsg &msg, TIter &iter,
                            std::size_t size,
                            TNextLayerWriter &&nextLayerWriter,
  ...
  ErrorStatus writeInternal(Field &field, const TMsg &msg, TIter &iter,
    ...
    auto &thisObj = BaseImpl::thisLayer();
    auto handler = makeWriteRedirectionHandler(
        iter, size, std::forward<TNextLayerWriter>(nextLayerWriter));
    auto id = thisObj.getMsgIdFromField(field);
...
namespace details {
template <typename T> struct MsgIdLayerCheckHelper {
...
/// @related MsgIdLayer
template <typename T> constexpr bool isMsgIdLayer() {

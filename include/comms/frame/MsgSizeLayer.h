...
///         functionality. See also @ref page_custom_size_layer tutorial page.
/// @headerfile comms/frame/MsgSizeLayer.h
/// @extends comms::frame::FrameLayerBase
template <typename TField, typename TNextLayer, typename... TOptions>
class MsgSizeLayer
    : public comms::frame::details::MsgSizeLayerBase<TField, TNextLayer,
                                                     TOptions...> {
  using BaseImpl =
      comms::frame::details::MsgSizeLayerBase<TField, TNextLayer, TOptions...>;
...
public:
  ...
  ///          reports it's not enough (returns
  ...
  doRead(Field &field, TMsg &msg, TIter &iter, std::size_t size,
    ...
    static_assert(
        ...
        "Current implementation of MsgSizeLayer requires iterator used for "
        "reading to be random-access one.");
    ...
    std::size_t requiredRemainingSize =
        thisObj.getRemainingSizeFromField(field);

    if (actualRemainingSize < requiredRemainingSize) {
      BaseImpl::setMissingSize(requiredRemainingSize - actualRemainingSize,
                               extraValues...);
  ...
  comms::ErrorStatus doUpdate(Field &field, TIter &iter, std::size_t size,
    ...
    using ConstNullptrType =
        typename details::MsgSizeLayerConstNullPtrCastHelper<
            !std::is_void<LocalMsgPtr>::value>::template Type<LocalMsgPtr>;
    auto noMsgPtr = static_cast<ConstNullptrType>(nullptr);
    return doUpdateInternal(noMsgPtr, field, iter, size,
                            std::forward<TNextLayerUpdater>(nextLayerUpdater),
                            NoMsgTypeTag<>());
  }

  /// @brief Customized update functionality, invoked by @ref update().
  /// @details Similar to other @ref comms::frame::MsgSizeLayer::doUpdate()
  /// "doUpdate()",
  ...
  comms::ErrorStatus doUpdate(const TMsg &msg, Field &field, TIter &iter,
                              std::size_t size,
                              TNextLayerUpdater &&nextLayerUpdater) const {
    return doUpdateInternal(&msg, field, iter, size,
                            std::forward<TNextLayerUpdater>(nextLayerUpdater),
                            ValidMsgTypeTag<>());
  }
...
protected:
  ...
  ///     invoked). Can be @b nullptr in case message object hasn't been created
  ///     yet
  ...
  /// @param[in] msg Pointer to message object being written, may be nullptr (in
  /// case invoked
  ///     from @ref comms::frame::MsgSizeLayer::doUpdate "doUpdate()")
  /// @param[out] field Field, value of which needs to be populated
  /// @note May be non-static in the extending class
  template <typename TMsg>
  static void prepareFieldForWrite(std::size_t size, const TMsg *msg,
                                   Field &field) {
    static_cast<void>(msg);
    field.setValue(size);
  }
...
private:
  ...
  template <typename TMsg>
  using MsgLengthTag = typename comms::util::LazyShallowConditional<
      details::FrameLayerHasFieldsImpl<TMsg>::Value ||
      TMsg::hasLength()>::template Type<MsgHasLengthTag, MsgNoLengthTag>;

  template <typename TMsg, typename TIter, typename TWriter>
  ErrorStatus writeInternalHasLength(Field &field, const TMsg &msg, TIter &iter,
                                     std::size_t size,
  ...
  ErrorStatus writeInternalOutput(Field &field, const TMsg &msg, TIter &iter,
    ...
    if ((es != ErrorStatus::Success) && (es != ErrorStatus::UpdateRequired)) {
      return es;
    }
  ...
  template <typename TMsg, typename TIter, typename TWriter>
  ErrorStatus
  writeInternalNoLengthTagged(Field &field, const TMsg &msg, TIter &iter,
  ...
  template <typename TMsg, typename TIter, typename TWriter>
  ErrorStatus writeInternalNoLengthTagged(Field &field, const TMsg &msg,
                                          TIter &iter, std::size_t size,
  ...
  template <typename TMsg, typename TIter, typename TWriter>
  ErrorStatus writeInternalNoLength(Field &field, const TMsg &msg, TIter &iter,
                                    std::size_t size,
                                    TWriter &&nextLayerWriter) const {
    static_assert(
    ...
    return writeInternalNoLengthTagged(
        field, msg, iter, size, std::forward<TWriter>(nextLayerWriter), Tag());
  }
  ...
  ErrorStatus writeInternal(Field &field, const TMsg &msg, TIter &iter,
    ...
    return writeInternalHasLength(field, msg, iter, size,
                                  std::forward<TWriter>(nextLayerWriter));
  }
  ...
  ErrorStatus writeInternal(Field &field, const TMsg &msg, TIter &iter,
    ...
    return writeInternalNoLength(field, msg, iter, size,
                                 std::forward<TWriter>(nextLayerWriter));
  }

  template <typename TMsg, typename... TParams>
  constexpr std::size_t fieldLengthInternal(const TMsg &msg,
                                            FixedLengthTag<TParams...>) const {
    return BaseImpl::doFieldLength(msg);
  }

  template <typename TMsg, typename... TParams>
  std::size_t fieldLengthInternal(const TMsg &msg,
  ...
  comms::ErrorStatus doUpdateInternal(const TMsg *msg, Field &field,
                                      TIter &iter, std::size_t size,
    ...
    return doUpdateForward(msg, iter, size - field.length(),
                           std::forward<TNextLayerUpdater>(nextLayerUpdater),
                           tag);
  }
  ...
  template <typename TMsg, typename TIter, typename TNextLayerUpdater,
            typename... TParams>
  comms::ErrorStatus doUpdateForward(const TMsg *msg, TIter &iter,
                                     std::size_t size,
                                     TNextLayerUpdater &&nextLayerUpdater,
...
namespace details {
template <typename T> struct MsgSizeLayerCheckHelper {
...
template <typename T> constexpr bool isMsgSizeLayer() {

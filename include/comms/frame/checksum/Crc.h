...
template <typename TResult> using CrcInitTableType = std::array<TResult, 256>;

template <typename TResult, TResult TPoly> struct CrcInitTable {
...
template <> struct CrcInitTable<std::uint16_t, 0x1021> {
...
template <> struct CrcInitTable<std::uint16_t, 0x8005> {
  using Table = CrcInitTableType<std::uint16_t>;
  static const Table &get() {
    static const Table table = {
...
template <> struct CrcInitTable<std::uint32_t, 0x04c11db7> {
...
public:
  ...
  TResult operator()(TIter &iter, std::size_t len) const {
    ...
    for (std::size_t byte = 0U; byte < len; ++byte) {
      ...
      comms::cast_assign(val) =
          reflect(val) ^ static_cast<decltype(val)>(rem >> (Width - 8));
      comms::cast_assign(rem) =
          initTable[val] ^ static_cast<decltype(rem)>(rem << 8);
      ++iter;
    }
...
private:
  ...
  template <typename...>
  using ReflectTag = typename comms::util::LazyShallowConditional<
      TReflect>::template Type<DoReflectTag, NoReflectTag>;

  template <typename...>
  using ReflectRemTag = typename comms::util::LazyShallowConditional<
      TReflectRem>::template Type<DoReflectTag, NoReflectTag>;
  ...
  static TVal reflectInternal(TVal value, std::size_t bitsCount,
                              DoReflectTag<TParams...>) {
    return static_cast<TVal>(doReflect(value, bitsCount));
  }

  template <typename TVal, typename... TParams>
  static constexpr TVal reflectInternal(TVal value, std::size_t,
                                        NoReflectTag<TParams...>) {
    return value;
  }
...
///     @li @b Using reflection for final value
using Crc_32 =
    Crc<std::uint32_t, 0x04c11db7, 0xffffffff, 0xffffffff, true, true>;

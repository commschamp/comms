...
public:
    ...
    ///     @li @ref comms::frame::FrameLayerBase::updateMissingSize() "updateMissingSize()" -
    ///     to calculate update missing size if such is requested.
    ///     @li @ref comms::frame::FrameLayerBase::setMissingSize() "setMissingSize()" -
    ...
    /// @brief Update recently written (using write()) message contents data.
    /// @details Similar to other @ref comms::frame::FrameLayerBase::update() "update()"
    ...
    /// @brief Default implementation of the "update" functaionality.
    /// @details Similar to other @ref comms::frame::FrameLayerBase::doUpdate() "doUpdate()"
    ///     but also receiving recently written message object.
    /// @tparam TMsg Type of message object.
    /// @tparam TIter Type of iterator used for updating.
    /// @tparam TNextLayerWriter next layer updater object type.
...
protected:
    ...
    ///     "read" function such as @ref comms::frame::FrameLayerBase::read() "read()"
    ///     or @ref comms::frame::FrameLayerBase::readFieldsCached() "readFieldsCached()"
...
///     case "read" of frame operation returns @ref comms::ErrorStatus::NotEnoughData.
///     @code
///     using Frame = ...
///     Frame frame;
///     Frame::MsgPtr msg;
///     std::size_t missingSize = 0U;
///     auto es = frame.read(msg, readIter, size, comms::frame::missingSize(missingSize));
...
///     auto es = frame.read(msg, readIter, size, comms::frame::msgPayload(payloadIter, payloadSize));

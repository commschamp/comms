...
/// @brief Base class for all the middle (non @ref MsgDataLayer) protocol
/// transport layers.
...
///     function with the same signature. The @ref NextLayer is stored as a
///     private data meber.
...
public:
  ...
  using AllFields = typename std::decay<decltype(std::tuple_cat(
      std::declval<std::tuple<Field>>(),
      std::declval<typename TNextLayer::AllFields>()))>::type;
  ...
  NextLayer &nextLayer() { return m_nextLayer; }

  /// @brief Get "const" access to the next layer object.
  const NextLayer &nextLayer() const { return m_nextLayer; }

  /// @brief Get access to this layer object.
  ThisLayer &thisLayer() { return static_cast<ThisLayer &>(*this); }
  ...
  static constexpr bool canSplitRead() {
    return (!ParsedOptions::HasDisallowReadUntilDataSplit) &&
           NextLayer::canSplitRead();
  }
  ...
  ///         typename... TExtraValues> comms::ErrorStatus doRead(
  ///             Field& field, // field object used to read required data
  ///             TMsg& msg, // Ref to smart pointer to message object, or
  ///             message object itself TIter& iter, // iterator used for
  ///             reading std::size_t size, // size of the remaining data
  ///             TNextLayerReader&& nextLayerReader, // next layer reader
  ///             object TExtraValues... extraValues)
  ///         {
  ///             // internal logic prior next layer read, such as reading the
  ///             field value auto es = field.read(iter, size);
  ///             ...
  ///             // request next layer to perform read operation
  ///             es = nextLayerReader.read(msg, iter, size - field.length(),
  ///             extraValues...);
  ...
  ///     @li @ref comms::frame::FrameLayerBase::updateMissingSize()
  ///     "updateMissingSize()" - to calculate update missing size if such is
  ///     requested.
  ///     @li @ref comms::frame::FrameLayerBase::setMissingSize()
  ///     "setMissingSize()" - to set specific value as missing size if suce
  ///     information is requested.
  ///     @li @ref comms::frame::FrameLayerBase::setMsgId() "setMsgId()" -
  ///     to set the value of message id
  ///     @li @ref comms::frame::FrameLayerBase::setMsgIndex() "setMsgIndex()" -
  ...
  ///     of the following functions: @ref comms::frame::missingSize(),
  ///     @ref comms::frame::msgId(), @ref comms::frame::msgIndex(),
  ///     @ref comms::frame::msgPayload().
  ...
  comms::ErrorStatus read(TMsg &msg, TIter &iter, std::size_t size,
                          TExtraValues... extraValues) {
    using Tag = typename comms::util::LazyShallowConditional<
        ParsedOptions::HasForceReadUntilDataSplit>::
        template Type<SplitReadTag, NormalReadTag>;

    static_assert(
        std::is_same<Tag, NormalReadTag<>>::value || canSplitRead(),
  ...
  /// @details Same as @b read by stops read operation when data layer is
  /// reached.
  ...
  template <typename TMsg, typename TIter, typename... TExtraValues>
  comms::ErrorStatus readUntilData(TMsg &msg, TIter &iter, std::size_t size,
                                   TExtraValues... extraValues) {
  ...
  ///       advanced will pinpoint the location of the error.
  template <typename TMsg, typename TIter, typename... TExtraValues>
  comms::ErrorStatus readFromData(TMsg &msg, TIter &iter, std::size_t size,
                                  TExtraValues... extraValues) {
  ...
  ///     transport fields (@ref AllFields type of the last protocol layer
  ///     class).
  /// @param[in, out] msg Reference to smart pointer, that already holds or
  ///     will hold allocated message object, or reference to actual message
  ///     object (which extends @ref comms::MessageBase).
  ...
  comms::ErrorStatus readFieldsCached(TAllFields &allFields, TMsg &msg,
                                      TIter &iter, std::size_t size,
                                      TExtraValues... extraValues) {
    using AllFieldsDecayed = typename std::decay<TAllFields>::type;
    static_assert(util::tupleIsTailOf<AllFields, AllFieldsDecayed>(),
                  "Passed tuple is wrong.");
    static const std::size_t Idx = std::tuple_size<AllFieldsDecayed>::value -
  ...
  /// while caching
  ///     the read transport information fields.
  /// @details Very similar to @ref readUntilData() member function, but adds
  /// "allFields"
  ///     parameter to store read transport information fields.
  ///     The function will also invoke the same @b doRead() member function
  ///     provided by the derived class, as described with @ref read().
  ...
  template <typename TAllFields, typename TMsg, typename TIter,
            typename... TExtraValues>
  comms::ErrorStatus readUntilDataFieldsCached(TAllFields &allFields, TMsg &msg,
                                               TIter &iter, std::size_t size,
                                               TExtraValues... extraValues) {
    using AllFieldsDecayed = typename std::decay<TAllFields>::type;
    static_assert(util::tupleIsTailOf<AllFields, AllFieldsDecayed>(),
  ...
  /// @brief Finalise the read operation by reading the message payload while
  /// caching
  ///     the read transport information fields.
  /// @details Should be called to finalise the read operation started by
  ...
  template <typename TAllFields, typename TMsg, typename TIter,
            typename... TExtraValues>
  comms::ErrorStatus readFromDataFieldsCached(TAllFields &allFields, TMsg &msg,
                                              TIter &iter, std::size_t size,
                                              TExtraValues... extraValues) {
    return nextLayer().readFromDataFieldsCached(allFields, msg, iter, size,
                                                extraValues...);
  }
  ...
  ///             Field& field, // field object used to update and write
  ///             required data const TMsg& msg, // reference to ready to be
  ///             sent message object TIter& iter, // iterator used for writing
  ...
  comms::ErrorStatus write(const TMsg &msg, TIter &iter,
  ...
  /// @brief Serialise message into output data sequence while caching the
  /// written transport
  ///     information fields.
  /// @details Very similar to @ref write() member function, but adds
  /// "allFields"
  ...
  /// @return Status of the write operation.
  template <typename TAllFields, typename TMsg, typename TIter>
  comms::ErrorStatus writeFieldsCached(TAllFields &allFields, const TMsg &msg,
                                       TIter &iter, std::size_t size) const {
    using AllFieldsDecayed = typename std::decay<TAllFields>::type;
    static_assert(util::tupleIsTailOf<AllFields, AllFieldsDecayed>(),
                  "Passed tuple is wrong.");
    static const std::size_t Idx = std::tuple_size<AllFieldsDecayed>::value -
  ...
  ///     It performs a call to @ref doFieldLength() member function with
  ///     message parameter to get info about current field length. To update
  ///     the default behaviour just override the function in the derived class.
  /// @tparam TMsg Type of message object.
  /// @param[in] msg Message object
  /// @return length of the field + length reported by the next layer.
  template <typename TMsg> constexpr std::size_t length(const TMsg &msg) const {
  ...
  ///     provided (or inherited) by the derived class, which must have the
  ///     following signature and logic:
  ///     @code
  ///         template<typename TIter, typename TNextLayerUpdater>
  ///         comms::ErrorStatus doUpdate(
  ///             Field& field, // field object to update and re-write if
  ///             necessary TIter& iter, // iterator used for updateing
  ///             std::size_t size, // Number of remaning bytes in the output
  ///             buffer. TNextLayerUpdater&& nextLayerUpdater // next layer
  ///             updater object
  ///             )
  ///         {
  ...
  /// @brief Update recently written (using write()) message contents data.
  /// @details Similar to other @ref comms::frame::FrameLayerBase::update()
  /// "update()"
  ...
  /// @return Status of the update operation.
  template <typename TMsg, typename TIter>
  comms::ErrorStatus update(const TMsg &msg, TIter &iter,
                            std::size_t size) const {
    Field field;
    auto &derivedObj = static_cast<const TDerived &>(*this);
    auto updater = createNextLayerUpdater();
  ...
  /// @details Very similar to @ref update() member function, but adds
  /// "allFields"
  ...
  /// @param[in] size Number of bytes that have been written using
  /// writeFieldsCached().
  /// @return Status of the update operation.
  template <typename TAllFields, typename TIter>
  ErrorStatus updateFieldsCached(TAllFields &allFields, TIter &iter,
                                 std::size_t size) const {
    using AllFieldsDecayed = typename std::decay<TAllFields>::type;
    static_assert(util::tupleIsTailOf<AllFields, AllFieldsDecayed>(),
                  "Passed tuple is wrong.");
  ...
  /// @return Status of the update operation.
  template <typename TAllFields, typename TMsg, typename TIter>
  ErrorStatus updateFieldsCached(TAllFields &allFields, const TMsg &msg,
                                 TIter &iter, std::size_t size) const {
    using AllFieldsDecayed = typename std::decay<TAllFields>::type;
    static_assert(util::tupleIsTailOf<AllFields, AllFieldsDecayed>(),
                  "Passed tuple is wrong.");
  ...
  ///     member function, unless the derived class provides its own @ref
  ///     doUpdate() member function to override the default behavior.@n This
  ///     function in this layer does nothing, just advances the iterator by the
  ///     length of the @ref Field.
  ...
  template <typename TIter, typename TNextLayerUpdater>
  comms::ErrorStatus doUpdate(Field &field, TIter &iter, std::size_t size,
                              TNextLayerUpdater &&nextLayerUpdater) const {
    return updateInternal(field, iter, size,
                          std::forward<TNextLayerUpdater>(nextLayerUpdater),
                          LengthTag<>());
  }

  /// @brief Default implementation of the "update" functaionality.
  /// @details Similar to other @ref comms::frame::FrameLayerBase::doUpdate()
  /// "doUpdate()"
  ///     but also receiving recently written message object.
  /// @tparam TMsg Type of message object.
  /// @tparam TIter Type of iterator used for updating.
  ...
  comms::ErrorStatus doUpdate(const TMsg &msg, Field &field, TIter &iter,
                              std::size_t size,
                              TNextLayerUpdater &&nextLayerUpdater) const {
    return updateInternal(msg, field, iter, size,
                          std::forward<TNextLayerUpdater>(nextLayerUpdater),
                          LengthTag<>());
  }
  ...
  static constexpr std::size_t doFieldLength() { return Field::minLength(); }
  ...
  ///     layer. One of the layers (usually comms::frame::MsgIdLayer)
  ...
  ///     object
  template <typename TId> MsgPtr createMsg(TId &&id, unsigned idx = 0) {
  ...
  static auto accessCachedField(TAllFields &allFields)
      -> decltype(std::get<std::tuple_size<
                               typename std::decay<TAllFields>::type>::value -
                           std::tuple_size<AllFields>::value>(allFields)) {
    using AllFieldsDecayed = typename std::decay<TAllFields>::type;
    static_assert(util::tupleIsTailOf<AllFields, AllFieldsDecayed>(),
                  "Passed tuple is wrong.");
    static const std::size_t Idx = std::tuple_size<AllFieldsDecayed>::value -
...
protected:
  ...
  ///     passed field object. The function can be overriden by the extending
  ///     class.
  /// @param[in] msgPtr Pointer to message object (if available), can be
  /// nullptr.
  ...
  template <typename TMsg, typename TIter>
  static comms::ErrorStatus doWriteField(const TMsg *msgPtr, const Field &field,
                                         TIter &iter, std::size_t len) {
    static_cast<void>(msgPtr);
  ...
  ///     @b false otherwise.
  template <typename T> static constexpr bool isMessageObjRef() {
  ...
  template <typename TMsg> static void resetMsg(TMsg &msg) {
    resetMsgInternal(msg,
                     MsgTypeTag<typename std::decay<decltype(msg)>::type>());
  }
...
private:
  ...
  template <typename TMsg>
  using MsgTypeTag = typename comms::util::LazyShallowConditional<
      comms::isMessage<TMsg>()>::template Type<MessageObjTag, SmartPtrTag>;

  template <typename TMsg, typename... TParams>
  static auto toMsgPtrInternal(TMsg &msg,
                               MessageObjTag<TParams...>) -> decltype(&msg) {
    return &msg;
  }

  template <typename TMsg, typename... TParams>
  static auto toMsgPtrInternal(TMsg &msg,
                               SmartPtrTag<TParams...>) -> decltype(msg.get()) {
    return msg.get();
  }

protected:
  /// @brief Get a pointer to the message object.
  /// @details The function works seamlessly for both smart pointer and
  /// reference
  ///     to the real object
  /// @see @ref isMessageObjRef().
  template <typename TMsg>
  static auto toMsgPtr(TMsg &msg)
      -> decltype(toMsgPtrInternal(
          msg, MsgTypeTag<typename std::decay<decltype(msg)>::type>())) {
    return toMsgPtrInternal(
        msg, MsgTypeTag<typename std::decay<decltype(msg)>::type>());
  }
  ...
  ///     "read" operation using @ref comms::frame::missingSize().
  /// @param[in] size Remaining number of bytes in the input buffer.
  /// @param[out] extraValues Variadic parameters passed to the
  ///     "read" function such as @ref comms::frame::FrameLayerBase::read()
  ///     "read()" or @ref comms::frame::FrameLayerBase::readFieldsCached()
  ///     "readFieldsCached()"
  ...
  ///     "readFieldsCached()"
  template <typename... TExtraValues>
  void updateMissingSize(const Field &field, std::size_t size,
  ...
  /// @details Updates the value reference to which was passed to the
  ///     "read" operation using @ref comms::frame::msgId().
  /// @param[in] val Value to assign.
  /// @param[out] extraValues Variadic parameters passed to the
  ///     "read" function such as @ref comms::frame::FrameLayerBase::read()
  ///     "read()" or @ref comms::frame::FrameLayerBase::readFieldsCached()
  ...
  /// @details Updates the value reference to which was passed to the
  ///     "read" operation using @ref comms::frame::msgIndex().
  /// @param[in] val Value to assign.
  /// @param[out] extraValues Variadic parameters passed to the
  ///     "read" function such as @ref comms::frame::FrameLayerBase::read()
  ///     "read()" or @ref comms::frame::FrameLayerBase::readFieldsCached()
  ...
  static Field &getField(TAllFields &allFields) {
    ...
    static_assert(std::is_same<Field, FieldType>::value,
  ...
  template <typename...>
  using LengthTag = typename comms::util::LazyShallowConditional<(
      MinFieldLength == MaxFieldLength)>::template Type<FixedLengthTag,
                                                        VarLengthTag>;

  class NextLayerReader {
  public:
    explicit NextLayerReader(NextLayer &nextLayer) : m_nextLayer(nextLayer) {}

    template <typename TMsgPtr, typename TIter, typename... TExtraValues>
    ErrorStatus read(TMsgPtr &msg, TIter &iter, std::size_t size,
                     TExtraValues... extraValues) {
  ...
  template <typename TAllFields> class NextLayerCachedFieldsReader {
  public:
    NextLayerCachedFieldsReader(NextLayer &nextLayer, TAllFields &allFields)
        : m_nextLayer(nextLayer), m_allFields(allFields) {}

    template <typename TMsgPtr, typename TIter, typename... TExtraValues>
    ErrorStatus read(TMsgPtr &msg, TIter &iter, std::size_t size,
                     TExtraValues... extraValues) {
      return m_nextLayer.readFieldsCached(m_allFields, msg, iter, size,
                                          extraValues...);
    }
  ...
  template <typename TAllFields> class NextLayerCachedFieldsUntilDataReader {
  public:
    NextLayerCachedFieldsUntilDataReader(NextLayer &nextLayer,
    ...
    ErrorStatus read(TMsgPtr &msg, TIter &iter, std::size_t size,
                     TExtraValues... extraValues) {
      return m_nextLayer.readUntilDataFieldsCache(m_allFields, msg, iter, size,
                                                  extraValues...);
    }
  ...
  template <typename TAllFields> class NextLayerCachedFieldsWriter {
  public:
    NextLayerCachedFieldsWriter(const NextLayer &nextLayer,
                                TAllFields &allFields)
        : m_nextLayer(nextLayer), m_allFields(allFields) {}
  ...
  template <typename TAllFields> class NextLayerCachedFieldsUpdater {
  public:
    NextLayerCachedFieldsUpdater(const NextLayer &nextLayer,
                                 TAllFields &allFields)
        : m_nextLayer(nextLayer), m_allFields(allFields) {}
  ...
  createNextLayerCachedFieldsUntilDataReader(TAllFields &fields) {
    return NextLayerCachedFieldsUntilDataReader<TAllFields>(m_nextLayer,
                                                            fields);
  }
...
private:
  ...
  template <typename TMsg, typename TIter, typename... TExtraValues>
  comms::ErrorStatus readInternal(TMsg &msg, TIter &iter, std::size_t size,
                                  NormalReadTag<>,
                                  TExtraValues... extraValues) {
    Field field;
    auto &derivedObj = static_cast<TDerived &>(*this);
  ...
  template <typename TMsgPtr, typename TIter, typename... TExtraValues>
  comms::ErrorStatus readInternal(TMsgPtr &msgPtr, TIter &iter,
                                  std::size_t size, SplitReadTag<>,
                                  TExtraValues... extraValues) {
    auto fromIter = iter;
  ...
  comms::ErrorStatus updateInternal(Field &field, TIter &iter, std::size_t size,
  ...
  template <typename TMsg, typename TIter, typename TNextLayerUpdater,
            typename... TParams>
  comms::ErrorStatus updateInternal(const TMsg &msg, Field &field, TIter &iter,
                                    std::size_t size,
                                    TNextLayerUpdater &&nextLayerUpdater,
                                    FixedLengthTag<TParams...>) const {
    auto len = field.length();
  ...
  template <typename... TExtraValues>
  void updateMissingSizeInternal(std::size_t size,
  ...
  template <typename... TExtraValues>
  void updateMissingSizeInternal(const Field &field, std::size_t size,
                                 details::MissingSizeRetriever<> retriever,
                                 TExtraValues... extraValues) const {
    static_assert(details::isMissingSizeRetriever<
                      typename std::decay<decltype(retriever)>::type>(),
  ...
  void updateMissingSizeInternal(std::size_t size, T retriever,
                                 TExtraValues... extraValues) const {
  ...
  template <typename T, typename... TExtraValues>
  static void setMissingSizeInternal(std::size_t val, T retriever,
                                     TExtraValues... extraValues) {
    static_cast<void>(retriever);
    static_assert(!details::isMissingSizeRetriever<
                      typename std::decay<decltype(retriever)>::type>(),
  ...
  template <typename TId> static void setMsgIdInternal(TId val) {
    static_cast<void>(val);
  }

  template <typename TId, typename U, typename... TExtraValues>
  static void setMsgIdInternal(TId val, details::MsgIdRetriever<U> retriever,
                               TExtraValues... extraValues) {
    retriever.setValue(val);
  ...
  static void setMsgIdInternal(TId val, T retriever,
                               TExtraValues... extraValues) {
    static_cast<void>(retriever);
    static_assert(!details::isMsgIdRetriever<
                      typename std::decay<decltype(retriever)>::type>(),
                  "Mustn't be message id retriever");
    setMsgIdInternal(val, extraValues...);
  }

  static void setMsgIndexInternal(std::size_t val) { static_cast<void>(val); }
  ...
  template <typename T, typename... TExtraValues>
  static void setMsgIndexInternal(std::size_t val, T retriever,
                                  TExtraValues... extraValues) {
    static_cast<void>(retriever);
    static_assert(!details::isMsgIndexRetriever<
                      typename std::decay<decltype(retriever)>::type>(),
                  "Mustn't be missing size retriever");
    setMsgIndexInternal(val, extraValues...);
  }
...
template <typename TField, typename TNextLayer, typename TDerived,
          typename... TOptions>
FrameLayerBase<TField, TNextLayer, TDerived, TOptions...> &toFrameLayerBase(
    FrameLayerBase<TField, TNextLayer, TDerived, TOptions...> &layer) {
  return layer;
}
...
template <typename TField, typename TNextLayer, typename TDerived,
          typename... TOptions>
constexpr const FrameLayerBase<TField, TNextLayer, TDerived, TOptions...> &
toFrameLayerBase(
    const FrameLayerBase<TField, TNextLayer, TDerived, TOptions...> &layer) {
  return layer;
}

/// @brief Add "missing size" output parameter to frame's "read" operation.
/// @details Can be passed as variadic parameters to "read" functions
///     of frame (see @ref comms::frame::FrameLayerBase::read()
///     and @ref  comms::frame::FrameLayerBase::readFieldsCached()).
///     It can be used to retrieve missing length information in
///     case "read" of frame operation returns @ref
...
///     auto es = frame.read(msg, readIter, size,
///     comms::frame::missingSize(missingSize)); if (es ==
///     comms::ErrorStatus::NotEnoughData) {
///         ... // missingSize will hold a minimal number of bytes that are
///         required to be yet read
///     }
///     @endcode
/// @param[out] val Missing size value to be returned.
/// @return Implementation dependent object accepted by "read" functions.
/// @see @ref comms::frame::FrameLayerBase::read()
/// @see @ref comms::frame::FrameLayerBase::readFieldsCached()
inline details::MissingSizeRetriever<> missingSize(std::size_t &val) {
  return details::MissingSizeRetriever<>(val);
}

/// @brief Add "message ID" output parameter to frame's "read" operation.
/// @details Can be passed as variadic parameters to "read" functions
///     of frame (see @ref comms::frame::FrameLayerBase::read()
///     and @ref  comms::frame::FrameLayerBase::readFieldsCached()).
...
///     auto es = frame.read(msg, readIter, size, comms::frame::msgId(msgId));
///     if (es == comms::ErrorStatus::Success) {
...
/// @see @ref comms::frame::FrameLayerBase::read()
/// @see @ref comms::frame::FrameLayerBase::readFieldsCached()
template <typename TId> details::MsgIdRetriever<TId> msgId(TId &val) {
  return details::MsgIdRetriever<TId>(val);
}

/// @brief Add "message index" output parameter to frame's "read" operation.
...
///     @endcode
/// @param[out] iter Iterator pointing to the begining of the message payload in
/// the
///     input buffer.

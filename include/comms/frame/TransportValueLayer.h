...
///     @ref comms::option::def::ExtraTransportFields option to the interface
///     definition
///     @ref comms::Message class.
///     This layer is a mid level layer, expects other mid level layer or
///     MsgDataLayer to be its next one.
/// @tparam TField Type of the field that is used read / write extra transport
/// value.
...
///     @li  @ref comms::option::def::ExtendingClass - Use this option to
///     provide a class
///         name of the extending class, which can be used to extend existing
///         functionality. See also @ref page_custom_transport_value_layer
///         tutorial page.
///     @li @ref comms::option::def::FrameLayerSuppressReadUntilDataSplitForcing
///     - Use
///         this option when there is a need to disable passing
///         @ref comms::option::def::FrameLayerForceReadUntilDataSplit option to
///         the
///         @ref comms::frame::FrameLayerBase base class. The passing of
///         @ref comms::option::def::FrameLayerForceReadUntilDataSplit option
///         happens when the @ref comms::frame::TransportValueLayer preceeds
///         (wraps)
///         @ref comms::frame::MsgIdLayer and is unable to re-assign the read
///         field value to the message object, because the latter hasn't been
///         created yet.
///     @li @ref comms::option::def::PseudoValue - Mark the handled value to be
///     "pseudo"
...
class TransportValueLayer
    : public comms::frame::details::TransportValueLayerBase<
          TField, TIdx, TNextLayer, TOptions...> {
  using BaseImpl =
      comms::frame::details::TransportValueLayerBase<TField, TIdx, TNextLayer,
                                                     TOptions...>;
  using ParsedOptionsInternal =
      comms::frame::details::TransportValueLayerOptionsParser<TOptions...>;
...
public:
  ...
  /// @brief Compile time inquiry of whether the @ref
  ...
  ///     (implemented by @ref comms::frame::MsgDataLayer), assigns the
  ...
  ///     the message object already has the value of the transport field
  ///     updated.
  ...
  doRead(Field &field, TMsg &msg, TIter &iter, std::size_t size,
    ...
    auto es =
        readFieldInternal(field, msg, iter, size, ValueTag<>(), extraValues...);
  ...
  /// @details If the layer is marked as "pseudo" (using @ref
  /// comms::option::def::PseudoValue)
  ...
  /// comms::option::def::PseudoValue)
  ///     option, then the report length is 0.
  template <typename TMsg> static std::size_t doFieldLength(const TMsg &) {
...
protected:
  /// @brief Re-assign the value from the input field to appropriate transport
  /// field
  ///     in the message object.
  /// @details Default implementation just assigns to the field accessed using
  ///     @b TIdx index passed as the template argument to the class
  ///     definition.@n May be overridden by the extending class if some complex
  ///     functionality is required.
  /// @param[out] msg Reference to the created message object
  /// @param[in] field Field, value of which needs to be re-assigned
  /// @deprecated Use @ref
  /// comms::frame::TransportValueLayer::reassignFieldValueToMsg()
  /// "reassignFieldValueToMsg()"
  ///     instead.
  ...
  static void reassignFieldValue(TMsg &msg, const Field &field) {
    ...
    static_assert(TIdx <
                      std::tuple_size<typename MsgType::TransportFields>::value,
                  "TIdx is too big, exceeds the amount of transport fields "
  ...
  /// @details Copies the field value from the appropriate transport field of
  /// the
...
private:
  ...
  template <typename...>
  using ValueTag = typename comms::util::LazyShallowConditional<
      ParsedOptionsInternal::HasPseudoValue>::template Type<PseudoValueTag,
                                                            NormalValueTag>;

  template <typename... TParams>
  static constexpr std::size_t
  doFieldLengthInternal(PseudoValueTag<TParams...>) {
    return 0U;
  }

  template <typename... TParams>
  static constexpr std::size_t
  doFieldLengthInternal(NormalValueTag<TParams...>) {
    return BaseImpl::doFieldLength();
  }

  template <typename TMsg, typename TIter, typename... TExtraValues>
  comms::ErrorStatus readFieldInternal(Field &field, TMsg &msg, TIter &iter,
                                       std::size_t &len, PseudoValueTag<>,
                                       TExtraValues...) {
  ...
  comms::ErrorStatus writeFieldInternal(Field &field, const TMsg &msg,
...
namespace details {
template <typename T> struct TransportValueLayerCheckHelper {
...
template <typename T> constexpr bool isTransportValueLayer() {

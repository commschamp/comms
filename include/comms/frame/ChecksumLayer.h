...
#include "comms/frame/details/ChecksumLayerBase.h"
...
///     data written by all the wrapped internal layers and append it to the end
///     of the written data. When reading, this layer is responsible to verify
...
///     the
///         @b ChecksumLayer will invoke @b read operation of inner (wrapped)
///         layers and only if it is successful, it will calculate and verify
///         the checksum value. Usage of @ref
...
template <typename TField, typename TCalc, typename TNextLayer,
          typename... TOptions>
class ChecksumLayer
    : public comms::frame::details::ChecksumLayerBase<TField, TCalc, TNextLayer,
                                                      TOptions...> {
  using BaseImpl =
      comms::frame::details::ChecksumLayerBase<TField, TCalc, TNextLayer,
                                               TOptions...>;
...
public:
  ...
  /// @brief Customized update functionality, invoked by @ref update().
  /// @details Similar to other @ref comms::frame::ChecksumLayer::doUpdate()
  /// "doUpdate()",
...
protected:
  ...
  /// @note May be non-static in the extending class. In case of non-static must
  /// be const.
...
private:
  ...
  ErrorStatus readVerify(Field &field, TMsg &msg, TIter &iter, std::size_t size,
    ...
    bool checksumValid = false;
    auto checksum = thisObj.calculateChecksum(BaseImpl::toMsgPtr(msg), fromIter,
                                              len, checksumValid);
...
namespace details {
template <typename T> struct ChecksumLayerCheckHelper {
...
template <typename T> constexpr bool isChecksumLayer() {

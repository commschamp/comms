...
///     field type definition must use options (@ref
///     comms::option::def::DefaultNumValue) to specify its default value to be
///     equal to the expected "sync" value.
...
///         functionality. See also @ref page_custom_sync_prefix_layer.
...
class SyncPrefixLayer
    : public comms::frame::details::SyncPrefixLayerBase<TField, TNextLayer,
                                                        TOptions...> {
  using BaseImpl =
      comms::frame::details::SyncPrefixLayerBase<TField, TNextLayer,
                                                 TOptions...>;
  using ParsedOptionsInternal =
      comms::frame::details::SyncPrefixLayerOptionsParser<TOptions...>;
...
public:
  ...
  /// @brief Customized read functionality, invoked by @ref
...
private:
  ...
  ErrorStatus readInternal(Field &field, TMsg &msg, TIter &iter,
    ...
    auto remSize =
        size - static_cast<std::size_t>(std::distance(fromIter, iter));
...
namespace details {
template <typename T> struct SyncPrefixLayerCheckHelper {
...
template <typename T> constexpr bool isSyncPrefixLayer() {

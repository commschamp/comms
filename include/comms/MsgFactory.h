...
///     message. This class @b DOESN'T use dynamic memory allocation to store
///     its internal data structures, hence can be used in any bare-metal and
///     other embedded environment.@n The types of all messages provided in @b
///     TAllMessages are analysed at compile time and best "id to message
///     object" mapping strategy is chosen, unless the "dispatch" type forcing
///     options are used (see description below).
...
///     same ID, but implemented as different classes) are also supported.
///     However they must follow one another in this std::tuple, i.e. be sorted.
/// @tparam TOptions Zero or more options. The supported options are:
///     @li @ref comms::option::app::InPlaceAllocation - Option to specify that
///     custom
///         message objects are @b NOT allocated using dynamic memory, instead
///         an uninitialised area of memory in private members is used to
///         contain any type of custom message (provided with TAllMessages
///         template parameter) and placement "new" operator is used to
///         initialise requested message in this area. The allocated message
///         objects are returned from createMsg() function wrapped in the smart
///         pointer (variant of std::unique_ptr). If
///         @ref comms::option::app::InPlaceAllocation option is used, then the
///         smart pointer definition contains custom deleter, which will
///         explicitly invoke destructor of the message when the smart pointer
///         is out of scope. It means that it is @b NOT possible to create new
///         message with this factory if previously allocated one wasn't
///         destructed yet. If @ref comms::option::app::InPlaceAllocation option
///         is NOT used, than the requested message objects are allocated using
///         dynamic memory and returned wrapped in std::unique_ptr without
///         custom deleter.
///     @li @ref comms::option::app::SupportGenericMessage - Option used to
///     allow
...
///         @ref comms::option::app::ForceDispatchLinearSwitch - Force a
///         particular dispatch way when creating message object given the
///         numeric ID (see @ref comms::MsgFactory::createMsg()). The dispatch
///         methods are properly described in @ref page_dispatch tutorial page.
///         If none of these options are provided, then the MsgFactory
///         used a default way, which is equivalent to calling
///         @ref comms::dispatchMsgType() (see also @ref
///         page_dispatch_message_type_default). To inquire what actual dispatch
///         type is used, please use one of the following constexpr member
///         functions:
...
/// @pre If @ref comms::option::app::InPlaceAllocation option is provided, only
/// one custom
///     message can be allocated. The next one can be allocated only after
///     previous message has been destructed.
...
class MsgFactory
  ...
  static_assert(
      TMsgBase::hasMsgIdType(),
      "Usage of MsgFactory requires Message interface to provide ID type. "
      "Use comms::option::def::MsgIdType option in message interface type "
      "definition.");
...
public:
  ...
  /// @details If supported a variant of @ref comms::GenericMessage, @b void
  /// otherwise
  ...
  /// @param idx Relative index (or offset) of the message with the same ID. In
  /// case
  ...
  ///     first template parameter to this class). If @ref
  ///     comms::option::app::InPlaceAllocation option was used and previously
  ///     allocated message wasn't de-allocated yet, the empty (null) pointer
  ///     will be returned.
  MsgPtr createMsg(MsgIdParamType id, unsigned idx = 0U,
                   CreateFailureReason *reason = nullptr) const {
    return Base::createMsg(id, idx, reason);
  }

  /// @brief Allocate and initialise @ref comms::GenericMessage object.
  /// @details If @ref comms::option::app::SupportGenericMessage option hasn't
  /// been
  ///     provided, this function will return empty @b MsgPtr pointer. Otherwise
  ///     the relevant allocator will be used to allocate @ref
  ...
  bool canAllocate() const { return Base::canAllocate(); }

  /// @brief Get number of message types from @ref AllMessages, that have the
  /// specified ID.
  /// @param id ID of the message.
  /// @return Number of message classes that report same ID.
  std::size_t msgCount(MsgIdParamType id) const { return Base::msgCount(id); }

  /// @brief Compile time inquiry whether all the message classes in the
  ///     @b TAllMessages bundle have unique IDs.
  static constexpr bool hasUniqueIds() { return Base::hasUniqueIds(); }
  ...
  /// @return @b true in case of in-place allocation, @b false in case of
  /// dynamic memory use.
  static constexpr bool hasInPlaceAllocation() {
    return ParsedOptions::HasInPlaceAllocation;
  }

  /// @brief Compile time inquiry whether factory supports @ref
  /// comms::GenericMessage allocation

...
template <typename...> class DispatchMsgIdRetrieveHelper {
public:
  ...
  static typename TMessage::MsgIdParamType doGetId() {
    ...
    static_assert(!std::is_same<Tag<TMessage>, NoIdTag<>>::value,
...
private:
  ...
  template <typename... TParams> using NoIdTag = comms::details::tag::Tag3<>;

  template <typename TMessage, typename...>
  using DynamicCheckTag = typename comms::util::LazyShallowConditional<
      comms::isMessage<TMessage>() &&
      TMessage::hasGetId()>::template Type<HasDynamicIdTag, NoIdTag>;

  template <typename TMessage>
  using Tag = typename comms::util::
      LazyShallowConditional<messageHasStaticNumId<TMessage>()>::template Type<
          HasStaticIdTag, DynamicCheckTag, TMessage>;

  template <typename TMessage, typename... TParams>
  static typename TMessage::MsgIdParamType
  doGetIdInternal(HasStaticIdTag<TParams...>) {
  ...
  template <typename TMessage, typename... TParams>
  static typename TMessage::MsgIdParamType
  doGetIdInternal(HasDynamicIdTag<TParams...>) {

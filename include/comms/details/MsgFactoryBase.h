...
template <typename TAllMessages> constexpr bool msgFactoryAllHaveStaticNumId() {
  return allMessagesHaveStaticNumId<TAllMessages>();
}

template <typename TMessage> constexpr bool msgFactoryMessageHasStaticNumId() {
...
class MsgFactoryBase {
  ...
  template <typename...> struct InPlaceAllocDeepCondWrap {
    template <typename TInterface, typename TAllocMessages,
              typename TOrigMessages, typename TId, typename TDefaultType,
              typename...>
    using Type = typename comms::util::
        LazyDeepConditional<InterfaceHasVirtualDestructor>::template Type<
            comms::util::alloc::details::InPlaceSingleDeepCondWrap,
            comms::util::alloc::details::
                InPlaceSingleNoVirtualDestructorDeepCondWrap,
            TInterface, TAllocMessages, TOrigMessages, TId, TDefaultType>;
  };

  template <typename...> struct DynMemoryAllocDeepCondWrap {
    template <typename TInterface, typename TAllocMessages,
              typename TOrigMessages, typename TId, typename TDefaultType,
              typename...>
    using Type = typename comms::util::
        LazyDeepConditional<InterfaceHasVirtualDestructor>::template Type<
            ...
            comms::util::alloc::details::
                DynMemoryNoVirtualDestructorDeepCondWrap,
            TInterface, TOrigMessages, TId, TDefaultType>;
  };

  using Alloc = typename comms::util::LazyDeepConditional<
      ParsedOptionsInternal::HasInPlaceAllocation>::
      template Type<InPlaceAllocDeepCondWrap, DynMemoryAllocDeepCondWrap,
                    TMsgBase, AllMessagesInternal, TAllMessages,
                    typename TMsgBase::MsgIdType, GenericMessageInternal>;
...
public:
  ...
  MsgPtr createMsg(MsgIdParamType id, unsigned idx,
                   CreateFailureReason *reason) const {
  ...
  MsgPtr createGenericMsg(MsgIdParamType id, unsigned idx) const {
    ...
    using Tag = typename comms::util::LazyShallowConditional<
        ParsedOptions::HasSupportGenericMessage>::template Type<AllocGenericTag,
                                                                NoAllocTag>;

    return createGenericMsgInternal(id, idx, Tag(), DestructorTag<>());
  }

  bool canAllocate() const { return m_alloc.canAllocate(); }
...
protected:
  ...
  MsgPtr allocMsg(TArgs &&...args) const {
    ...
    static_assert(std::has_virtual_destructor<TObj>::value,
                  "This function is expected to be called for message objects "
                  "with virtual destructor");
    static_assert((!ParsedOptionsInternal::HasInPlaceAllocation) ||
                      comms::util::IsInTuple<
                          AllMessagesInternal>::template Type<TObj>::value,
...
private:
  ...
  template <typename... TParams> using NoAllocTag = comms::details::tag::Tag2<>;

  template <typename... TParams> using ForcedTag = comms::details::tag::Tag3<>;
  ...
  template <typename...>
  using DispatchTag = typename comms::util::LazyShallowConditional<
      ParsedOptions::HasForcedDispatch>::template Type<ForcedTag, StandardTag>;

  template <typename...>
  using DestructorTag = typename comms::util::LazyShallowConditional<
      InterfaceHasVirtualDestructor>::template Type<VirtualDestructorTag,
  ...
  public:
    ...
    MsgPtr getMsg() { return std::move(m_msg); }

    template <typename T> void handle() { m_msg = m_a.template alloc<T>(); }
  ...
  public:
    explicit NonVirtualDestructorCreateHandler(MsgIdParamType id, unsigned idx,
                                               Alloc &a)
        : m_id(id), m_idx(idx), m_a(a) {}

    MsgPtr getMsg() { return std::move(m_msg); }

    template <typename T> void handle() {
  ...
  template <typename... TParams>
  MsgPtr createGenericMsgInternal(MsgIdParamType id, unsigned idx,
                                  AllocGenericTag<TParams...>,
                                  VirtualDestructorTag<TParams...>) const {
    static_cast<void>(idx);
    static_assert(
        std::is_base_of<Message, typename ParsedOptions::GenericMessage>::value,
        "The requested GenericMessage class must have the same interface class "
        "as all other messages");
  ...
  MsgPtr createGenericMsgInternal(MsgIdParamType id, unsigned idx,
                                  AllocGenericTag<TParams...>,
                                  NonVirtualDestructorTag<TParams...>) const {
    static_assert(
        std::is_base_of<Message, typename ParsedOptions::GenericMessage>::value,
        "The requested GenericMessage class must have the same interface class "
        "as all other messages");
  ...
  static MsgPtr createGenericMsgInternal(MsgIdParamType, NoAllocTag<TParams...>,
                                         TDestructorTag) {
    return MsgPtr();
  }

  template <typename THandler, typename... TParams>
  static bool dispatchMsgTypeInternal(MsgIdParamType id, unsigned idx,
                                      THandler &handler,
                                      StandardTag<TParams...>) {
  ...
  template <typename THandler>
  static bool
  dispatchMsgTypeInternal(MsgIdParamType id, unsigned idx, THandler &handler,
  ...
  static constexpr bool isDispatchPolymorphicInternal(ForcedTag<TParams...>) {
    return std::is_same<comms::traits::dispatch::Polymorphic,
                        typename ParsedOptions::ForcedDispatch>::value;
  }
  ...
  isDispatchStaticBinSearchInternal(ForcedTag<TParams...>) {
    return std::is_same<comms::traits::dispatch::StaticBinSearch,
                        typename ParsedOptions::ForcedDispatch>::value;
  }

  template <typename... TParams>
  static constexpr bool
  isDispatchStaticBinSearchInternal(StandardTag<TParams...>) {
  ...
  static constexpr bool isDispatchLinearSwitchInternal(ForcedTag<TParams...>) {
    return std::is_same<comms::traits::dispatch::LinearSwitch,
                        typename ParsedOptions::ForcedDispatch>::value;
  }

  template <typename... TParams>
  static constexpr bool
  isDispatchLinearSwitchInternal(StandardTag<TParams...>) {
    return false;
  }

  template <typename... TParams>
  MsgPtr createMsgInternal(MsgIdParamType id, unsigned idx, bool &success,
                           VirtualDestructorTag<TParams...>) const {

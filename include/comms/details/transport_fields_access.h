...
#define COMMS_ACCESS_TRANSPORT_FIELD_FUNC(T_, t_, n_)                          \
  typename std::tuple_element<COMMS_CONCATENATE(TransportFieldIdx_, n_),       \
                              typename Base::T_>::type &                       \
  COMMS_CONCATENATE(transportField_, n_)()
#define COMMS_ACCESS_TRANSPORT_FIELD_CONST_FUNC(T_, t_, n_)                    \
  const typename std::tuple_element<COMMS_CONCATENATE(TransportFieldIdx_, n_), \
                                    typename Base::T_>::type &                 \
  COMMS_CONCATENATE(transportField_, n_)() const
...
#define COMMS_ACCESS_TRANSPORT_FIELD_FUNC(T_, t_, n_)                          \
  FUNC_AUTO_REF_RETURN(                                                        \
      COMMS_CONCATENATE(transportField_, n_),                                  \
      decltype(std::get<COMMS_CONCATENATE(TransportFieldIdx_, n_)>(t_)))
#define COMMS_ACCESS_TRANSPORT_FIELD_CONST_FUNC(T_, t_, n_)                    \
  FUNC_AUTO_REF_RETURN_CONST(                                                  \
      COMMS_CONCATENATE(transportField_, n_),                                  \
      decltype(std::get<COMMS_CONCATENATE(TransportFieldIdx_, n_)>(t_)))
#define COMMS_MSG_TRANSPORT_FIELDS_ACCESS_FUNC                                 \
  FUNC_AUTO_REF_RETURN(transportFields,                                        \
                       decltype(comms::toMessage(*this).transportFields()))
#define COMMS_MSG_TRANSPORT_FIELDS_ACCESS_CONST_FUNC                           \
  FUNC_AUTO_REF_RETURN_CONST(                                                  \
      transportFields, decltype(comms::toMessage(*this).transportFields()))
...
#define COMMS_TRANSPORT_FIELD_ACC_FUNC_1(T_, t_, n_)                           \
...
#define COMMS_CHOOSE_TRANSPORT_FIELD_ACC_FUNC_(N, T_, t_, ...)                 \
  COMMS_EXPAND(COMMS_TRANSPORT_FIELD_ACC_FUNC_##N(T_, t_, __VA_ARGS__))
#define COMMS_CHOOSE_TRANSPORT_FIELD_ACC_FUNC(N, T_, t_, ...)                  \
  COMMS_EXPAND(COMMS_CHOOSE_TRANSPORT_FIELD_ACC_FUNC_(N, T_, t_, __VA_ARGS__))
#define COMMS_DO_TRANSPORT_FIELD_ACC_FUNC(T_, t_, ...)                         \
  COMMS_EXPAND(COMMS_CHOOSE_TRANSPORT_FIELD_ACC_FUNC(                          \
      COMMS_NUM_ARGS(__VA_ARGS__), T_, t_, __VA_ARGS__))

#define COMMS_DEFINE_TRANSPORT_FIELD_ENUM(...)                                 \
  COMMS_EXPAND(COMMS_DEFINE_ENUM(TransportFieldIdx, __VA_ARGS__));

...
template <typename TEndian> class MessageInterfaceEndianBase {
public:
...
public:
  ...
  using MsgIdParamType =
      typename comms::util::Conditional<std::is_integral<MsgIdType>::value ||
                                        std::is_enum<MsgIdType>::value>::
          template Type<MsgIdType, const MsgIdType &>;
...
public:
  ...
  static_assert(
      TIdx < std::tuple_size<TransportFields>::value,
      "Index provided to comms::option::def::VersionInExtraTransportFields "
      "exceeds size of the tuple");
...
template <typename TBase> class MessageInterfaceIdInfoBase : public TBase {
public:
  using MsgIdParamType = typename TBase::MsgIdParamType;

  MsgIdParamType getId() const { return getIdImpl(); }
...
template <typename TBase> class MessageInterfaceValidBase : public TBase {
public:
  bool valid() const { return validImpl(); }

protected:
  ~MessageInterfaceValidBase() noexcept = default;
  virtual bool validImpl() const { return true; }
};

// ------------------------------------------------------

template <typename TBase> class MessageInterfaceLengthBase : public TBase {
public:
  std::size_t length() const { return lengthImpl(); }
...
template <class T, class R = void> struct MessageInterfaceIfHasRetType {
  using Type = R;
};
...
template <class T>
struct MessageInterfaceDispatchRetTypeHelper<
    T, typename MessageInterfaceIfHasRetType<typename T::RetType>::Type> {
  using Type = typename T::RetType;
};

template <class T>
using MessageInterfaceDispatchRetType =
...
public:
  ...
  DispatchRetType dispatch(Handler &handler) { return dispatchImpl(handler); }
...
protected:
  ...
  virtual DispatchRetType dispatchImpl(Handler &handler) {
    ...
    using Tag = typename comms::util::Conditional<std::is_void<
        DispatchRetType>::value>::template Type<VoidHandleRetTypeTag,
...
private:
  ...
  static DispatchRetType dispatchInternal(VoidHandleRetTypeTag) { return; }
...
template <typename TBase> class MessageInterfaceRefreshBase : public TBase {
public:
  bool refresh() { return refreshImpl(); }

protected:
  ~MessageInterfaceRefreshBase() noexcept = default;
  virtual bool refreshImpl() { return false; }
};

// ------------------------------------------------------

template <typename TBase> class MessageInterfaceNameBase : public TBase {
public:

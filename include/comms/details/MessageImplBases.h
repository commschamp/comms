...
template <typename TAllFields> class MessageImplFieldsContainer;
...
public:
  ...
  static constexpr bool areFieldsVersionDependent() {
    return comms::field::basic::CommonFuncs::IsAnyFieldVersionDependentBoolType<
        TAllFields...>::value;
  }

  static constexpr bool doFieldsHaveNonDefaultRefresh() {
    return comms::field::basic::CommonFuncs::
        AnyFieldHasNonDefaultRefreshBoolType<TAllFields...>::value;
  }

  template <typename TIter>
  comms::ErrorStatus doRead(TIter &iter, std::size_t size) {
    using Tag = typename comms::util::LazyShallowConditional<
        comms::field::basic::CommonFuncs::AllFieldsHaveReadNoStatusBoolType<
            TAllFields...>::value>::template Type<NoStatusTag, UseStatusTag>;
  ...
  comms::ErrorStatus doWrite(TIter &iter, std::size_t size) const {
    ...
    using Tag = typename comms::util::LazyShallowConditional<
        comms::field::basic::CommonFuncs::AllFieldsHaveWriteNoStatusBoolType<
            TAllFields...>::value>::template Type<NoStatusTag, UseStatusTag>;
  ...
  bool doValid() const {
    return util::tupleAccumulate(
        fields(), true, comms::field::details::FieldValidCheckHelper<>());
  }

  std::size_t doLength() const {
    return util::tupleAccumulate(
        fields(), static_cast<std::size_t>(0U),
        comms::field::details::FieldLengthSumCalcHelper<>());
  }

  template <std::size_t TFromIdx> std::size_t doLengthFrom() const {
    return util::tupleAccumulateFromUntil<TFromIdx,
  ...
  template <std::size_t TUntilIdx> std::size_t doLengthUntil() const {
    return util::tupleAccumulateFromUntil<0, TUntilIdx>(
        fields(), static_cast<std::size_t>(0U),
        comms::field::details::FieldLengthSumCalcHelper<>());
  }
  ...
  std::size_t doLengthFromUntil() const {
    return util::tupleAccumulateFromUntil<TFromIdx, TUntilIdx>(
        fields(), static_cast<std::size_t>(0U),
        comms::field::details::FieldLengthSumCalcHelper<>());
  }

  static constexpr std::size_t doMinLength() {
    return comms::util::tupleTypeAccumulate<AllFields>(
  ...
  static constexpr std::size_t doMinLengthFrom() {
    return comms::util::tupleTypeAccumulateFromUntil<
        TFromIdx, std::tuple_size<AllFields>::value, AllFields>(
        std::size_t(0), comms::field::details::FieldMinLengthSumCalcHelper<>());
  }

  template <std::size_t TUntilIdx>
  static constexpr std::size_t doMinLengthUntil() {
    return comms::util::tupleTypeAccumulateFromUntil<0, TUntilIdx, AllFields>(
        std::size_t(0), comms::field::details::FieldMinLengthSumCalcHelper<>());
  }

  template <std::size_t TFromIdx, std::size_t TUntilIdx>
  static constexpr std::size_t doMinLengthFromUntil() {
    return comms::util::tupleTypeAccumulateFromUntil<TFromIdx, TUntilIdx,
                                                     AllFields>(
        std::size_t(0), comms::field::details::FieldMinLengthSumCalcHelper<>());
  }
  ...
  bool doRefresh() {
    return util::tupleAccumulate(fields(), false,
                                 comms::field::details::FieldRefreshHelper<>());
  }
...
protected:
  ...
  comms::ErrorStatus doReadUntil(TIter &iter, std::size_t len) {
    return doReadUntilAndUpdateLen<TIdx>(iter, len);
  }

  template <std::size_t TIdx, typename TIter>
  comms::ErrorStatus doReadUntilAndUpdateLen(TIter &iter, std::size_t &len) {
  ...
  template <std::size_t TIdx, typename TIter>
  comms::ErrorStatus doReadFrom(TIter &iter, std::size_t len) {
    return doReadFromAndUpdateLen<TIdx>(iter, len);
  }

  template <std::size_t TIdx, typename TIter>
  comms::ErrorStatus doReadFromAndUpdateLen(TIter &iter, std::size_t &len) {
    auto status = comms::ErrorStatus::Success;
  ...
  comms::ErrorStatus doReadFromUntil(TIter &iter, std::size_t len) {
    return doReadFromUntilAndUpdateLen<TFromIdx, TUntilIdx>(iter, len);
  }

  template <std::size_t TFromIdx, std::size_t TUntilIdx, typename TIter>
  comms::ErrorStatus doReadFromUntilAndUpdateLen(TIter &iter,
                                                 std::size_t &len) {
    auto status = comms::ErrorStatus::Success;
    util::tupleForEachFromUntil<TFromIdx, TUntilIdx>(
        fields(), makeFieldReader(iter, status, len));
    return status;
  }
  ...
  void doReadNoStatusFromUntil(TIter &iter) {
    util::tupleForEachFromUntil<TFromIdx, TUntilIdx>(
        fields(), makeFieldNoStatusReader(iter));
  }

  template <std::size_t TIdx, typename TIter>
  comms::ErrorStatus doWriteUntil(TIter &iter, std::size_t len) const {
    return doWriteUntilAndUpdateLen<TIdx>(iter, len);
  }

  template <std::size_t TIdx, typename TIter>
  comms::ErrorStatus doWriteUntilAndUpdateLen(TIter &iter,
  ...
  template <std::size_t TIdx, typename TIter>
  comms::ErrorStatus doWriteFrom(TIter &iter, std::size_t len) const {
    return doWriteFromAndUpdateLen<TIdx>(iter, len);
  }

  template <std::size_t TIdx, typename TIter>
  comms::ErrorStatus doWriteFromAndUpdateLen(TIter &iter,
                                             std::size_t &len) const {
    auto status = comms::ErrorStatus::Success;
  ...
  template <std::size_t TFromIdx, std::size_t TUntilIdx, typename TIter>
  comms::ErrorStatus doWriteFromUntil(TIter &iter, std::size_t len) const {
    return doWriteFromUntilAndUpdateLen<TFromIdx, TUntilIdx>(iter, len);
  }

  template <std::size_t TFromIdx, std::size_t TUntilIdx, typename TIter>
  comms::ErrorStatus doWriteFromUntilAndUpdateLen(TIter &iter,
                                                  std::size_t &len) const {
    auto status = comms::ErrorStatus::Success;
    util::tupleForEachFromUntil<TFromIdx, TUntilIdx>(
        fields(), makeFieldWriter(iter, status, len));
    return status;
  }
  ...
  void doWriteNoStatusFromUntil(TIter &iter) const {
    util::tupleForEachFromUntil<TFromIdx, TUntilIdx>(
        fields(), makeFieldNoStatusWriter(iter));
  }
...
private:
  ...
  comms::ErrorStatus doReadInternal(TIter &iter, std::size_t size,
  ...
  template <typename TIter, typename... TParams>
  comms::ErrorStatus doWriteInternal(TIter &iter, std::size_t size,
  ...
  template <typename TIter>
  static comms::field::details::FieldReadHelper<TIter>
  makeFieldReader(TIter &iter, comms::ErrorStatus &status, std::size_t &size) {
    return comms::field::details::FieldReadHelper<TIter>(status, iter, size);
  }

  template <typename TIter>
  static comms::field::details::FieldReadNoStatusHelper<TIter>
  makeFieldNoStatusReader(TIter &iter) {
    return comms::field::details::FieldReadNoStatusHelper<TIter>(iter);
  }

  template <typename TIter>
  static comms::field::details::FieldWriteHelper<TIter>
  makeFieldWriter(TIter &iter, comms::ErrorStatus &status, std::size_t &size) {
    return comms::field::details::FieldWriteHelper<TIter>(status, iter, size);
  }

  template <typename TIter>
  static comms::field::details::FieldWriteNoStatusHelper<TIter>
  makeFieldNoStatusWriter(TIter &iter) {
...
template <typename TBase> class MessageImplVersionBase : public TBase {
public:
  using VersionType = typename TBase::VersionType;

  bool doFieldsVersionUpdate() {
    return comms::field::basic::CommonFuncs::setVersionForMembers(
        TBase::fields(), TBase::version());
  }
...
protected:
  MessageImplVersionBase() { doFieldsVersionUpdate(); }
...
protected:
  ...
  virtual comms::ErrorStatus readImpl(typename BaseImpl::ReadIterator &iter,
                                      std::size_t size) override {
    using Tag = typename comms::util::LazyShallowConditional<
        std::is_same<TActual, void>::value>::template Type<NoActual, HasActual>;
    return readImplInternal(iter, size, Tag());
  }

private:
  template <typename... TParams> using HasActual = comms::details::tag::Tag1<>;

  template <typename... TParams> using NoActual = comms::details::tag::Tag2<>;

  template <typename... TParams>
  comms::ErrorStatus readImplInternal(typename BaseImpl::ReadIterator &iter,
                                      std::size_t size, NoActual<TParams...>) {
    return BaseImpl::doRead(iter, size);
  }

  template <typename... TParams>
  comms::ErrorStatus readImplInternal(typename BaseImpl::ReadIterator &iter,
                                      std::size_t size, HasActual<TParams...>) {
...
protected:
  ...
  virtual comms::ErrorStatus writeImpl(typename BaseImpl::WriteIterator &iter,
...
private:
  ...
  template <typename... TParams>
  comms::ErrorStatus writeImplInternal(typename BaseImpl::WriteIterator &iter,
                                       std::size_t size,
...
template <typename TBase, typename TActual,
          typename TFailOnInvalidStatusWrapper>
...
protected:
  ...
  virtual bool refreshImpl() override {
    using Tag = typename comms::util::LazyShallowConditional<std::is_same<
        TActual, void>::value>::template Type<NoDowncast, Downcast>;
    return refreshInternal(Tag());
  }

private:
  template <typename... TParams> using Downcast = comms::details::tag::Tag1<>;

  template <typename... TParams> using NoDowncast = comms::details::tag::Tag2<>;

  template <typename... TParams> bool refreshInternal(Downcast<TParams...>) {
    return static_cast<TActual *>(this)->doRefresh();
  }

  template <typename... TParams> bool refreshInternal(NoDowncast<TParams...>) {
...
protected:
  ...
  dispatchImpl(typename TBase::Handler &handler) override {
...
public:
  ...
  static constexpr typename TBase::MsgIdParamType doGetId() { return MsgId; }
...
protected:
  ...
  virtual typename TBase::MsgIdParamType getIdImpl() const override {
    using Tag = typename comms::util::LazyShallowConditional<std::is_same<
        TActual, void>::value>::template Type<NoDowncastTag, DowncastTag>;
...
private:
  ...
  getIdInternal(NoDowncastTag<TParams...>) const {
...
template <typename TBase> class MessageImplNoIdBase : public TBase {
protected:
  ~MessageImplNoIdBase() noexcept = default;
  virtual typename TBase::MsgIdParamType getIdImpl() const override {
    static const typename TBase::MsgIdType MsgId = typename TBase::MsgIdType();
    static constexpr bool The_message_id_is_not_supposed_to_be_retrieved =
        false;

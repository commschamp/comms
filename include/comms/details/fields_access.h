...
#define COMMS_FIELD_VALUE_ACCESS_CONST_FUNC                                    \
  const typename Base::ValueType &value() const
#define COMMS_ACCESS_MEMBER_FIELD_FUNC(T_, t_, n_)                             \
  typename std::tuple_element<COMMS_CONCATENATE(FieldIdx_, n_),                \
                              typename Base::T_>::type &                       \
  COMMS_CONCATENATE(field_, n_)()
#define COMMS_ACCESS_MEMBER_FIELD_CONST_FUNC(T_, t_, n_)                       \
  const typename std::tuple_element<COMMS_CONCATENATE(FieldIdx_, n_),          \
                                    typename Base::T_>::type &                 \
  COMMS_CONCATENATE(field_, n_)() const
#define COMMS_MSG_FIELDS_ACCESS_FUNC typename Base::AllFields &fields()
#define COMMS_MSG_FIELDS_ACCESS_CONST_FUNC                                     \
  const typename Base::AllFields &fields() const

#else // #ifdef COMMS_MUST_DEFINE_BASE
#define COMMS_FIELD_VALUE_ACCESS_FUNC                                          \
  FUNC_AUTO_REF_RETURN(value,                                                  \
                       decltype(comms::field::toFieldBase(*this).value()))
#define COMMS_FIELD_VALUE_ACCESS_CONST_FUNC                                    \
  FUNC_AUTO_REF_RETURN_CONST(                                                  \
      value, decltype(comms::field::toFieldBase(*this).value()))
#define COMMS_ACCESS_MEMBER_FIELD_FUNC(T_, t_, n_)                             \
  FUNC_AUTO_REF_RETURN(                                                        \
      COMMS_CONCATENATE(field_, n_),                                           \
      decltype(std::get<COMMS_CONCATENATE(FieldIdx_, n_)>(t_)))
#define COMMS_ACCESS_MEMBER_FIELD_CONST_FUNC(T_, t_, n_)                       \
  FUNC_AUTO_REF_RETURN_CONST(                                                  \
      COMMS_CONCATENATE(field_, n_),                                           \
      decltype(std::get<COMMS_CONCATENATE(FieldIdx_, n_)>(t_)))
...
#define COMMS_MSG_FIELDS_ACCESS_CONST_FUNC                                     \
  FUNC_AUTO_REF_RETURN_CONST(fields,                                           \
                             decltype(comms::toMessageBase(*this).fields()))
...
#define COMMS_CHOOSE_FIELD_ACC_FUNC_(N, T_, t_, ...)                           \
  COMMS_EXPAND(COMMS_FIELD_ACC_FUNC_##N(T_, t_, __VA_ARGS__))
#define COMMS_CHOOSE_FIELD_ACC_FUNC(N, T_, t_, ...)                            \
  COMMS_EXPAND(COMMS_CHOOSE_FIELD_ACC_FUNC_(N, T_, t_, __VA_ARGS__))
#define COMMS_DO_FIELD_ACC_FUNC(T_, t_, ...)                                   \
  COMMS_EXPAND(COMMS_CHOOSE_FIELD_ACC_FUNC(COMMS_NUM_ARGS(__VA_ARGS__), T_,    \
                                           t_, __VA_ARGS__))

#define COMMS_DEFINE_FIELD_ENUM(...)                                           \
  COMMS_EXPAND(COMMS_DEFINE_ENUM(FieldIdx, __VA_ARGS__));
...
#define COMMS_FIELD_ACC_FUNC_NOTEMPLATE_1(n_)                                  \
...
#define COMMS_CHOOSE_FIELD_ACC_FUNC_NOTEMPLATE_(N, ...)                        \
  COMMS_EXPAND(COMMS_FIELD_ACC_FUNC_NOTEMPLATE_##N(__VA_ARGS__))
#define COMMS_CHOOSE_FIELD_ACC_FUNC_NOTEMPLATE(N, ...)                         \
  COMMS_EXPAND(COMMS_CHOOSE_FIELD_ACC_FUNC_NOTEMPLATE_(N, __VA_ARGS__))
#define COMMS_DO_FIELD_ACC_FUNC_NOTEMPLATE(...)                                \
  COMMS_EXPAND(COMMS_CHOOSE_FIELD_ACC_FUNC_NOTEMPLATE(                         \
      COMMS_NUM_ARGS(__VA_ARGS__), __VA_ARGS__))

// ----------------------------------------------

#define COMMS_FIELD_TYPEDEF(T_, P_, pref_, n_)                                 \
  using COMMS_CONCATENATE(P_, n_) =                                            \
      typename std::tuple_element<COMMS_CONCATENATE(pref_, n_), T_>::type;

#define COMMS_FIELD_TYPEDEF_1(T_, P_, pref_, n_)                               \
...
#define COMMS_CHOOSE_FIELD_TYPEDEF_(N, T_, P_, pref_, ...)                     \
  COMMS_EXPAND(COMMS_FIELD_TYPEDEF_##N(T_, P_, pref_, __VA_ARGS__))
#define COMMS_CHOOSE_FIELD_TYPEDEF(N, T_, P_, pref_, ...)                      \
  COMMS_EXPAND(COMMS_CHOOSE_FIELD_TYPEDEF_(N, T_, P_, pref_, __VA_ARGS__))
#define COMMS_DO_FIELD_TYPEDEF(T_, P_, pref_, ...)                             \
  COMMS_EXPAND(COMMS_CHOOSE_FIELD_TYPEDEF(COMMS_NUM_ARGS(__VA_ARGS__), T_, P_, \
                                          pref_, __VA_ARGS__))

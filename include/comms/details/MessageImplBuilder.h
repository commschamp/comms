...
template <typename TMessage, typename... TOptions> class MessageImplBuilder {
  using ParsedOptions = MessageImplOptionsParser<TOptions...>;

  static_assert(ParsedOptions::HasFieldsImpl,
                "Option comms::option::def::FieldsImpl must be used");

  using FieldsBase = typename ParsedOptions::template BuildFieldsImpl<TMessage>;
  ...
  using NameBase = typename ParsedOptions::template BuildNameImpl<
      PolymorphicStaticNumIdBase>;

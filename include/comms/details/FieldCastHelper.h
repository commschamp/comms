...
template <typename...> class FieldCastHelper {
public:
...
private:
  ...
  using Convertible = std::is_convertible<typename TFieldFrom::ValueType,
                                          typename TFieldTo::ValueType>;

  template <typename TField>
  using IsIntegral = std::integral_constant<
      bool, std::is_enum<typename TField::ValueType>::value ||
                std::is_integral<typename TField::ValueType>::value>;

  template <typename TFieldFrom, typename TFieldTo>
  using UseStaticCast =
      std::integral_constant<bool, Convertible<TFieldFrom, TFieldTo>::value ||
                                       (IsIntegral<TFieldFrom>::value &&
                                        IsIntegral<TFieldTo>::value)>;

  template <typename TFieldFrom, typename TFieldTo>
  using Tag = typename comms::util::LazyShallowConditional<UseStaticCast<
      TFieldFrom, TFieldTo>::value>::template Type<StaticCastTag, WriteReadTag>;

  template <typename TFieldTo, typename TFieldFrom, typename... TParams>
  static TFieldTo castInternal(const TFieldFrom &field,
                               StaticCastTag<TParams...>) {
    TFieldTo result;
  ...
  template <typename TFieldTo, typename TFieldFrom, typename... TParams>
  static TFieldTo castInternal(const TFieldFrom &field,
                               WriteReadTag<TParams...>) {
    static_assert(TFieldFrom::minLength() <= TFieldTo::maxLength(),

...
public:
  ...
  ArrayView(TIter iter, size_type len) noexcept
      : m_data(reinterpret_cast<const_pointer>(&(*iter))), m_len(len) {}

  /// @brief Construct out of array of elements with known size
  template <std::size_t TN>
  ArrayView(const T (&data)[TN]) noexcept : m_data(data), m_len(TN) {}

  /// @brief Construct out of array of elements with known size
  template <std::size_t TN>
  ArrayView(T (&data)[TN]) noexcept : m_data(data), m_len(TN) {}
  ...
  template <std::size_t TN> ArrayView &operator=(const T (&data)[TN]) {
    m_data = data;
    m_len = TN;
    return *this;
  }
  ...
  /// @brief Assign array of elements with known size
  template <std::size_t TN> ArrayView &operator=(T (&data)[TN]) {
    m_data = data;
    m_len = TN;
    return *this;
  }
  ...
  constexpr const_iterator end() const noexcept { return begin() + m_len; }
  ...
  /// @pre The view is not empty
  constexpr const_reference back() const { return m_data[m_len - 1U]; }
...
bool operator<(const ArrayView<T> &view1, const ArrayView<T> &view2) noexcept {
  return std::lexicographical_compare(view1.begin(), view1.end(), view2.begin(),
                                      view2.end());
}

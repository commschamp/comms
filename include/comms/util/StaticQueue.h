...
/// @brief This file contains the definition and implementation of the static
/// queue,
...
template <typename T> class StaticQueueBase {
public:
  template <typename TDerived, typename TQueueType> class IteratorBase;
...
protected:
  using ValueType = T;
  using StorageType =
      comms::util::AlignedStorage<sizeof(ValueType),
  ...
  using ConstReverseLinearisedIterator =
      std::reverse_iterator<ConstLinearisedIterator>;
  using LinearisedIteratorRange =
      std::pair<LinearisedIterator, LinearisedIterator>;
  using ConstLinearisedIteratorRange =
      std::pair<ConstLinearisedIterator, ConstLinearisedIterator>;

  StaticQueueBase(StorageTypePtr data, std::size_t capacity)
      : m_data(data), m_capacity(capacity), m_startIdx(0), m_count(0) {}

  StaticQueueBase(const StaticQueueBase &) = delete;
  StaticQueueBase(StaticQueueBase &&) = delete;

  ~StaticQueueBase() noexcept { clear(); }
  ...
  int indexOf(ConstReference element) const {
    ...
    if ((cellPtr < &m_data[0]) || (&m_data[capacity()] <= cellPtr)) {
  ...
  ConstLinearisedIterator lbegin() const { return clbegin(); }
  ...
  ConstReverseLinearisedIterator rlbegin() const { return crlbegin(); }
  ...
  ConstLinearisedIterator lend() const { return clend(); }
  ...
  ConstReverseLinearisedIterator rlend() const { return crlend(); }
  ...
  LinearisedIterator erase(LinearisedIterator pos) {
    ...
    auto isInRangeFunc = [](LinearisedIterator pos,
                            const LinearisedIteratorRange range) -> bool {
      return ((range.first <= pos) && (pos < range.second));
    };

    COMMS_ASSERT(isInRangeFunc(pos, rangeOne) || isInRangeFunc(pos, rangeTwo));
  ...
  Iterator erase(Iterator pos) {
    ...
    auto isInRangeFunc = [](Pointer elemPtr,
                            const LinearisedIteratorRange range) -> bool {
  ...
  ConstIterator cbegin() const {
    return ConstIterator(*this,
                         reinterpret_cast<ConstLinearisedIterator>(&m_data[0]));
  }

  Iterator end() {
    return Iterator(*this,
                    reinterpret_cast<LinearisedIterator>(&m_data[size()]));
  }
  ...
  ConstIterator cend() const {
    return ConstIterator(
        *this, reinterpret_cast<ConstLinearisedIterator>(&m_data[size()]));
  }

  template <typename TOther> void assignElements(TOther &&other) {
    static_assert(std::is_base_of<StaticQueueBase,
                                  typename std::decay<TOther>::type>::value,
                  "Assignment works only on the same types");

    using DecayedQueueType = typename std::decay<decltype(other)>::type;
    using NonRefQueueType =
        typename std::remove_reference<decltype(other)>::type;

    using QueueValueType = typename comms::util::
        Conditional<std::is_const<NonRefQueueType>::value>::template Type<
            const typename DecayedQueueType::ValueType,
            typename DecayedQueueType::ValueType>;

    using ElemRefType = typename comms::util::Conditional<
        std::is_rvalue_reference<decltype(other)>::value>::
        template Type<typename std::add_rvalue_reference<QueueValueType>::type,
  ...
  template <typename... TArgs> void emplaceBack(TArgs &&...args) {
    COMMS_ASSERT(!full());
    if (full()) {
      return;
    }
  ...
  bool operator==(const StaticQueueBase &other) const {
    ...
    auto otherRangeOneSize =
        std::distance(otherRangeOne.first, otherRangeOne.second);
    ...
    COMMS_ASSERT(std::distance(currIter, rangeTwo.second) ==
...
private:
  template <typename U> void createValueAtIndex(U &&value, std::size_t index) {
  ...
  template <typename U> void pushBackNotFull(U &&value) {
    COMMS_ASSERT(!full());
    createValueAtIndex(std::forward<U>(value), size());
    ++m_count;
  }

  template <typename... TArgs> void emplaceBackNotFull(TArgs &&...args) {
    COMMS_ASSERT(!full());
  ...
  template <typename U> void pushFrontNotFull(U &&value) {
    COMMS_ASSERT(!full());
  ...
  void lineariseByMove(const std::pair<TIter, TIter> firstRange,
    ...
    COMMS_ASSERT(movConstructFirstSize <=
                 (firstRange.second - firstRange.first));
    ...
    for (auto iter = std::max(newPlacePtr, firstRange.first);
         iter != firstRange.second; ++iter) {
      iter->~T();
    }
  ...
  void lineariseByMoveOneTwo(const LinearisedIteratorRange &rangeOne,
  ...
  void lineariseByMoveTwoOne(const LinearisedIteratorRange &rangeOne,
                             const LinearisedIteratorRange &rangeTwo) {
    ...
    m_startIdx =
        (capacity() - std::distance(rangeOne.first, rangeOne.second)) - size();
  }
  ...
  void moveRange(TIter rangeBeg, TIter rangeEnd, TIter target) {
    ...
    auto moveConstructSize = std::min(std::distance(rangeBeg, rangeEnd),
...
protected:
  ...
  using IteratorCategory =
      typename std::iterator_traits<ArrayIterator>::iterator_category;
  using iterator_category = IteratorCategory;
  using ValueType = typename std::iterator_traits<ArrayIterator>::value_type;
  using value_type = ValueType;
  using DifferenceType =
      typename std::iterator_traits<ArrayIterator>::difference_type;
  using difference_type = DifferenceType;
  using Pointer = typename std::iterator_traits<ArrayIterator>::pointer;
  using pointer = Pointer;
  using ConstPointer = typename std::add_pointer<typename std::add_const<
      typename std::remove_pointer<Pointer>::type>::type>::type;
  ...
  IteratorBase(QueueType &queue, ArrayIterator iterator)
      : m_queue(queue), m_iterator(iterator) {}
  ...
  QueueType &getQueue() { return m_queue; }

  typename std::add_const<QueueType &>::type getQueue() const {
    return m_queue;
  }

  ArrayIterator &getIterator() { return m_iterator; }
...
class StaticQueueBase<T>::ConstIterator
    : public StaticQueueBase<T>::template IteratorBase<
          typename StaticQueueBase<T>::ConstIterator,
          const StaticQueueBase<T>> {
  using Base = typename StaticQueueBase<T>::template IteratorBase<
      typename StaticQueueBase<T>::ConstIterator, const StaticQueueBase<T>>;
...
class StaticQueueBase<T>::Iterator
    : public StaticQueueBase<T>::template IteratorBase<
          typename StaticQueueBase<T>::Iterator, StaticQueueBase<T>> {
  using Base = typename StaticQueueBase<T>::template IteratorBase<
      typename StaticQueueBase<T>::Iterator, StaticQueueBase<T>>;
...
protected:
  ...
  Reference front() { return reinterpret_cast<Reference>(Base::front()); }

  ConstReference front() const {
    return reinterpret_cast<ConstReference>(Base::front());
  }

  Reference back() { return reinterpret_cast<Reference>(Base::back()); }
  ...
  ReverseLinearisedIterator invalidReverseIter() {
    return ReverseLinearisedIterator(reinterpret_cast<LinearisedIterator>(
  ...
  ReverseLinearisedIterator rlbegin() {
    return ReverseLinearisedIterator(
        reinterpret_cast<LinearisedIterator>(Base::rlbegin().base()));
  }
  ...
  ReverseLinearisedIterator rlend() {
    return ReverseLinearisedIterator(
        reinterpret_cast<LinearisedIterator>(Base::rlend().base()));
  }
...
protected:
  ...
  StaticQueueBaseOptimised &
  operator=(const StaticQueueBaseOptimised &other) = default;
  StaticQueueBaseOptimised &
  operator=(StaticQueueBaseOptimised &&other) = default;
};

template <>
class StaticQueueBaseOptimised<std::int8_t>
    : public CastWrapperQueueBase<std::int8_t, std::uint8_t> {
...
class StaticQueueBaseOptimised<T *>
    : public CastWrapperQueueBase<
          T *, typename comms::util::SizeToType<sizeof(T *)>::Type> {
  using Base =
      CastWrapperQueueBase<T *,
                           typename comms::util::SizeToType<sizeof(T *)>::Type>;
...
public:
  ...
  /// @brief Linearised iterator range type - std::pair of (first,
  /// one-past-last) iterators.
  ...
  StaticQueue() : Base(&m_array[0], TSize) {}
  ...
  StaticQueue(const StaticQueue &queue) : Base(&m_array[0], TSize) {
  ...
  ///       of the internal elements do not throw, Basic otherwise.
  StaticQueue(StaticQueue &&queue) : Base(&m_array[0], TSize) {
  ...
  ///       of the internal elements do not throw, Basic otherwise.
  template <std::size_t TAnySize>
  StaticQueue(StaticQueue<T, TAnySize> &&queue) : Base(&m_array[0], TSize) {
    Base::assignElements(std::move(queue));
  }
  ...
  /// @note Thread safety: Safe
  /// @note Exception guarantee: No throw.
  static constexpr std::size_t capacity() { return TSize; }
  ...
  /// @note Exception guarantee: No throw in case the destructor of the
  ///       popped element doesn't throw. Basic guarantee otherwise.
  void popBack(std::size_t count) { Base::popBack(count); }

  /// @brief Same as popBack(std::size_t)
  void pop_back(std::size_t count) { popBack(count); }
  ...
  /// @note Exception guarantee: No throw in case the destructor of the
  ///       popped element doesn't throw. Basic guarantee otherwise.
  void popFront(std::size_t count) { Base::popFront(count); }

  /// @brief Same as popFront(std::size_t)
  inline void pop_front(std::size_t count) { popFront(count); }
  ...
  template <typename U> inline void push_front(U &&value) {
  ...
  ConstReference front() const { return Base::front(); }
  ...
  ConstReference back() const { return Base::back(); }
  ...
  ///       a writer.
  /// @note Exception guarantee: No throw.
  Reference operator[](std::size_t index) { return Base::operator[](index); }
  ...
  Reference at(std::size_t index) { return Base::at(index); }

  /// @brief Const version of at().
  ConstReference at(std::size_t index) const { return Base::at(index); }
  ...
  ///       a writer.
  /// @note Exception guarantee: No throw.
  int indexOf(ConstReference element) const { return Base::indexOf(element); }
  ...
  /// @note Thread safety: Safe
  /// @note Exception guarantee: No throw
  LinearisedIterator invalidIter() { return Base::invalidIter(); }

  /// @brief Const version of invalidIter()
  ConstLinearisedIterator invalidIter() const { return Base::invalidIter(); }
  ...
  /// @see linearise()
  /// @see invalidIter()
  LinearisedIterator lbegin() { return Base::lbegin(); }

  /// @brief Same as clbegin().
  ConstLinearisedIterator lbegin() const { return Base::lbegin(); }

  /// @brief Const version of lbegin().
  ConstLinearisedIterator clbegin() const { return Base::clbegin(); }
  ...
  ReverseLinearisedIterator rlbegin() { return Base::rlbegin(); }

  /// @brief Same as crlbegin().
  ConstReverseLinearisedIterator rlbegin() const { return Base::rlbegin(); }

  /// @brief Const version of rlbegin().
  ConstReverseLinearisedIterator crlbegin() const { return Base::crlbegin(); }
  ...
  /// @see linearise()
  /// @see invalidIter()
  LinearisedIterator lend() { return Base::lend(); }

  /// @brief Same as clend().
  ConstLinearisedIterator lend() const { return Base::lend(); }

  /// @brief Const version of lend().
  ConstLinearisedIterator clend() const { return Base::clend(); }
  ...
  /// @see linearise()
  /// @see invalidReverseIter()
  ReverseLinearisedIterator rlend() { return Base::rlend(); }

  /// @brief Same as crlend().
  ConstReverseLinearisedIterator rlend() const { return Base::rlend(); }

  /// @brief Const version of rlend();
  ConstReverseLinearisedIterator crlend() const { return Base::crlend(); }
  ...
  ///       a writer.
  /// @note Exception guarantee: No throw.
  bool linearised() const { return Base::linearised(); }

  /// @brief Same as linearised();
  bool isLinearised() const { return linearised(); }
  ...
  /// @see isLinearised()
  /// @see arrayTwo()
  LinearisedIteratorRange arrayOne() { return Base::arrayOne(); }

  /// @brief Const version of former arrayOne().
  ConstLinearisedIteratorRange arrayOne() const { return Base::arrayOne(); }
  ...
  /// @see isLinearised()
  /// @see arrayOne()
  LinearisedIteratorRange arrayTwo() { return Base::arrayTwo(); }

  /// @brief Const version of former arrayTwo().
  ConstLinearisedIteratorRange arrayTwo() const { return Base::arrayTwo(); }
  ...
  ///       of the internal elements do not throw, Basic otherwise.
  void resize(std::size_t newSize) { Base::resize(newSize); }
  ...
  ///       of the internal elements do not throw, Basic otherwise.
  LinearisedIterator erase(LinearisedIterator pos) { return Base::erase(pos); }
...
template <typename T, std::size_t TSize>
class StaticQueue<T, TSize>::ConstIterator
...
public:
  ...
  /// @brief Const version of operator*
  ConstReference operator*() const { return Base::operator*(); }
...
///          about linearisation of the queue.
template <typename T, std::size_t TSize>
class StaticQueue<T, TSize>::Iterator
...
public:
  ...
  /// @brief Iterator comparison operator
  /// @param other Other iterator
  bool operator<(const Iterator &other) const { return Base::operator<(other); }

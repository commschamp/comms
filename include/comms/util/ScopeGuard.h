...
/// @file
/// @brief Contains definition of the "Scope Guard" idiom, see @ref
/// comms::util::ScopeGuard.
...
/// @brief Implements <a
/// href="https://en.wikibooks.org/wiki/More_C%2B%2B_Idioms/Scope_Guard">Scope
/// Guard Idiom</a>.
...
///          auto guard = comms::util::makeScopeGuard(&func, std::ref(arg1),
///          arg2);
///
...
template <typename TFunc> class ScopeGuard {
public:
  /// @brief Constructor
  ...
  explicit ScopeGuard(TFunc &&func)
      : m_func(std::forward<TFunc>(func)), m_engaged(true) {}
  ...
  ScopeGuard(ScopeGuard &&guard)
      : m_func(std::move(guard.m_func)), m_engaged(std::move(guard.m_engaged)) {
    guard.release();
  }
  ...
  bool isReleased() const { return !m_engaged; }
...
auto makeScopeGuard(TFunc &&func, TParams... args)
  ...
  auto bindObj =
      std::bind(std::forward<TFunc>(func), std::forward<TParams>(args)...);

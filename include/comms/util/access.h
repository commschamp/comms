...
// Bug in gcc-14, reporting "free-nonheap-object" error when working with
// vectors and using C++20/23, but only in Release mode
COMMS_GNU_WARNING_DISABLE("-Wfree-nonheap-object")
#endif // #if COMMS_IS_GCC_14 && defined(NDEBUG) && (COMMS_IS_CPP20 ||
       // COMMS_IS_CPP23)
...
template <typename T>
using AccessSelectIntType = typename comms::util::Conditional<
    std::is_signed<T>::value>::template Type<int, unsigned>;

template <typename T>
using AccessOptimisedValueType =
    typename comms::util::LazyShallowConditional<sizeof(T) >= sizeof(int)>::
        template Type<AccessSelectTypeItself, AccessSelectIntType, T>;
...
typename std::decay<T>::type signExtCommon(T value, std::size_t size) {
  ...
  ValueType mask = static_cast<ValueType>(static_cast<ValueType>(1)
                                          << ((size * BinDigits) - 1));
  if (value & mask) {
...
template <typename...> class SignExt {
  template <typename... TParams> using FullSize = comms::details::tag::Tag1<>;
...
public:
  ...
  static typename std::decay<T>::type value(T val) {
    ...
    using Tag = typename comms::util::LazyShallowConditional<
        sizeof(ValueType) == TSize>::template Type<FullSize, PartialSize>;
...
private:
  template <std::size_t TSize, typename TByteType, typename T,
            typename... TParams>
  static typename std::decay<T>::type valueInternal(T val,
                                                    FullSize<TParams...>) {
    return val;
  }
...
using AccessIteratorByteType =
    typename std::iterator_traits<typename std::decay<TIter>::type>::value_type;

template <typename TIter> struct AccessContainerByteTypeDetector {
...
template <typename TIter>
using AccessContainerByteType = typename AccessContainerByteTypeDetector<
    typename std::decay<TIter>::type>::Type;

template <typename TIter>
using AccessByteType = typename comms::util::LazyShallowConditional<
    std::is_void<AccessIteratorByteType<TIter>>::value>::
    template Type<AccessContainerByteType, AccessIteratorByteType, TIter>;
...
template <typename TEndian> struct WriteUnsignedFuncWrapper;

template <> struct WriteUnsignedFuncWrapper<traits::endian::Big> {
  template <typename T, typename TIter>
...
template <> struct WriteUnsignedFuncWrapper<traits::endian::Little> {
  template <typename T, typename TIter>
  static void write(T value, std::size_t size, TIter &iter) {
...
void writeRandomAccess(T value, std::size_t size, TIter &iter) {
  ...
  static_assert(!std::is_void<ByteType>::value, "Invalid byte type");
  using UnsignedByteType = typename std::make_unsigned<ByteType>::type;
  static_assert(!std::is_const<UnsignedByteType>::value,
...
template <typename...> class WriteHelper {
  template <typename... TParams>
  using RandomAccessTag = comms::details::tag::Tag1<>;

  template <typename... TParams> using RegularTag = comms::details::tag::Tag2<>;

  template <typename TIter>
  using RandomAccessOrPointerTag = typename comms::util::LazyShallowConditional<
      std::is_pointer<TIter>::value &&
      std::is_unsigned<AccessByteType<TIter>>::value>::
      template Type<RegularTag, RandomAccessTag>;

  template <typename TIter>
  using Tag = typename comms::util::LazyShallowConditional<
      std::is_same<typename std::iterator_traits<TIter>::iterator_category,
                   std::random_access_iterator_tag>::value>::
      template Type<RandomAccessOrPointerTag, RegularTag, TIter>;

  template <typename TEndian, typename T, typename TIter, typename... TParams>
  static void writeInternal(T value, std::size_t size, TIter &iter,
                            RandomAccessTag<TParams...>) {
...
public:
  ...
  static void write(T value, std::size_t size, TIter &iter) {
    using ValueType = typename std::decay<T>::type;
    using AccessOptimisedValueType =
        details::AccessOptimisedValueType<ValueType>;

    return writeInternal<TEndian>(static_cast<AccessOptimisedValueType>(value),
                                  size, iter, Tag<TIter>());
  }
};
...
template <typename TEndian> struct ReadUnsignedFuncWrapper;

template <> struct ReadUnsignedFuncWrapper<traits::endian::Big> {
  template <typename T, typename TIter>
...
template <> struct ReadUnsignedFuncWrapper<traits::endian::Little> {
  template <typename T, typename TIter>
  static T read(std::size_t size, TIter &iter) {
...
T read(std::size_t size, TIter &iter) {
  ...
  using UnsignedType = typename std::make_unsigned<ValueType>::type;
  auto value =
      ReadUnsignedFuncWrapper<TEndian>::template read<UnsignedType>(size, iter);
...
template <typename...> class ReadHelper {
  template <typename... TParams>
  using PointerToSignedTag = comms::details::tag::Tag1<>;

  template <typename... TParams> using OtherTag = comms::details::tag::Tag2<>;

  template <typename TIter>
  using PointerCheckTag = typename comms::util::LazyShallowConditional<
      std::is_const<AccessByteType<TIter>>::value &&
      std::is_unsigned<AccessByteType<TIter>>::value>::
      template Type<OtherTag, PointerToSignedTag>;

  template <typename TIter>
  using Tag = typename comms::util::LazyShallowConditional<std::is_pointer<
      TIter>::value>::template Type<PointerCheckTag, OtherTag, TIter>;

  template <typename TEndian, typename T, typename TIter, typename... TParams>
  static T readInternal(std::size_t size, TIter &iter,
                        PointerToSignedTag<TParams...>) {
...
public:
  ...
  static T read(std::size_t size, TIter &iter) {
    ...
    using AccessOptimisedValueType =
        details::AccessOptimisedValueType<ValueType>;
    return static_cast<ValueType>(
        readInternal<TEndian, AccessOptimisedValueType>(size, iter,
                                                        Tag<TIter>()));
  }
...
void writeBig(T value, TIter &iter) {
  details::WriteHelper<>::template write<traits::endian::Big>(value, TSize,
                                                              iter);
}
...
void writeBig(T value, std::size_t size, TIter &iter) {
  details::WriteHelper<>::template write<traits::endian::Big>(value, size,
                                                              iter);
}
...
/// @post The iterator is advanced.
template <typename T, typename TIter> void writeBig(T value, TIter &iter) {
  using ValueType = typename std::decay<T>::type;
...
T readBig(TIter &iter) {
  return details::ReadHelper<>::template read<traits::endian::Big, T, TSize>(
      iter);
}
...
///      and incremented at least sizeof(T) times.
/// @post The iterator is advanced.
template <typename T, typename TIter> T readBig(TIter &iter) {
  using ValueType = typename std::decay<T>::type;
...
///      and incremented at least sizeof(T) times.
/// @post The iterator is advanced.
template <typename T, typename TIter> T readBig(TIter &iter, std::size_t size) {
  return details::ReadHelper<>::template read<traits::endian::Big, T>(size,
                                                                      iter);
}
...
void writeLittle(T value, TIter &iter) {
  details::WriteHelper<>::template write<traits::endian::Little>(value, TSize,
                                                                 iter);
}
...
void writeLittle(T value, std::size_t size, TIter &iter) {
  details::WriteHelper<>::template write<traits::endian::Little>(value, size,
                                                                 iter);
}
...
///      and incremented at least sizeof(T) times.
/// @post The iterator is advanced.
template <typename T, typename TIter> void writeLittle(T value, TIter &iter) {
  using ValueType = typename std::decay<T>::type;
...
T readLittle(TIter &iter) {
  return details::ReadHelper<>::template read<traits::endian::Little, T, TSize>(
      iter);
}
...
///      and incremented at least sizeof(T) times.
/// @post The iterator is advanced.
template <typename T, typename TIter> T readLittle(TIter &iter) {
  using ValueType = typename std::decay<T>::type;
...
T readLittle(TIter &iter, std::size_t size) {
  return details::ReadHelper<>::template read<traits::endian::Little, T>(size,
                                                                         iter);
}
...
/// @brief Same as writeBig<T, TIter>()
template <typename T, typename TIter>
void writeData(T value, std::size_t size, TIter &iter,
...
/// @brief Same as readBig<T, TIter>()
template <typename T, typename TIter>
T readData(TIter &iter, std::size_t size,
           const traits::endian::Little &endian) {
  static_cast<void>(endian);

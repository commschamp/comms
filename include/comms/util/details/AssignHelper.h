...
private:
  ...
  template <typename... TParams> using StdSpanTag = comms::details::tag::Tag3<>;

  template <typename... TParams> using UnknownTag = comms::details::tag::Tag4<>;
  ...
  template <typename T>
  using ConstructorTag = typename comms::util::LazyShallowConditional<
      comms::util::detect::hasPtrSizeConstructor<T>()>::
      template Type<UsePtrSizeConstructorTag, UnknownTag>;

  template <typename T>
  using SpanConstructorTag = typename comms::util::LazyShallowConditional<
      comms::util::detect::details::IsStdSpan<T>::Value>::
      template Type<StdSpanTag, ConstructorTag, T>;

  template <typename T>
  using Tag = typename comms::util::LazyShallowConditional<
      comms::util::detect::hasAssignFunc<T>()>::
      template Type<UseAssignTag, SpanConstructorTag, T>;

  template <typename T>
  using ReserveTag = typename comms::util::LazyShallowConditional<
      comms::util::detect::hasReserveFunc<T>()>::template Type<HasReserveTag,
                                                               NoReserveTag>;

  template <typename T, typename TIter, typename... TParams>
  static void assignInternal(T &obj, TIter from, TIter to,
                             UseAssignTag<TParams...>) {
    using ObjType = typename std::decay<decltype(obj)>::type;
  ...
  template <typename T, typename TIter, typename... TParams>
  static void assignInternal(T &obj, TIter from, TIter to,
                             UsePtrSizeConstructorTag<TParams...>) {
    ...
    static_assert(
        ...
        "Only random access iterator is supported for provided type "
        "assignments");
  ...
  static void assignInternal(T &obj, TIter from, TIter to,
    ...
    auto fromPtr =
        const_cast<PointerType>(reinterpret_cast<ConstPointerType>(&(*from)));
    auto toPtr = fromPtr + std::distance(from, to);
    assignInternal(obj, fromPtr, toPtr, UsePtrSizeConstructorTag<TParams...>());
  }

  template <typename T, typename... TParams>
  static void reserveInternal(T &obj, std::size_t len,
                              HasReserveTag<TParams...>) {
    obj.reserve(len);
  }

  template <typename T, typename... TParams>
  static void reserveInternal(T &obj, std::size_t len,
                              NoReserveTag<TParams...>) {

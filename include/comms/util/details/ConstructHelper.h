...
template <typename T> class ConstructHelper {
public:
  using RetType = T;

  template <typename TIter> static RetType construct(TIter from, TIter to) {
...
public:
  ...
  template <typename TIter> static RetType construct(TIter from, TIter to) {
    return RetType(&(*from), static_cast<typename RetType::size_type>(

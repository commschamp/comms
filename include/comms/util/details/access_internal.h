...
#include "comms/util/SysEndian.h"
...
template <>
struct WriteUnsignedFuncWrapper<comms::traits::endian::Big>
{
    template <typename T, typename TIter>
...
template <>
struct WriteUnsignedFuncWrapper<comms::traits::endian::Little>
{
    template <typename T, typename TIter>
    static void write(T value, std::size_t size, TIter& iter)
    {
...
class WriteHelper
{
    ...
    template <typename... TParams>
    using UseMemcpyTag = comms::details::tag::Tag3<>;

    template <typename... TParams>
    using ByteByByteTag = comms::details::tag::Tag4<>;
    ...
    template <typename TEndian>
    using MemcpySelectTag =
        typename comms::util::LazyShallowConditional<
            std::is_same<TEndian, comms::util::SysEndian>::value
        >::template Type<
            UseMemcpyTag,
            ByteByByteTag
        >;
    ...
    template <typename TEndian, std::size_t TSize, typename T, typename TIter, typename... TParams>
    static void writeInternal(T value, TIter& iter, RandomAccessTag<TParams...>)
    {
        writeInternal<TEndian, T, TSize>(value, iter, MemcpySelectTag<TEndian>());
    }

    template <typename TEndian, std::size_t TSize, typename T, typename TIter, typename... TParams>
    static void writeInternal(T value, TIter& iter, RegularTag<TParams...>)
    {
        write(value, TSize, iter);
    }

    template <typename TEndian, std::size_t TSize, typename T, typename TIter, typename... TParams>
    static void writeInternal(T value, TIter& iter, UseMemcpyTag<TParams...>)
    {
        std::memcpy(&(*iter), &value, TSize);
        std::advance(iter, TSize);
    }

    template <typename TEndian, std::size_t TSize, typename T, typename TIter, typename... TParams>
    static void writeInternal(T value, TIter& iter, ByteByByteTag<TParams...>)
    {
        write(value, TSize, iter);
    }
...
public:
    ...
    static void write(T value, std::size_t size, TIter& iter)
    {
        ...
        return writeInternal<TEndian>(static_cast<AccessOptimisedValueType>(value), size, iter, WriteTag<TIter>());
    }

    template <typename TEndian, std::size_t TSize, typename T, typename TIter>
    static void write(T value, TIter& iter)
    {
        writeInternal<TEndian, TSize>(value, iter, WriteTag<TIter>());
    }
};
...
template <>
struct ReadUnsignedFuncWrapper<comms::traits::endian::Big>
{
    template <typename T, typename TIter>
...
template <>
struct ReadUnsignedFuncWrapper<comms::traits::endian::Little>
{
    template <typename T, typename TIter>
    static T read(std::size_t size, TIter& iter)
    {
...
class ReadHelper
{
    ...
    template <typename TEndian, typename T, std::size_t TSize, typename TIter, typename... TParams>
    static T readInternal(TIter& iter, UseMemcpyTag<TParams...>)
    {
        using ValueType = typename std::decay<T>::type;
        ...
        std::memcpy(&retval, &(*iter), TSize);
        std::advance(iter, TSize);
        return static_cast<T>(retval);
    }

    template <typename TEndian, typename T, std::size_t TSize, typename TIter, typename... TParams>
    static T readInternal(TIter& iter, ByteByByteTag<TParams...>)
    {
        using ValueType = typename std::decay<T>::type;
        static_assert(TSize <= sizeof(ValueType), "Precondition failure");
...
public:
    ...
    static T read(std::size_t size, TIter& iter)
    {
        ...
        return
            static_cast<ValueType>(
                readInternal<TEndian, AccessOptimisedValueType>(size, iter, ReadTag<TIter>()));
    }
    ...
    static T read(TIter& iter)
    {
        ...
        auto retval = readInternal<TEndian, UnsignedValueType, TSize>(iter, MemcpySelectTag<TEndian>());
        //auto retval = read<TEndian, ValueType>(TSize, iter);

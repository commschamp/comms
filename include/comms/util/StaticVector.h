...
template <typename T> class StaticVectorBase {
public:
  using value_type = T;
  ...
  using CellType =
      comms::util::AlignedStorage<sizeof(T), std::alignment_of<T>::value>;

  static_assert(sizeof(CellType) == sizeof(T), "Type T must be padded");

  StaticVectorBase(CellType *dataPtr, std::size_t cap)
      : m_data(dataPtr), m_capacity(cap) {}

  ~StaticVectorBase() noexcept { clear(); }
  ...
  template <typename TIter> void assign(TIter from, TIter to) {
    clear();
  ...
  T &operator[](std::size_t pos) { return elem(pos); }

  const T &operator[](std::size_t pos) const { return elem(pos); }
  ...
  template <typename U> T *insert(const T *pos, U &&value) {
  ...
  template <typename TIter> T *insert(const T *pos, TIter from, TIter to) {
    using Tag = typename std::iterator_traits<TIter>::iterator_category;
    return insert_internal(pos, from, to, Tag());
  }

  template <typename... TArgs> T *emplace(const T *iter, TArgs &&...args) {
  ...
  T *erase(const T *from, const T *to) {
    ...
    COMMS_ASSERT(static_cast<std::size_t>(std::distance(eraseFrom, eraseTo)) ==
                 eraseCount);
...
private:
  ...
  T &elem(std::size_t idx) { return reinterpret_cast<T &>(cell(idx)); }
  ...
  template <typename TIter>
  T *insert_internal(const T *pos, TIter from, TIter to,
...
template <typename T, std::size_t TSize> struct StaticVectorStorageBase {
  using ElementType =
      comms::util::AlignedStorage<sizeof(T), std::alignment_of<T>::value>;
...
class StaticVectorGeneric : public StaticVectorStorageBase<T, TSize>,
...
public:
  ...
  template <std::size_t TOtherSize>
  StaticVectorGeneric &
  operator=(const StaticVectorGeneric<T, TOtherSize> &other) {
...
public:
  ...
  explicit StaticVectorCasted(size_type count) : Base(count) {}

  template <typename TIter>
  StaticVectorCasted(TIter from, TIter to) : Base(from, to) {}

  template <std::size_t TOtherSize>
  StaticVectorCasted(const StaticVectorCasted<TOrig, TCast, TOtherSize> &other)
      : Base(other) {}

  StaticVectorCasted(const StaticVectorCasted &other) : Base(other) {}
  ...
  template <std::size_t TOtherSize>
  StaticVectorCasted &
  operator=(const StaticVectorCasted<TOrig, TCast, TOtherSize> &other) {
    Base::operator=(other);
    return *this;
  }
  ...
  reference front() { return *(reinterpret_cast<pointer>(&(Base::front()))); }

  const_reference front() const {
    return *(reinterpret_cast<const_pointer>(&(Base::front())));
  }

  reference back() { return *(reinterpret_cast<pointer>(&(Base::back()))); }

  const_reference back() const {
    return *(reinterpret_cast<const_pointer>(&(Base::back())));
  }

  pointer data() { return reinterpret_cast<pointer>(Base::data()); }
  ...
  iterator insert(const_iterator iter, size_type count, const_reference value) {
    return reinterpret_cast<iterator>(Base::insert(
        reinterpret_cast<typename Base::const_iterator>(iter), count,
        *(reinterpret_cast<typename Base::const_pointer>(&value))));
  }
  ...
  iterator insert(const_iterator iter, TIter from, TIter to) {
    return reinterpret_cast<iterator>(Base::insert(
        reinterpret_cast<typename Base::const_iterator>(iter), from, to));
  }
  ...
  iterator emplace(const_iterator iter, TArgs &&...args) {
    return reinterpret_cast<iterator>(
        Base::emplace(reinterpret_cast<typename Base::const_iterator>(iter),
                      std::forward<TArgs>(args)...));
  }

  iterator erase(const_iterator iter) { return erase(iter, iter + 1); }

  /// @brief Erases elements.
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/erase">Reference</a>
...
template <bool TSignedIntegral> struct StaticVectorBaseSignedIntegral;

template <> struct StaticVectorBaseSignedIntegral<true> {
  template <typename T, std::size_t TSize>
  using Type =
      StaticVectorCasted<T, typename std::make_unsigned<T>::type, TSize>;
};

template <> struct StaticVectorBaseSignedIntegral<false> {
  template <typename T, std::size_t TSize>
  using Type = StaticVectorGeneric<T, TSize>;
};

template <typename T, std::size_t TSize>
using ChooseStaticVectorBase = typename StaticVectorBaseSignedIntegral<
...
class StaticVector : public details::ChooseStaticVectorBase<T, TSize> {
  ...
  template <typename U, std::size_t TOtherSize> friend class StaticVector;
...
public:
  ...
  /// @brief Constructor
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/vector">Reference</a>
  StaticVector(size_type count, const T &value) : Base(count, value) {}

  /// @brief Constructor
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/vector">Reference</a>
  explicit StaticVector(size_type count) : Base(count) {}

  /// @brief Constructor
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/vector">Reference</a>
  template <typename TIter>
  StaticVector(TIter from, TIter to) : Base(from, to) {}

  /// @brief Copy constructor
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/vector">Reference</a>
  template <std::size_t TOtherSize>
  StaticVector(const StaticVector<T, TOtherSize> &other) : Base(other) {}

  /// @brief Copy constructor
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/vector">Reference</a>
  StaticVector(const StaticVector &other) : Base(other) {}

  /// @brief Constructor
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/vector">Reference</a>
  StaticVector(std::initializer_list<value_type> init) : Base(init) {}
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/operator%3D">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/assign">Reference</a>
  void assign(size_type count, const T &value) { Base::assign(count, value); }
  ...
  /// @details The bounds check is performed with COMMS_ASSERT() macro, which
  /// means
  ...
  ///     to operator[]().
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/at">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/operator_at">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/front">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/back">Reference</a>
  /// @pre The vector is not empty.
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/data">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/begin">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/end">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/rbegin">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/rend">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/empty">Reference</a>
  bool empty() const { return Base::empty(); }

  /// @brief Returns the number of elements.
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/size">Reference</a>
  size_type size() const { return Base::size(); }

  /// @brief Returns the maximum possible number of elements.
  /// @details Same as capacity().
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/max_size">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/reserve">Reference</a>
  void reserve(size_type new_cap) { return Base::reserve(new_cap); }

  /// @brief Returns the number of elements that can be held in currently
  /// allocated storage.
  /// @details Same as max_size().
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/capacity">Reference</a>
  /// @return TSize provided as template argument.
  ...
  /// @details Does nothing.
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/shrink_to_fit">Reference</a>
  void shrink_to_fit() {}

  /// @brief Clears the contents.
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/clear">Reference</a>
  void clear() { Base::clear(); }

  /// @brief Inserts elements.
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/insert">Reference</a>
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/emplace">Reference</a>
  template <typename... TArgs>
  iterator emplace(const_iterator iter, TArgs &&...args) {
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/push_back">Reference</a>
  /// @pre The vector mustn't be full.
  void push_back(const T &value) { Base::push_back(value); }

  /// @brief Adds an element to the end.
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/push_back">Reference</a>
  /// @pre The vector mustn't be full.
  void push_back(T &&value) { Base::push_back(std::move(value)); }

  /// @brief Constructs an element in place at the end.
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/emplace_back">Reference</a>
  /// @pre The vector mustn't be full.
  template <typename... TArgs> void emplace_back(TArgs &&...args) {
  ...
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/pop_back">Reference</a>
  /// @pre The vector mustn't be empty.
  void pop_back() { Base::pop_back(); }

  /// @brief Changes the number of elements stored.
  /// @see <a
  /// href="http://en.cppreference.com/w/cpp/container/vector/resize">Reference</a>
...
template <typename T> class StaticVector<T, 0U> {
...
public:
  ...
  template <typename TIter> StaticVector(TIter from, TIter to) {
    if (from == to) {
      return;
    }
  ...
  StaticVector(const StaticVector &other) { static_cast<void>(other); }
  ...
  template <typename... TArgs> void emplace_back(TArgs &&...) {
    static constexpr bool Must_not_be_called_for_zero_sized_vector = false;
    static_cast<void>(Must_not_be_called_for_zero_sized_vector);
    COMMS_ASSERT(Must_not_be_called_for_zero_sized_vector);
  }
...
/// @see <a
/// href="http://en.cppreference.com/w/cpp/container/vector/operator_cmp">Reference</a>
...
template <typename T> static constexpr bool isStaticVector() {
...
/// @see <a
/// href="http://en.cppreference.com/w/cpp/container/vector/swap2">Reference</a>
/// @related comms::util::StaticVector
template <typename T, std::size_t TSize1, std::size_t TSize2>

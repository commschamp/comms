...
struct DynMemoryDeleteHandler {
  template <typename TObj> void handle(TObj &obj) const {
    using HandleTag = typename comms::util::LazyShallowConditional<
        std::is_void<TDefaultType>::value>::template Type<NoDefaultCastTag,
                                                          DefaultCastCheckTag>;
...
private:
  ...
  void handleInternal(TObj &obj, DefaultCastCheckTag<TParams...>) const {
    ...
    using Tag = typename comms::util::LazyShallowConditional<std::is_same<
        ObjType, TInterfaceType>::value>::template Type<ForcedDefaultCastTag,
                                                        NoDefaultCastTag>;
...
template <typename TInterface, typename TAllMessages, typename TDeleteHandler,
          typename TId>
...
template <typename TInterface, typename TAllMessages, typename TDeleteHandler,
          typename TId>
class NoVirtualDestructorInPlaceDeleter
    : public NoVirtualDestructorDeleter<TInterface, TAllMessages,
                                        TDeleteHandler, TId> {
  using Base =
...
public:
  ...
  NoVirtualDestructorInPlaceDeleter(TId id, unsigned idx, bool &allocated)
      : Base(id, idx), m_allocated(&allocated) {}

  NoVirtualDestructorInPlaceDeleter(const NoVirtualDestructorInPlaceDeleter &) =
      delete;
  NoVirtualDestructorInPlaceDeleter(NoVirtualDestructorInPlaceDeleter &&other)
      : Base(std::move(other)), m_allocated(other.m_allocated) {
  ...
  NoVirtualDestructorInPlaceDeleter &
  operator=(const NoVirtualDestructorInPlaceDeleter &) = delete;
  NoVirtualDestructorInPlaceDeleter &
  operator=(NoVirtualDestructorInPlaceDeleter &&other) {
...
template <typename T> class InPlaceDeleter {
  template <typename U> friend class InPlaceDeleter;

public:
  InPlaceDeleter(bool *allocated = nullptr) : m_allocated(allocated) {}

  InPlaceDeleter(const InPlaceDeleter &other) = delete;

  template <typename U>
  InPlaceDeleter(InPlaceDeleter<U> &&other) : m_allocated(other.m_allocated) {
  ...
  template <typename U> InPlaceDeleter &operator=(InPlaceDeleter<U> &&other) {
    static_assert(std::is_base_of<T, U>::value ||
                      std::is_base_of<U, T>::value ||
                      std::is_convertible<U, T>::value ||
                      std::is_convertible<T, U>::value,
...
///     with this allocator.
template <typename TInterface> class DynMemory {
public:
  ...
  /// @param[in] args Extra arguments to be passed to allocated object's
  /// constructor.
  ...
  template <typename TObj> static Ptr wrap(TObj *obj) {
    static_assert(std::is_base_of<TInterface, TObj>::value,
                  "TObj does not inherit from TInterface");
    return Ptr(obj);
  }
  ...
  static constexpr bool canAllocate() { return true; }
};
...
/// @tparam TInterface Common interface class for all objects being allocated
///     with this allocator.
/// @tparam TAllMessages Tuple of all messages types, object of which could be
/// allocated.
/// @tparam TId Type of message ID
/// @tparam TDefaultType Message type to cast to when correct type cannot be
/// recognised.
...
class DynMemoryNoVirtualDestructor {
  using Deleter = details::NoVirtualDestructorDeleter<
      TInterface, TAllMessages,
      details::DynMemoryDeleteHandler<TInterface, TDefaultType>, TId>;
...
template <typename TInterface, typename TAllTypes> class InPlaceSingle {
public:
  /// @brief Smart pointer (std::unique_ptr) to the allocated object.
  ...
  /// constructor.
  /// @return Smart pointer to the allocated object.
  /// @pre If @b TObj is NOT the same as @b TInterface, i.e. @b TInterface is a
  /// base
  ///     class of @b TObj, then @b TInterface must have virtual destructor.
  template <typename TObj, typename... TArgs> Ptr alloc(TArgs &&...args) {
    if (m_allocated) {
      return Ptr();
    }
    ...
    Ptr obj(reinterpret_cast<TInterface *>(&m_place),
  ...
  const void *allocAddr() const { return &m_place; }
  ...
  /// @param[in] obj Pointer to previously allocated object.
  /// @return Smart pointer to the wrapped object.
  template <typename TObj> Ptr wrap(TObj *obj) {
    if (obj == nullptr) {
      return Ptr();
    }
    ...
    static_assert(std::is_base_of<TInterface, TObj>::value,
                  "TObj does not inherit from TInterface");
    COMMS_ASSERT(obj == reinterpret_cast<TInterface *>(
                            &m_place)); // Wrong object if fails
    COMMS_ASSERT(m_allocated);          // Error if not set
    return Ptr(reinterpret_cast<TInterface *>(&m_place),
               details::InPlaceDeleter<TInterface>(&m_allocated));
  }

  /// @brief Inquiry whether allocation is possible.
  bool canAllocate() const { return !m_allocated; }
...
/// @tparam TInterface Common interface class for all objects being allocated
///     with this allocator.
/// @tparam TAllocMessages All the possible message types that can be allocated
/// with this
///     allocator bundled in @b std::tuple. They are used to identify the
///     size required to allocate any of the provided objects.
/// @tparam TOrigMessages All the original message types (without @ref
/// comms::GenericMessage added)
///     ids of which are known at compile time.
/// @tparam TDefaultType Message type to cast to when correct type cannot be
/// recognised.
///     @b void means does not exist.
template <typename TInterface, typename TAllocMessages, typename TOrigMessages,
          typename TId, typename TDefaultType = void>
class InPlaceSingleNoVirtualDestructor {
  using Deleter = details::NoVirtualDestructorInPlaceDeleter<
      TInterface, TOrigMessages,
      details::InPlaceDeleteHandler<TInterface, TDefaultType>, TId>;
...
public:
  ...
  Ptr alloc(TId id, unsigned idx, TArgs &&...args) {
    ...
    static_assert(
        comms::util::IsInTuple<TAllocMessages>::template Type<TObj>::value,
        ""
        "TObj must be in provided tuple of supported types");
...
template <typename TInterface, std::size_t TSize,
...
public:
  ...
  template <typename TObj, typename... TArgs> Ptr alloc(TArgs &&...args) {
    auto iter = std::find_if(
        ...
        [](const PoolElem &elem) -> bool { return !elem.allocated(); });
  ...
  template <typename TObj> Ptr wrap(TObj *obj) {
    auto iter = std::find_if(
        m_pool.begin(), m_pool.end(), [obj](const PoolElem &elem) -> bool {
...
template <typename...> struct InPlaceSingleDeepCondWrap {
  template <typename TInterface, typename TAllTypes, typename...>
  using Type = comms::util::alloc::InPlaceSingle<TInterface, TAllTypes>;
};

template <typename...> struct InPlaceSingleNoVirtualDestructorDeepCondWrap {
  template <typename TInterface, typename TAllocMessages,
            typename TOrigMessages, typename TId, typename TDefaultType,
            typename...>
  using Type = comms::util::alloc::InPlaceSingleNoVirtualDestructor<
      TInterface, TAllocMessages, TOrigMessages, TId, TDefaultType>;
};

template <typename...> struct DynMemoryDeepCondWrap {
  template <typename TInterface, typename...>
  using Type = comms::util::alloc::DynMemory<TInterface>;
};

template <typename...> struct DynMemoryNoVirtualDestructorDeepCondWrap {
  template <typename TInterface, typename TAllMessages, typename TId,
            typename TDefaultType, typename...>
  using Type =
      comms::util::alloc::DynMemoryNoVirtualDestructor<TInterface, TAllMessages,
                                                       TId, TDefaultType>;
};

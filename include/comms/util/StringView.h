...
public:
  ...
  /// @brief Constructor
  StringView(const std::string &str) noexcept : Base(str.c_str(), str.size()) {}
  ...
  /// @details Omits the last '\0' character if such exists
  template <std::size_t TN> StringView &operator=(const char (&str)[TN]) {
    Base::operator=(str);
    if ((0U <= TN) && (back() == '\0')) {
      remove_suffix(1);
    }
  ...
  /// @brief Assign array of characters with known size
  /// @details Omits the last '\0' character if such exists
  template <std::size_t TN> StringView &operator=(char (&str)[TN]) {
    Base::operator=(str);
    if ((0U <= TN) && (back() == '\0')) {
      remove_suffix(1);
    }
  ...
  constexpr iterator begin() const noexcept { return Base::begin(); }
  ...
  constexpr iterator end() const noexcept { return Base::end(); }
  ...
  reverse_iterator rend() const noexcept { return Base::rend(); }
  ...
  /// @brief Similar to <a
  /// href="http://en.cppreference.com/w/cpp/string/basic_string_view/at">std::string_view::at()</a>
  /// @details Checks the range with @ref COMMS_ASSERT() macro without throwing
  /// exception.
  ...
  constexpr const_reference front() const { return Base::front(); }

  /// @brief Same as <a
  /// href="http://en.cppreference.com/w/cpp/string/basic_string_view/back">std::string_view::back()</a>
  constexpr const_reference back() const { return Base::back(); }
  ...
  constexpr size_type length() const noexcept { return Base::length(); }
  ...
  /// @brief Same as <a
  /// href="http://en.cppreference.com/w/cpp/string/basic_string_view/compare">std::string_view::compare()</a>.
  int compare(size_type pos1, size_type count1, const StringView &other,
  ...
  /// @brief Same as <a
  /// href="http://en.cppreference.com/w/cpp/string/basic_string_view/compare">std::string_view::compare()</a>.
  int compare(const char *s) const { return compare(0U, size(), s); }
...
/// @related StringView
inline bool operator<(const StringView &str1, const StringView &str2) {
...
inline void swap(comms::util::StringView &str1, comms::util::StringView &str2) {
  str1.swap(str2);
}

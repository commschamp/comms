...
public:
  template <typename T> static std::size_t maxSizeOf(const T &val) {
    ...
    using Tag = typename comms::util::LazyShallowConditional<
        ...
        template Type<HasMaxSizeTag, NoMaxSizeTag>;
...
/// @cond SKIP_DOC
template <typename T> std::size_t maxSizeOf(const T &val) {

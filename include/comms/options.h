...
template <typename T> struct IsRatio {
...
template <typename T> constexpr bool isRatio() { return IsRatio<T>::Value; }

template <typename T, T TVal> struct DefaultNumValueInitialiser {
  template <typename TField> void operator()(TField &&field) {
...
struct NumValueRangeValidator {
  static_assert(TMinValue <= TMaxValue,
                "Min value must be not greater than Max value");

  template <typename TField>
  constexpr bool operator()(const TField &field) const {
    using MinTag = typename comms::util::Conditional<(
        std::numeric_limits<decltype(MinValue)>::min() <
        MinValue)>::template Type<CompareTag, ReturnTrueTag>;

    using MaxTag = typename comms::util::Conditional<(
        MaxValue < std::numeric_limits<decltype(MaxValue)>::max())>::
        template Type<CompareTag, ReturnTrueTag>::type;

    return aboveMin(field.value(), MinTag()) &&
           belowMax(field.value(), MaxTag());
  }
...
struct BitmaskReservedBitsValidator {
  template <typename TField> constexpr bool operator()(TField &&field) const {
    using FieldType = typename std::decay<TField>::type;
    using ValueType = typename FieldType::ValueType;

    return (field.value() & static_cast<ValueType>(TMask)) ==
           static_cast<ValueType>(TValue);
  }
};

template <comms::field::OptionalMode TVal> struct DefaultOptModeInitialiser {
  template <typename TField> void operator()(TField &field) const {
    field.setMode(TVal);
  }
};

template <std::size_t TIdx> struct DefaultVariantIndexInitialiser {
  template <typename TField> void operator()(TField &field) {
...
template <typename TEndian> struct Endian {};
...
/// @headerfile comms/options.h
template <std::intmax_t TId> struct StaticNumIdImpl {};
...
/// @headerfile comms/options.h
template <typename TMsg> struct MsgType {};

/// @brief Option used to specify actual type of the field.
/// @headerfile comms/options.h
template <typename TField> struct FieldType {};
...
///     of such fields, bundled in @b std::tuple, to @ref comms::Message
///     interface class.
/// @tparam TFields The fields of the message bundled in std::tuple.
/// @headerfile comms/options.h
template <typename TFields> struct ExtraTransportFields {};
...
/// @headerfile comms/options.h
template <std::size_t TIdx> struct VersionInExtraTransportFields {};
...
/// @tparam TFields The fields of the message bundled in std::tuple.
/// @headerfile comms/options.h
template <typename TFields> struct FieldsImpl;

/// @cond SKIP_DOC
template <typename... TFields> struct FieldsImpl<std::tuple<TFields...>> {};
/// @endcond
...
/// @brief Option used to specify number of bytes that is used for field
/// serialisation.
...
/// @headerfile comms/options.h
template <std::size_t TLen, bool TSignExtend = true> struct FixedLength {};

/// @brief Option used to specify number of bits that is used for field
/// serialisation
///     when a field is a member of comms::field::Bitfield.
/// @details For example, the protocol specifies that two independent integer
///     values of 6 and 10 bits respectively packed into two bytes to save
///     space. Such combined field may be defined as:
///     @code
///     using MyFieldBase = comms::Field<comms::option::def::BigEndian>;
///     using MyField =
...
/// @tparam TSignExtend Perform sign extension, relevant only to signed types.
/// @headerfile comms/options.h
template <std::size_t TLen, bool TSignExtend = true> struct FixedBitLength {};

/// @brief Option used to specify that field may have variable serialisation
/// length
/// @details Applicable only to numeric fields, such as comms::field::IntValue
///     or comms::field::EnumValue.
///     Use this option to specify that serialised value has
///     <a
///     href="https://en.wikipedia.org/wiki/Variable-length_quantity">Base-128</a>
...
/// @pre TMin <= TMax
/// @headerfile comms/options.h
template <std::size_t TMin, std::size_t TMax> struct VarLength {
...
///     for such option would be serialising a "current year" value. Most
///     protocols now specify it as an offset from year 2000 or later and
///     written as a single byte, i.e. to specify year 2015 is to write
///     value 15. However it may be inconvenient to manually adjust
///     serialised/deserialised value by predefined offset 2000. To help with
///     such case option comms::option::def::NumValueSerOffset can be used. For
///     example:
///     @code
///     using MyFieldBase = comms::Field<comms::option::def::BigEndian>;
///     using MyField =
...
///         >;
///     @endcode
///     Note that in the example above the field value (accessible by @b value()
///     member function of the field) will have type std::uint16_t and will be
///     equal to say 2015, while when serialised it consumes only 1 byte (thanks
///     to comms::option::def::FixedLength option) and reduced value of 15 is
///     written.
/// @tparam TOffset Offset value to be added when serialising field.
/// @headerfile comms/options.h
template <std::intmax_t TOffset> struct NumValueSerOffset {};
...
///     integer value of millimetres, but while processing it should be handled
///     as floating point value of meters. Such field is defined as:
///     @code
///     using MyFieldBase = comms::Field<comms::option::def::BigEndian>;
///     using MyField =
///         comms::field::IntValue<
...
/// @headerfile comms/options.h
template <std::intmax_t TNum, std::intmax_t TDenom> struct ScalingRatio {
...
/// @headerfile comms/options.h
template <typename TField> struct SequenceSizeFieldPrefix {};
...
/// @tparam TReadErrorStatus Error status to return in case read operation fails
/// when should not
/// @headerfile comms/options.h
template <typename TField, comms::ErrorStatus TReadErrorStatus =
                               comms::ErrorStatus::InvalidMsgData>
struct SequenceSerLengthFieldPrefix {};

/// @brief Option that forces <b>every element</b> of @ref
/// comms::field::ArrayList to
///     be prefixed with its serialisation length.
/// @details Similar to @ref SequenceSerLengthFieldPrefix but instead of the
/// whole
...
/// @brief Option that forces @b first element only of @ref
/// comms::field::ArrayList to
///     be prefixed with its serialisation length.
...
///                 comms::field::IntValue<MyFieldBase, char,
///                 comms::option::def::DefaultNumValue<0> >
///             >
///         >;
///     @endcode
/// @tparam TField Type of the field that represents suffix
/// @headerfile comms/options.h
template <typename TField> struct SequenceTerminationFieldSuffix {};
...
/// @tparam TField Type of the field that represents suffix
/// @headerfile comms/options.h
template <typename TField> struct SequenceTrailingFieldSuffix {};
...
///     the collection fields, such as comms::field::ArrayList or
///     comms::field::String which can be used to specify the size information
///     after it was read independently.
/// @headerfile comms/options.h
struct SequenceSizeForcingEnabled {};

/// @brief Option to enable external forcing of the collection's serialisation
/// length
///     duting "read" operation.
/// @details Sometimes the length information is detached from the data sequence
///     itself, i.e. there may be one or more independent fields between the
///     length field and the first byte of the collection. In such case it
///     becomes impossible to use @ref SequenceSerLengthFieldPrefix option.
///     Instead, the length information must be provided by external calls.
///     Usage of this option enables @b forceReadLength() and @b
///     clearReadLengthForcing() functions in the collection fields, such as
///     comms::field::ArrayList or comms::field::String which can be used to
///     specify the size information after it was read independently.
/// @headerfile comms/options.h
struct SequenceLengthForcingEnabled {};

/// @brief Option to enable external forcing of the collection element
///     serialisation length.
/// @details Some protocols may prefix the variable length lists with
/// serialisation
///     length of a <b>single element</b> in addition to the number of elements
///     in the list. Usage of this option
///     enables @b forceReadElemLength() and @b clearReadElemLengthForcing()
///     functions in the comms::field::ArrayList which can be used to specify
///     the element serialisation length after it was read independently. @n
/// @headerfile comms/options.h
struct SequenceElemLengthForcingEnabled {};

/// @brief Option used to define exact number of elements in the collection
/// field.
...
///     @b write() member functions of the collection field to behave as
///     expected.
/// @headerfile comms/options.h
template <std::size_t TSize> struct SequenceFixedSize {};
...
///     value. The provided initialiser class with the option will be
///     instantiated and its operator() is invoked which is responsible to
///     assign proper value to the field.
///     @code
...
/// @headerfile comms/options.h
template <typename T> struct DefaultValueInitialiser {};

/// @brief Option that specifies custom validation class.
/// @details By default, value of every field is considered to be valid
///     (@b valid() member function of the field returns @b true). If there is a
///     need to validate the value of the function, use this option to define
///     custom validation logic for the field. The validation class provided as
///     a template argument to this option must define the following member
///     function:
///     @code
///     struct MyValidator
///     {
...
///     comms::option::def::DefaultValueInitialiser option to specify proper
///     default value.
/// @note Direct usage of this option in the client code is not recommended.
/// It's
///     should be used for internal validators like @ref
///     comms::option::def::BitmaskReservedBits
/// @tparam T Type of the validator class.
/// @headerfile comms/options.h
template <typename T> struct ContentsValidator {};
...
///     If comms::option::def::FailOnInvalid is provided as an option to a
///     field, the validity is going to checked automatically after the read. If
///     invalid value is identified, error will be returned from the @b read()
///     operation.
/// @tparam TStatus Error status to return when the content of the read field is
/// invalid.
/// @headerfile comms/options.h
template <comms::ErrorStatus TStatus = comms::ErrorStatus::InvalidMsgData>
struct FailOnInvalid {};

/// @brief Option that forces field's read operation to ignore read data if
/// invalid value
///     is received.
...
///     @b std::ratio type.
/// @headerfile comms/options.h
template <typename TType, typename TRatio> struct Units {
...
/// @headerfile comms/options.h
using UnitsNanometers = Units<comms::traits::units::Distance,
                              comms::traits::units::NanometersRatio>;

/// @brief Alias option, specifying field value units are "micrometers".
/// @headerfile comms/options.h
using UnitsMicrometers = Units<comms::traits::units::Distance,
                               comms::traits::units::MicrometersRatio>;

/// @brief Alias option, specifying field value units are "millimeters".
/// @headerfile comms/options.h
using UnitsMillimeters = Units<comms::traits::units::Distance,
                               comms::traits::units::MillimetersRatio>;

/// @brief Alias option, specifying field value units are "centimeters".
/// @headerfile comms/options.h
using UnitsCentimeters = Units<comms::traits::units::Distance,
                               comms::traits::units::CentimetersRatio>;
...
/// @headerfile comms/options.h
using UnitsKilometers = Units<comms::traits::units::Distance,
                              comms::traits::units::KilometersRatio>;

/// @brief Alias option, specifying field value units are "nanometers per
/// second".
/// @headerfile comms/options.h
using UnitsNanometersPerSecond =
    Units<comms::traits::units::Speed,
          comms::traits::units::NanometersPerSecondRatio>;

/// @brief Alias option, specifying field value units are "micrometers per
/// second".
/// @headerfile comms/options.h
...
/// @brief Alias option, specifying field value units are "millimeters per
/// second".
/// @headerfile comms/options.h
...
/// @brief Alias option, specifying field value units are "centimeters per
/// second".
/// @headerfile comms/options.h
...
/// @headerfile comms/options.h
using UnitsMetersPerSecond = Units<comms::traits::units::Speed,
                                   comms::traits::units::MetersPerSecondRatio>;

/// @brief Alias option, specifying field value units are "kilometers per
/// second".
/// @headerfile comms/options.h
...
using UnitsKilometersPerHour =
    Units<comms::traits::units::Speed,
          comms::traits::units::KilometersPerHourRatio>;
...
///     class instead.
/// @tparam TVal Numeric value is to be assigned to the field in default
/// constructor.
/// @see @ref DefaultBigUnsignedNumValue
/// @headerfile comms/options.h
template <std::intmax_t TVal>
using DefaultNumValue = DefaultValueInitialiser<
    details::DefaultNumValueInitialiser<std::intmax_t, TVal>>;

/// @brief Alias to DefaultValueInitialiser, it defines initialiser class that
///     assigns big unsigned numeric value provided as the template argument to
///     this option.
...
using DefaultBigUnsignedNumValue = DefaultValueInitialiser<
...
///     This option can be used multiple times to provide several valid
///     ranges.@n If values are too big to fit into @b std::intmax_t type,
///     please use
...
///     that allows usage of multiple @ref ValidNumValueRange options. If this
///     is the case, please don't pass more than one @ref ValidNumValueRange
///     option.
...
using ValidNumValueRangeOverride =
    std::tuple<ValidNumValueRange<TMinValue, TMaxValue>, ValidRangesClear>;
...
/// @warning Some older compilers (@b gcc-4.7) fail to compile valid C++11 code
///     that allows usage of multiple @ref ValidNumValueRange options. If this
///     is the case, please don't pass more than one
...
/// @brief Similar to @ref ValidBigUnsignedNumValueRange, but overrides
/// (nullifies)
///     all previously set valid values ranges.
/// @see @ref ValidNumValueOverride
...
using ValidBigUnsignedNumValueRangeOverride =
    std::tuple<ValidBigUnsignedNumValueRange<TMinValue, TMaxValue>,
               ValidRangesClear>;
...
/// @details Equivalent to @b ValidBigUnsignedNumValueRangeOverride<TValue,
/// TValue>
/// @deprecated Use @ref ValidRangesClear instead.
template <std::uintmax_t TValue>
using ValidBigUnsignedNumValueOverride =
    ValidBigUnsignedNumValueRangeOverride<TValue, TValue>;
...
using BitmaskReservedBits =
    ContentsValidator<details::BitmaskReservedBitsValidator<TMask, TValue>>;

/// @brief Alias to DefaultValueInitialiser, it sets default mode
///     to field::Optional field.
/// @tparam TVal Optional mode value is to be assigned to the field in default
/// constructor.
/// @see @ref MissingByDefault
/// @see @ref ExistsByDefault
/// @headerfile comms/options.h
template <comms::field::OptionalMode TVal>
using DefaultOptionalMode =
    DefaultValueInitialiser<details::DefaultOptModeInitialiser<TVal>>;
...
/// @brief Alias to DefaultValueInitialiser, it initalises comms::field::Variant
/// field
///     to contain valid default value of the specified member.
/// @tparam TIdx Index of the default member.
/// @headerfile comms/options.h
template <std::size_t TIdx>
using DefaultVariantIndex =
    DefaultValueInitialiser<details::DefaultVariantIndexInitialiser<TIdx>>;
...
///     forwarding read to the wrapped layer(s).
/// @tparam TEscField - Escape field to avoid detection of the field being
/// seeked.
/// @headerfile comms/options.h
template <typename TEscField = void> struct FrameLayerSeekField {};
...
///     and are not present on I/O link when serialized. Sometimes it is
///     convenient to have such values abstracted away as fields, which are not
///     actually serialised. Using this option will have such effect: read/write
///     operaitons will not change the value of iterators and will report
///     immediate success. The serialisation length is always reported as 0.
/// @headerfile comms/options.h
...
/// @deprecated Use @ref comms::option::def::FrameLayerForceReadUntilDataSplit
/// instead.
using ProtocolLayerForceReadUntilDataSplit = FrameLayerForceReadUntilDataSplit;

/// @brief Option to forcefully disable passing the @ref
/// FrameLayerForceReadUntilDataSplit
///     option to the layer definition.
/// @details Can be used by some customized layers default implementation of
///     which pass the @ref
///     comms::option::def::FrameLayerForceReadUntilDataSplit option to the @ref
...
/// @deprecated Use @ref
/// comms::option::def::FrameLayerSuppressReadUntilDataSplitForcing instead.
using ProtocolLayerSuppressReadUntilDataSplitForcing =
    FrameLayerSuppressReadUntilDataSplitForcing;
...
/// @deprecated Use @ref
/// comms::option::def::FrameLayerDisallowReadUntilDataSplit instead.
using ProtocolLayerDisallowReadUntilDataSplit =
    FrameLayerDisallowReadUntilDataSplit;
...
using ExistsSinceVersion =
    ExistsBetweenVersions<TVer, std::numeric_limits<std::uintmax_t>::max()>;
...
/// @details The version information can be accessed using @b getVersion()
/// member function.
/// @headerfile comms/options.h
...
/// @headerfile comms/options.h
template <typename T> struct ExtendingClass {};

/// @brief Option to specify index of member field containing remaining length
/// in bytes
/// @details Applicable only to @ref comms::field::Bundle fields.
/// @headerfile comms/options.h
template <std::size_t TIdx> struct RemLengthMemberField {};
...
/// @brief Avoid invocation of built-in @ref comms::field::Variant::reset()
/// "reset()"
...
/// @brief Mark complex fields like @ref comms::field::Bundle or @ref
/// comms::field::Variant
///     that their members are or are not version dependent.
/// @details Usage of this options eliminates compile time checks of whether the
/// members
///     are version dependent significantly reducing compilation times for
///     fields with a long list of members.
/// @tparam TVersionDependent - @b true in case members are version dependent,
/// @b false otherwise.
template <bool TVersionDependent> struct HasVersionDependentMembers {};

/// @brief Remove an ability to update field's value by the client code
struct FixedValue {};

/// @brief Option used to specify display offset for the @ref
/// comms::field::IntValue field.
/// @tparam TOffset Display offset.
/// @headerfile comms/options.h
template <std::intmax_t TOffset> struct DisplayOffset {};
...
/// @headerfile comms/options.h
template <typename TIter> struct ReadIterator {};

/// @brief Option used to specify type of iterator used for writing.
/// @tparam TIter Type of the iterator.
/// @headerfile comms/options.h
template <typename TIter> struct WriteIterator {};
...
/// @brief Option that inhibits implementation of
/// comms::MessageBase::writeImpl()
///     regardless of other availability conditions.
/// @headerfile comms/options.h
struct NoWriteImpl {};

/// @brief Option that inhibits implementation of
/// comms::MessageBase::validImpl()
///     regardless of other availability conditions.
/// @headerfile comms/options.h
struct NoValidImpl {};

/// @brief Option that inhibits implementation of
/// comms::MessageBase::lengthImpl()
///     regardless of other availability conditions.
/// @headerfile comms/options.h
struct NoLengthImpl {};

/// @brief Option that inhibits implementation of
/// comms::MessageBase::refreshImpl()
///     regardless of other availability conditions.
/// @headerfile comms/options.h
...
template <typename TGenericMessage> struct SupportGenericMessage {};
...
///     such fields to use @ref comms::util::StaticVector or @ref
///     comms::util::StaticString with the capacity provided by this option.
/// @tparam TSize Size of the storage area in number of elements, for strings it
/// does @b NOT include
///     the '\0' terminating character.
/// @headerfile comms/options.h
template <std::size_t TSize> struct FixedSizeStorage {};
...
///     <a
///     href="http://en.cppreference.com/w/cpp/container/vector">std::vector</a>
///     as their internal storage types. The @ref FixedSizeStorage option forces
...
///     Such type must define the same public interface as @b std::string (when
///     used with comms::field::String) or @b std::vector (when used with
///     comms::field::ArrayList).
/// @tparam TType Custom storage type
/// @headerfile comms/options.h
template <typename TType> struct CustomStorageType {};

/// @brief Option that forces usage of fixed size storage for sequences with
/// fixed
///     size.
/// @details Equivalent to @ref FixedSizeStorage option, but applicable only
///     to sequence types @ref comms::field::ArrayList or @ref
///     comms::field::String, that alrady use @ref SequenceFixedSize option.
///     Usage of this option do not require knowledge of the storage area size.
/// @headerfile comms/options.h
...
/// @details Can be used with @ref comms::field::String and raw data @ref
/// comms::field::ArrayList. @n
///     For @ref comms::field::String it will force usage of
///     <a
///     href="https://en.cppreference.com/w/cpp/string/basic_string_view">std::string_view</a>
///     (if available) as inner storage type (instead of @b std::string). In
///     case @b std::string_view is unavalable (C++17 support is disabled or
///     standard library of insufficient version) @ref comms::util::StringView
///     will be used instead.@n
///     For raw data @ref comms::field::ArrayList it will force usage of
///     <a
///     href="https://en.cppreference.com/w/cpp/container/span">std::span&lt;std::uint8_t&gt;</a>(if
///     available) as inner storage type (instead of @b
///     std::vector<std::uint8_t>). In case @b std::span is unavalable (C++20
///     support is disabled or standard library of insufficient version) @ref
///     comms::util::ArrayView will be used instead.@n
...
///     such as @ref comms::option::app::CustomStorageType or @ref
///     comms::option::app::FixedSizeStorage
/// @note To force usage of provided @ref comms::util::StringView or @ref
/// comms::util::ArrayView
///     instead of standard @b std::string_view or @b std::span, use @ref
///     comms::option::app::CustomStorageType option.
/// @headerfile comms/options.h
struct OrigDataView {};

/// @brief Force a particular way to dispatch message object and/or type.
/// @tparam T Expected to be one of the tags from @ref comms::traits::dispatch
/// namespace.
template <typename T> struct ForceDispatch {};

/// @brief Force generation of v-tables and polymorphic dispatch logic for
///     message object and/or message object type
using ForceDispatchPolymorphic =
    ForceDispatch<comms::traits::dispatch::Polymorphic>;

/// @brief Force generation of static binary search dispatch logic for
///     message object and/or message object type
using ForceDispatchStaticBinSearch =
    ForceDispatch<comms::traits::dispatch::StaticBinSearch>;

/// @brief Force generation of linear switch statmenets for dispatch logic of
///     message object and/or message object type
using ForceDispatchLinearSwitch =
    ForceDispatch<comms::traits::dispatch::LinearSwitch>;

/// @brief Force usage of the provide message factory.
/// @details Applicable to @ref comms::frame::MsgIdLayer.
/// @tparam TFactory Factory class, expected to expose the same interface as
/// @ref comms::MsgFactory
template <typename TFactory> struct MsgFactory {};

/// @brief Force usage of the provide message factory.
/// @details Similar to @ref comms::option::app::MsgFactory, but the template
///     parameter is template template class with the same template arguments as
///     @ref comms::MsgFactory>.
/// @tparam TFactory Factory template, expected to have the same template
/// parameters and expose
...
template <typename TEndian> using Endian = comms::option::def::Endian<TEndian>;
...
template <typename T> using MsgIdType = comms::option::def::MsgIdType<T>;
...
template <typename TMsg> using MsgType = comms::option::def::MsgType<TMsg>;

/// @brief Same as @ref comms::option::def::FieldType
/// @deprecated Use @ref comms::option::def::FieldType instead.
template <typename TMsg> using FieldType = comms::option::def::FieldType<TMsg>;
...
/// @deprecated Use @ref comms::option::def::VersionInExtraTransportFields
/// instead.
template <std::size_t TIdx>
using VersionInExtraTransportFields =
...
template <typename TField>
using SequenceSizeFieldPrefix =
    comms::option::def::SequenceSizeFieldPrefix<TField>;

/// @brief Same as @ref comms::option::def::SequenceSerLengthFieldPrefix
/// @deprecated Use @ref comms::option::def::SequenceSerLengthFieldPrefix
/// instead.
template <typename TField, comms::ErrorStatus TReadErrorStatus =
                               comms::ErrorStatus::InvalidMsgData>
...
/// @deprecated Use @ref comms::option::def::SequenceElemSerLengthFieldPrefix
/// instead.
template <typename TField, comms::ErrorStatus TReadErrorStatus =
                               comms::ErrorStatus::InvalidMsgData>
using SequenceElemSerLengthFieldPrefix =
    comms::option::def::SequenceElemSerLengthFieldPrefix<TField,
                                                         TReadErrorStatus>;

/// @brief Same as @ref
/// comms::option::def::SequenceElemFixedSerLengthFieldPrefix
/// @deprecated Use @ref
/// comms::option::def::SequenceElemFixedSerLengthFieldPrefix instead.
template <typename TField, comms::ErrorStatus TReadErrorStatus =
                               comms::ErrorStatus::InvalidMsgData>
using SequenceElemFixedSerLengthFieldPrefix =
    comms::option::def::SequenceElemFixedSerLengthFieldPrefix<TField,
                                                              TReadErrorStatus>;

/// @brief Same as @ref comms::option::def::SequenceTerminationFieldSuffix
/// @deprecated Use @ref comms::option::def::SequenceTerminationFieldSuffix
/// instead.
template <typename TField>
using SequenceTerminationFieldSuffix =
    comms::option::def::SequenceTerminationFieldSuffix<TField>;

/// @brief Same as @ref comms::option::def::SequenceTrailingFieldSuffix
/// @deprecated Use @ref comms::option::def::SequenceTrailingFieldSuffix
/// instead.
template <typename TField>
using SequenceTrailingFieldSuffix =
    comms::option::def::SequenceTrailingFieldSuffix<TField>;
...
using SequenceSizeForcingEnabled =
    comms::option::def::SequenceSizeForcingEnabled;

/// @brief Same as @ref comms::option::def::SequenceLengthForcingEnabled
/// @deprecated Use @ref comms::option::def::SequenceLengthForcingEnabled
/// instead.
using SequenceLengthForcingEnabled =
    comms::option::def::SequenceLengthForcingEnabled;

/// @brief Same as @ref comms::option::def::SequenceElemLengthForcingEnabled
/// @deprecated Use @ref comms::option::def::SequenceElemLengthForcingEnabled
/// instead.
using SequenceElemLengthForcingEnabled =
...
template <std::intmax_t TMinValue, std::intmax_t TMaxValue>
using ValidNumValueRangeOverride =
    comms::option::def::ValidNumValueRangeOverride<TMinValue, TMaxValue>;
...
/// @deprecated Use @ref comms::option::def::ValidBigUnsignedNumValueRange
/// instead.
template <std::uintmax_t TMinValue, std::uintmax_t TMaxValue>
...
/// @brief Same as @ref
/// comms::option::def::ValidBigUnsignedNumValueRangeOverride
/// @deprecated Use @ref
/// comms::option::def::ValidBigUnsignedNumValueRangeOverride instead.
template <std::uintmax_t TMinValue, std::uintmax_t TMaxValue>
using ValidBigUnsignedNumValueRangeOverride =
    comms::option::def::ValidBigUnsignedNumValueRangeOverride<TMinValue,
                                                              TMaxValue>;
...
/// @deprecated Use @ref comms::option::def::ValidBigUnsignedNumValueOverride
/// instead.
template <std::uintmax_t TValue>
using ValidBigUnsignedNumValueOverride =
...
template <std::uintmax_t TMask, std::uintmax_t TValue = 0U>
using BitmaskReservedBits =
    comms::option::def::BitmaskReservedBits<TMask, TValue>;
...
/// @deprecated Use @ref comms::option::def::FrameLayerVerifyBeforeRead instead.
using ChecksumLayerVerifyBeforeRead =
...
template <typename T> using VersionType = comms::option::def::VersionType<T>;
...
template <typename T> using Handler = comms::option::app::Handler<T>;
...
using SupportGenericMessage =
    comms::option::app::SupportGenericMessage<TGenericMessage>;
...
/// @deprecated Use @ref
/// comms::option::app::SequenceFixedSizeUseFixedSizeStorage instead.
using SequenceFixedSizeUseFixedSizeStorage =
...
/// @deprecated Use @ref comms::option::app::ForceDispatchStaticBinSearch
/// instead.
using ForceDispatchStaticBinSearch =

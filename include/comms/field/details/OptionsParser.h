...
template <typename... TOptions> class OptionsParser;

template <> class OptionsParser<> {
public:
  ...
  static constexpr std::size_t SequenceFixedSize =
      std::numeric_limits<std::size_t>::max();
  static constexpr MembersVersionDependency ForcedMembersVersionDependency =
      MembersVersionDependency_NotSpecified;

  template <typename TField> using AdaptInvalidByDefault = TField;

  template <typename TField> using AdaptVersionStorage = TField;

  template <typename TField> using AdaptSerOffset = TField;

  template <typename TField> using AdaptVersionsRange = TField;

  template <typename TField> using AdaptFixedLengthLimit = TField;

  template <typename TField> using AdaptFixedBitLengthLimit = TField;

  template <typename TField> using AdaptVarLengthLimits = TField;

  template <typename TField> using AdaptAvailableLengthLimit = TField;

  template <typename TField> using AdaptSequenceElemLengthForcing = TField;
  ...
  template <typename TField> using AdaptSequenceSizeForcing = TField;

  template <typename TField> using AdaptSequenceLengthForcing = TField;

  template <typename TField> using AdaptSequenceFixedSize = TField;

  template <typename TField> using AdaptSequenceSizeFieldPrefix = TField;

  template <typename TField> using AdaptSequenceSerLengthFieldPrefix = TField;

  template <typename TField> using AdaptSequenceTrailingFieldSuffix = TField;

  template <typename TField> using AdaptSequenceTerminationFieldSuffix = TField;

  template <typename TField> using AdaptRemLengthMemberField = TField;

  template <typename TField> using AdaptDefaultValueInitialiser = TField;

  template <typename TField> using AdaptMultiRangeValidation = TField;

  template <typename TField> using AdaptCustomValidator = TField;

  template <typename TField> using AdaptFailOnInvalid = TField;

  template <typename TField> using AdaptIgnoreInvalid = TField;

  template <typename TField> using AdaptEmptySerialization = TField;

  template <typename TField> using AdaptCustomRead = TField;

  template <typename TField> using AdaptCustomRefresh = TField;

  template <typename TField> using AdaptCustomWrite = TField;

  template <typename TField> using AdaptFieldType = TField;

  template <typename TField> using AdaptMissingOnReadFail = TField;

  template <typename TField> using AdaptMissingOnInvalid = TField;

  template <typename TField>
  using AdaptVariantResetOnDestruct =
      typename comms::util::Conditional<std::is_same<
          typename TField::CommsTag, comms::field::tag::Variant>::value>::
          template Type<comms::field::adapter::VariantResetOnDestruct<TField>,
                        TField>;

  template <typename TField> using AdaptFixedValue = TField;

  template <typename TField> using AdaptDisplayOffset = TField;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::HasCustomRead, TOptions...>
...
class OptionsParser<comms::option::def::NumValueSerOffset<TOffset>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
class OptionsParser<comms::option::def::FixedLength<TLen, TSignExtend>,
...
public:
  ...
  using AdaptFixedLengthLimit =
      comms::field::adapter::FixedLength<FixedLength, FixedLengthSignExtend,
                                         TField>;
};

template <std::size_t TLen, bool TSignExtend, typename... TOptions>
class OptionsParser<comms::option::def::FixedBitLength<TLen, TSignExtend>,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  ...
  using AdaptFixedBitLengthLimit =
      comms::field::adapter::FixedBitLength<FixedBitLength,
                                            FixedBitLengthSignExtend, TField>;
};

template <std::size_t TMinLen, std::size_t TMaxLen, typename... TOptions>
class OptionsParser<comms::option::def::VarLength<TMinLen, TMaxLen>,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
...
template <typename... TOptions>
class OptionsParser<comms::option::def::AvailableLengthLimit, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
template <typename... TOptions>
class OptionsParser<comms::option::def::SequenceSizeForcingEnabled, TOptions...>
    : public OptionsParser<TOptions...> {
public:
  static constexpr bool HasSequenceSizeForcing = true;

  template <typename TField>
  using AdaptSequenceSizeForcing =
      comms::field::adapter::SequenceSizeForcing<TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::SequenceLengthForcingEnabled,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  static constexpr bool HasSequenceLengthForcing = true;

  template <typename TField>
  using AdaptSequenceLengthForcing =
      comms::field::adapter::SequenceLengthForcing<TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::SequenceElemLengthForcingEnabled,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  static constexpr bool HasSequenceElemLengthForcing = true;

  template <typename TField>
  using AdaptSequenceElemLengthForcing =
      comms::field::adapter::SequenceElemLengthForcing<TField>;
};

template <std::size_t TSize, typename... TOptions>
class OptionsParser<comms::option::def::SequenceFixedSize<TSize>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
  ...
  template <typename TField>
  using AdaptSequenceFixedSize =
      comms::field::adapter::SequenceFixedSize<SequenceFixedSize, TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::app::SequenceFixedSizeUseFixedSizeStorage,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
...
class OptionsParser<comms::option::def::SequenceSizeFieldPrefix<TSizeField>,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  ...
  template <typename TField>
  using AdaptSequenceSizeFieldPrefix =
      comms::field::adapter::SequenceSizeFieldPrefix<SequenceSizeFieldPrefix,
                                                     TField>;
};
...
public:
  ...
  static constexpr comms::ErrorStatus SequenceSerLengthFieldReadErrorStatus =
      TReadErrorStatus;

  template <typename TWrappedField>
  using AdaptSequenceSerLengthFieldPrefix =
      comms::field::adapter::SequenceSerLengthFieldPrefix<
          SequenceSerLengthFieldPrefix, SequenceSerLengthFieldReadErrorStatus,
          TWrappedField>;
};

template <typename TField, comms::ErrorStatus TReadErrorStatus,
          typename... TOptions>
class OptionsParser<comms::option::def::SequenceElemSerLengthFieldPrefix<
                        TField, TReadErrorStatus>,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  static constexpr bool HasSequenceElemSerLengthFieldPrefix = true;
  using SequenceElemSerLengthFieldPrefix = TField;
  static constexpr comms::ErrorStatus
      SequenceElemSerLengthFieldReadErrorStatus = TReadErrorStatus;
  ...
  using AdaptSequenceElemSerLengthFieldPrefix =
      comms::field::adapter::SequenceElemSerLengthFieldPrefix<
          ...
          SequenceElemSerLengthFieldReadErrorStatus, TWrappedField>;
};

template <typename TField, comms::ErrorStatus TReadErrorStatus,
          typename... TOptions>
class OptionsParser<comms::option::def::SequenceElemFixedSerLengthFieldPrefix<
                        TField, TReadErrorStatus>,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  ...
  static constexpr comms::ErrorStatus
      SequenceElemFixedSerLengthFieldReadErrorStatus = TReadErrorStatus;
  ...
  using AdaptSequenceElemFixedSerLengthFieldPrefix =
      comms::field::adapter::SequenceElemFixedSerLengthFieldPrefix<
          ...
          SequenceElemFixedSerLengthFieldReadErrorStatus, TWrappedField>;
};

template <typename TTrailField, typename... TOptions>
class OptionsParser<
    comms::option::def::SequenceTrailingFieldSuffix<TTrailField>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
  ...
  template <typename TField>
  using AdaptSequenceTrailingFieldSuffix =
      comms::field::adapter::SequenceTrailingFieldSuffix<
          SequenceTrailingFieldSuffix, TField>;
};

template <typename TTermField, typename... TOptions>
class OptionsParser<
    comms::option::def::SequenceTerminationFieldSuffix<TTermField>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
  ...
  using AdaptSequenceTerminationFieldSuffix =
      comms::field::adapter::SequenceTerminationFieldSuffix<
          SequenceTerminationFieldSuffix, TField>;
};

template <typename TInitialiser, typename... TOptions>
class OptionsParser<comms::option::def::DefaultValueInitialiser<TInitialiser>,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  ...
  using AdaptDefaultValueInitialiser =
      comms::field::adapter::DefaultValueInitialiser<DefaultValueInitialiser,
                                                     TField>;
};

template <typename TValidator, typename... TOptions>
class OptionsParser<comms::option::def::ContentsValidator<TValidator>,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  ...
  template <typename TField>
  using AdaptCustomValidator =
      comms::field::adapter::CustomValidator<CustomValidator, TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::HasCustomRefresh, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
class OptionsParser<comms::option::def::FailOnInvalid<TStatus>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
  ...
  template <typename TField>
  using AdaptFailOnInvalid =
      comms::field::adapter::FailOnInvalid<FailOnInvalidStatus, TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::IgnoreInvalid, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
template <typename... TOptions>
class OptionsParser<comms::option::def::InvalidByDefault, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
template <std::size_t TSize, typename... TOptions>
class OptionsParser<comms::option::app::FixedSizeStorage<TSize>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
class OptionsParser<comms::option::app::CustomStorageType<TType>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
class OptionsParser<comms::option::def::ScalingRatio<TNum, TDenom>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
class OptionsParser<comms::option::def::Units<TType, TRatio>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
template <typename... TOptions>
class OptionsParser<comms::option::app::OrigDataView, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
template <typename... TOptions>
class OptionsParser<comms::option::def::EmptySerialization, TOptions...>
    : public OptionsParser<TOptions...> {
public:
  static constexpr bool HasEmptySerialization = true;

  template <typename TField>
  using AdaptEmptySerialization =
      comms::field::adapter::EmptySerialization<TField>;
};

template <bool THasMultiRangeValidation> struct MultiRangeAssembler {
  template <typename TBase, typename T, T TMinValue, T TMaxValue>
  using Type = std::tuple<std::tuple<std::integral_constant<T, TMinValue>,
                                     std::integral_constant<T, TMaxValue>>>;
};

template <> struct MultiRangeAssembler<true> {
  ...
  using Type = typename std::decay<decltype(std::tuple_cat(
      ...
      std::declval<typename FalseAssembler::template Type<
          TBase, T, TMinValue, TMaxValue>>()))>::type;
};

template <typename TBase, typename T, T TMinValue, T TMaxValue>
using MultiRangeAssemblerT =
    typename MultiRangeAssembler<TBase::HasMultiRangeValidation>::template Type<
        TBase, T, TMinValue, TMaxValue>;

template <std::intmax_t TMinValue, std::intmax_t TMaxValue,
          typename... TOptions>
class OptionsParser<
    comms::option::def::ValidNumValueRange<TMinValue, TMaxValue>, TOptions...>
    : public OptionsParser<TOptions...> {
...
public:
  ...
  static_assert(!BaseImpl::HasMultiRangeValidation,
                "Sorry gcc-4.7 fails to compile valid C++11 code that allows "
                "multiple usage"
                "of comms::option::def::ValidNumValueRange options. Either use "
                "it only once or"
                "upgrade your compiler.");
#endif
  using MultiRangeValidationRanges =
      MultiRangeAssemblerT<BaseImpl, std::intmax_t, TMinValue, TMaxValue>;
  static constexpr bool HasMultiRangeValidation = true;

  template <typename TField>
  using AdaptMultiRangeValidation =
      comms::field::adapter::NumValueMultiRangeValidator<
          MultiRangeValidationRanges, TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::ValidRangesClear, TOptions...>
    : public OptionsParser<TOptions...> {
  using BaseImpl = OptionsParser<TOptions...>;
...
#endif
  using MultiRangeValidationRanges =
      MultiRangeAssemblerT<BaseImpl, std::uintmax_t, TMinValue, TMaxValue>;
  static constexpr bool HasMultiRangeValidation = true;
...
template <typename... TOptions>
class OptionsParser<comms::option::def::HasCustomVersionUpdate, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
class OptionsParser<comms::option::def::ExistsBetweenVersions<TFrom, TUntil>,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  ...
  using AdaptVersionsRange =
      comms::field::adapter::ExistsBetweenVersions<ExistsFromVersion,
                                                   ExistsUntilVersion, TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::VersionStorage, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
class OptionsParser<comms::option::def::RemLengthMemberField<TIdx>, TOptions...>
    : public OptionsParser<TOptions...> {
  using BaseImpl = OptionsParser<TOptions...>;
  static_assert(
      !BaseImpl::HasRemLengthMemberField,
...
public:
  ...
  using AdaptRemLengthMemberField =
      comms::field::adapter::RemLengthMemberField<RemLengthMemberFieldIdx,
                                                  TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::HasCustomWrite, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
class OptionsParser<comms::option::def::FieldType<TActField>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
template <typename... TOptions>
class OptionsParser<comms::option::def::MissingOnReadFail, TOptions...>
    : public OptionsParser<TOptions...> {
public:
  static constexpr bool HasMissingOnReadFail = true;

  template <typename TField>
  using AdaptMissingOnReadFail =
      comms::field::adapter::MissingOnReadFail<TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::MissingOnInvalid, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
template <typename... TOptions>
class OptionsParser<comms::option::def::VariantHasCustomResetOnDestruct,
                    TOptions...> : public OptionsParser<TOptions...> {
public:
  static constexpr bool HasVariantCustomResetOnDestruct = true;

  template <typename TField> using AdaptVariantResetOnDestruct = TField;
};
...
public:
  ...
  static constexpr MembersVersionDependency ForcedMembersVersionDependency =
      TVersionDependent ? MembersVersionDependency_Dependent
                        : MembersVersionDependency_Independent;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::FixedValue, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
template <std::intmax_t TOffset, typename... TOptions>
class OptionsParser<comms::option::def::DisplayOffset<TOffset>, TOptions...>
    : public OptionsParser<TOptions...> {
public:
  ...
  template <typename TField>
  using AdaptDisplayOffset =
      comms::field::adapter::DisplayOffset<TOffset, TField>;
};

template <typename... TOptions>
class OptionsParser<comms::option::def::HasName, TOptions...>
    : public OptionsParser<TOptions...> {
public:
...
template <typename... TOptions>
class OptionsParser<comms::option::app::EmptyOption, TOptions...>
    : public OptionsParser<TOptions...> {};

template <typename... TTupleOptions, typename... TOptions>
class OptionsParser<std::tuple<TTupleOptions...>, TOptions...>

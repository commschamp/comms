...
///     valid().
///
///     Refer to @ref sec_field_tutorial_variant for tutorial and usage
///     examples.
...
///     @li @ref comms::option::def::DefaultValueInitialiser - All wrapped
///     fields may
...
///         internal logic.
///     @li @ref comms::option::def::DefaultVariantIndex - By default the
///     Variant field
///         doesn't have any valid contents. This option may be used to specify
///         the index of the default member field.
///     @li @ref comms::option::def::EmptySerialization - Force empty
///     serialization.
///     @li @ref comms::option::def::FieldType - Set actual field type
///     @li @ref comms::option::def::FixedValue
///     @li @ref comms::option::def::HasCustomRead - Mark field to have custom
///     read
///         functionality
///     @li @ref comms::option::def::HasCustomRefresh - Mark field to have
///     custom
///         refresh functionality.
///     @li @ref comms::option::def::HasName
///     @li @ref comms::option::def::HasVersionDependentMembers
///     @li @ref comms::option::def::VariantHasCustomResetOnDestruct - avoid
///     calling
///         default @ref comms::field::Variant::reset() "reset()" on
///         destruction, assume it is called by the extending class destructor.
...
class Variant
    : public details::AdaptBasicFieldT<
          basic::Variant<TFieldBase,
                         details::OptionsParser<
                             TOptions...>::ForcedMembersVersionDependency,
                         TMembers>,
          TOptions...> {
  using BaseImpl = details::AdaptBasicFieldT<
      basic::Variant<
          TFieldBase,
...
public:
  ...
  /// @brief Constructor
  explicit Variant(const ValueType &val) : BaseImpl(val) {}

  /// @brief Constructor
  explicit Variant(ValueType &&val) : BaseImpl(std::move(val)) {}
  ...
  /// @brief Get minimal length that is required to serialise all possible
  /// contained fields.
  /// @return Always returns 0.
  static constexpr std::size_t minLength() { return BaseImpl::minLength(); }

  /// @brief Get maximal length that is required to serialise all possible
  /// contained fields.
  ...
  /// @brief Read operation without error check and status report is not
  /// supported.
  ...
  ///     function doesn't advance the iterator, but returns
  ...
  std::size_t currentField() const { return BaseImpl::currentField(); }
  ...
  void selectField(std::size_t idx) { BaseImpl::selectField(idx); }

  /// @brief Execute provided function object with current field as
  ///     parameter.
  /// @details The provided function object must define all the public @b
  /// operator()
  ...
  ///     be cleared via explicit @ref comms::field::Variant::deinitField()
  ///     "deinitField()" or @ref comms::field::Variant::reset() "reset()"
  ///     calls.
  ...
  /// @brief Destruct previously initialised (via @ref
  /// comms::field::Variant::initField() "initField()")
  ///     contained field.
  /// @tparam TIdx Index of the field type witin the @ref Members tuple.
  ...
  /// @brief Access already constructed field at specifed index (known at
  /// compile time).
  ...
  /// @brief Access already constructed field at specifed index (known at
  /// compile time).
  /// @details Use this function to get a const reference to the contained field
  /// type.
  /// @tparam TIdx Index of the field type witin the @ref Members tuple.
...
private:
  ...
  static_assert(!ParsedOptions::HasSequenceElemLengthForcing,
                ...
                "is not applicable to Variant field");
  ...
  static_assert(!ParsedOptions::HasSequenceLengthForcing,
                ...
                "not applicable to Variant field");
  ...
  static_assert(!ParsedOptions::HasSequenceFixedSizeUseFixedSizeStorage,
                ...
                "option is not applicable to Variant field");
...
template <typename TVar> class VariantEqualityCompHelper {
public:
  VariantEqualityCompHelper(const TVar &other, bool &result)
      : m_other(other), m_result(result) {}
...
VariantEqualityCompHelper<TVar> makeVariantEqualityCompHelper(TVar &other,
                                                              bool &result) {
  return VariantEqualityCompHelper<TVar>(other, result);
}

template <typename TVar> class VariantLessCompHelper {
public:
...
template <typename TVar>
VariantLessCompHelper<TVar> makeVariantLessCompHelper(TVar &other,
                                                      bool &result) {
...
bool operator==(const Variant<TFieldBase, TMembers, TOptions...> &field1,
  ...
  bool result = false;
  field1.currentFieldExec(
      details::makeVariantEqualityCompHelper(field2, result));
  return result;
}
...
///     does @b NOT define inner @b Field_* types for used member fields.
...
#define COMMS_VARIANT_MEMBERS_ACCESS(...)                                      \
  ...
  COMMS_AS_VARIANT_FUNC {                                                      \
    ...
    static_assert(std::tuple_size<typename Var::Members>::value ==             \
...
///     COMMS_VARIANT_MEMBERS_ACCESS_NOTEMPLATE() instead.
/// @related comms::field::Variant
...
///
///     MyField field;
///     auto& field1 = field.initField<0>(); // Initialise the field to contain
///     Field1 value field1.value() = ...;
///     @endcode
///     However, it would be convenient to provide names and easier access to
///     all the poisble variants. The #COMMS_VARIANT_MEMBERS_NAMES() macro does
///     exactly that when used inside the field class definition. Just inherit
///     from the @ref comms::field::Variant class and use the macro inside with
///     the names for the member fields:
///     @code
...
///         {
///             rerturn
///             initField<FieldIdx_member1>(std::forward<TArgs>(args)...);
///         }
///
...
///         }
///
///         // Const variant of the accessor to the stored field as first member
///         (Field1) const Field1& accessField_member1() const
///         {
///             return accessField<FieldIdx_member1>();
///         }
///
...
///         {
///             rerturn
///             initField<FieldIdx_member2>(std::forward<TArgs>(args)...);
///         }
///
...
///         }
///
///         // Const variant of the accessor to the stored field as second
///         member (Field2) const Field2& accessField_member2() const
///         {
///             return accessField<FieldIdx_member2>();
///         }
///
...
///         {
///             rerturn
///             initField<FieldIdx_member3>(std::forward<TArgs>(args)...);
///         }
///
...
///         }
///
///         // Const variant of the accessor to the stored field as third member
///         (Field3) const Field3& accessField_member3() const
///         {
///             return accessField<FieldIdx_member3>();
///         }
///
...
///     };
///     @endcode
///     @b NOTE, that provided names @b member1, @b member2, and @b member3,
///     have
...
#define COMMS_VARIANT_MEMBERS_NAMES(...)                                       \
  ...
  COMMS_EXPAND(COMMS_DO_FIELD_TYPEDEF(typename Base::Members, Field_,          \

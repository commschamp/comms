...
template <typename TBase> class VariantResetOnDestruct : public TBase {
  using BaseImpl = TBase;
  static_assert(std::is_same<typename BaseImpl::CommsTag,
                             comms::field::tag::Variant>::value,
                "Applicable only to variant fields");

public:
  ~VariantResetOnDestruct() { BaseImpl::reset(); }

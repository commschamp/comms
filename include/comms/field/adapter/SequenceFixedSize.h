...
template <typename TBase> class SequenceFixedSizeBase : public TBase {
...
public:
  ...
  explicit SequenceFixedSizeBase(std::size_t maxSize) : m_fixedSize(maxSize) {}

  SequenceFixedSizeBase(std::size_t maxSize, const ValueType &val)
      : BaseImpl(val), m_fixedSize(maxSize) {}

  SequenceFixedSizeBase(std::size_t maxSize, ValueType &&val)
      : BaseImpl(std::move(val)), m_fixedSize(maxSize) {}
  ...
  std::size_t length() const {
    ...
    if (currSize < m_fixedSize) {
      ...
      return BaseImpl::length() +
             (remSize * BaseImpl::elementLength(dummyElem));
    }

    using Tag = typename comms::util::LazyShallowConditional<
        std::is_integral<ElementType>::value &&
        (sizeof(ElementType) ==
         sizeof(std::uint8_t))>::template Type<HasRawDataTag, HasFieldsTag>;
  ...
  bool refresh() {
    ...
    using Tag = typename comms::util::LazyShallowConditional<
        ...
        template Type<HasResizeTag, NoResizeTag>;
...
private:
  ...
  std::size_t recalcLen(HasFieldsTag<TParams...>) const {
    using Tag =
        typename comms::util::LazyShallowConditional<ElementType::minLength() ==
                                                     ElementType::maxLength()>::
            template Type<HasFixedLengthElemsTag, HasVarLengthElemsTag>;
    return recalcLen(Tag());
  }
  ...
  template <typename... TParams> bool doRefresh(HasResizeTag<TParams...>) {
...
public:
  ...
  explicit SequenceFixedSize() : BaseImpl(TSize) {}

  explicit SequenceFixedSize(const ValueType &val) : BaseImpl(TSize, val) {}

  SequenceFixedSize(ValueType &&val) : BaseImpl(TSize, std::move(val)) {}

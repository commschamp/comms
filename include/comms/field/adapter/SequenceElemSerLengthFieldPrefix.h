...
public:
  ...
  SequenceElemSerLengthFieldPrefix(const SequenceElemSerLengthFieldPrefix &) =
      default;
  SequenceElemSerLengthFieldPrefix(SequenceElemSerLengthFieldPrefix &&) =
      default;
  SequenceElemSerLengthFieldPrefix &
  operator=(const SequenceElemSerLengthFieldPrefix &) = default;
  SequenceElemSerLengthFieldPrefix &
  operator=(SequenceElemSerLengthFieldPrefix &&) = default;

  std::size_t length() const {
    using ElemLengthTag = typename comms::util::LazyShallowConditional<
        BaseImpl::minElementLength() == BaseImpl::maxElementLength()>::
        template Type<FixedLengthElemTag, VarLengthElemTag>;
  ...
  static constexpr std::size_t minLength() { return LenField::minLength(); }
  ...
  ErrorStatus readElement(ElementType &elem, TIter &iter,
  ...
  template <typename TIter>
  ErrorStatus writeElement(const ElementType &elem, TIter &iter,
                           std::size_t &len) const {
  ...
  template <typename TIter>
  static void writeElementNoStatus(const ElementType &elem,
                                   TIter &iter) = delete;

  bool canWrite() const { return basic::CommonFuncs::canWriteSequence(*this); }
...
private:
  ...
  using LenFieldLengthTag =
      typename comms::util::LazyShallowConditional<LenField::minLength() ==
                                                   LenField::maxLength()>::
          template Type<FixedLengthLenFieldTag, VarLengthLenFieldTag>;

  template <typename... TParams>
  std::size_t lengthInternal(FixedLengthLenFieldTag<TParams...>,
                             FixedLengthElemTag<TParams...>) const {
    return (LenField::minLength() + BaseImpl::minElementLength()) *
           BaseImpl::getValue().size();
  }

  template <typename... TParams>
  std::size_t lengthInternal(FixedLengthLenFieldTag<TParams...>,
                             VarLengthElemTag<TParams...>) const {
    return lengthInternalIterative();
  }

  template <typename... TParams>
  std::size_t lengthInternal(VarLengthLenFieldTag<TParams...>,
                             FixedLengthElemTag<TParams...>) const {
  ...
  template <typename... TParams>
  std::size_t elementLengthInternal(const ElementType &elem,
                                    FixedLengthLenFieldTag<TParams...>) const {
    return LenField::minLength() + BaseImpl::elementLength(elem);
  }

  template <typename... TParams>
  std::size_t elementLengthInternal(const ElementType &elem,
                                    VarLengthLenFieldTag<TParams...>) const {
    LenField lenField;
    auto origElemLength = BaseImpl::elementLength(elem);
    auto elemLength =
        std::min(origElemLength, std::size_t(MaxAllowedElemLength));
    lenField.setValue(elemLength);

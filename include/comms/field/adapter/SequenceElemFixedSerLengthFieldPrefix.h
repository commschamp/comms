...
public:
  ...
  SequenceElemFixedSerLengthFieldPrefix(
      const SequenceElemFixedSerLengthFieldPrefix &) = default;
  SequenceElemFixedSerLengthFieldPrefix(
      SequenceElemFixedSerLengthFieldPrefix &&) = default;
  SequenceElemFixedSerLengthFieldPrefix &
  operator=(const SequenceElemFixedSerLengthFieldPrefix &) = default;
  SequenceElemFixedSerLengthFieldPrefix &
  operator=(SequenceElemFixedSerLengthFieldPrefix &&) = default;
  ...
  bool canWrite() const {
    ...
    return BaseImpl::canWrite() &&
           canWriteElement(BaseImpl::getValue().front());
  }
...
private:
  ...
  std::size_t lengthInternal(VarLengthLenFieldTag<TParams...>) const {
    ...
    if (!BaseImpl::getValue().empty()) {
      LenField lenField;
      lenField.setValue(std::min(BaseImpl::minElementLength(),
                                 std::size_t(MaxAllowedElemLength)));
  ...
  template <typename TIter> ErrorStatus readLen(TIter &iter, std::size_t &len) {
    auto fromIter = iter;
    LenField lenField;
  ...
  template <typename TIter> void writeLenNoStatus(TIter &iter) const {
    auto elemLength = BaseImpl::minElementLength();
    LenField lenField;
    lenField.setValue(elemLength);
  ...
  static_assert(BaseImpl::minElementLength() == BaseImpl::maxElementLength(),
                "Option SequenceElemFixedSerLengthFieldPrefix can be used only "
                "with fixed length "
                "elements.");

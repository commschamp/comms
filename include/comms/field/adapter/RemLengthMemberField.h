...
public:
  ...
  using LengthFieldType =
      typename std::tuple_element<TLenFieldIdx, ValueType>::type;
  using VersionType = typename BaseImpl::VersionType;

  RemLengthMemberField() { refreshLengthInternal(); }

  static constexpr std::size_t maxLength() { return MaxPossibleLen; }

  template <std::size_t TFromIdx> static constexpr std::size_t maxLengthFrom() {
    using Tag =
        typename comms::util::LazyShallowConditional <
        TLenFieldIdx<TFromIdx>::template Type<BaseRedirectTag, LocalTag>;
  ...
  static constexpr std::size_t maxLengthUntil() {
    using Tag = typename comms::util::LazyShallowConditional<
        TUntilIdx <= TLenFieldIdx>::template Type<BaseRedirectTag, LocalTag>;
  ...
  template <typename TIter> ErrorStatus read(TIter &iter, std::size_t len) {
    return readFromUntilAndUpdateLen<0, std::tuple_size<ValueType>::value>(iter,
                                                                           len);
  }

  template <std::size_t TFromIdx, typename TIter>
  ErrorStatus readFrom(TIter &iter, std::size_t len) {
    return readFromUntilAndUpdateLen<TFromIdx,
                                     std::tuple_size<ValueType>::value>(iter,
                                                                        len);
  }

  template <std::size_t TFromIdx, typename TIter>
  ErrorStatus readFromAndUpdateLen(TIter &iter, std::size_t &len) {
    return readFromAndUpdateLen<TFromIdx, std::tuple_size<ValueType>::value>(
        iter, len);
  }
...
private:
  ...
  template <typename... TParams> using LocalTag = comms::details::tag::Tag2<>;

  template <typename... TParams>
  using PerformOpTag = comms::details::tag::Tag3<>;

  template <typename... TParams> using SkipOpTag = comms::details::tag::Tag4<>;

  template <std::size_t TFromIdx, typename... TParams>
  static constexpr std::size_t
  maxLengthFromInternal(BaseRedirectTag<TParams...>) {
  ...
  static constexpr std::size_t
  maxLengthUntilInternal(BaseRedirectTag<TParams...>) {
  ...
  static constexpr std::size_t
  maxLengthFromUntilInternal(BaseRedirectTag<TParams...>) {
    return BaseImpl::template maxLengthFromUntil<TFromIdx, TUntilIdx>();
  }

  template <std::size_t TFromIdx, std::size_t TUntilIdx, typename... TParams>
  static constexpr std::size_t
  maxLengthFromUntilInternal(LocalTag<TParams...>) {
    return MaxPossibleLen;
  }

  template <std::size_t TFromIdx, std::size_t TUntilIdx, typename TIter,
            typename... TParams>
  ErrorStatus readFromUntilInternal(TIter &iter, std::size_t &len,
                                    BaseRedirectTag<TParams...>) {
  ...
  ErrorStatus readEarlierFieldsInternal(TIter &iter, std::size_t &len,
                                        PerformOpTag<TParams...>) {
    return BaseImpl::template readFromUntilAndUpdateLen<TFromIdx, TLenFieldIdx>(
        iter, len);
  }
  ...
  ErrorStatus readRemLengthFieldInternal(TIter &iter, std::size_t &len,
                                         std::size_t &remLen,
                                         PerformOpTag<TParams...>) {
    ...
    auto lenFieldLen =
        static_cast<std::size_t>(std::distance(beforeLenReadIter, iter));
  ...
  void skipUntilFieldInternal(std::size_t &reqLen, PerformOpTag<TParams...>) {
    ...
    auto fieldsLen =
        BaseImpl::template lengthFromUntil<TLenFieldIdx + 1, TUntilIdx>();
  ...
  ErrorStatus readFromUntilInternal(TIter &iter, std::size_t &len,
    ...
    using EarlierFieldsTag = typename comms::util::LazyShallowConditional<(
        TFromIdx < TLenFieldIdx)>::template Type<PerformOpTag, SkipOpTag>;

    auto es =
        readEarlierFieldsInternal<TFromIdx>(iter, len, EarlierFieldsTag());
    if (es != comms::ErrorStatus::Success) {
      return es;
    }

    using LenTag = typename comms::util::LazyShallowConditional<(
        TFromIdx <= TLenFieldIdx)>::template Type<PerformOpTag, SkipOpTag>;
    ...
    using SkipTag = typename comms::util::LazyShallowConditional<(
        TLenFieldIdx < TFromIdx)>::template Type<PerformOpTag, SkipOpTag>;

    skipUntilFieldInternal<TFromIdx>(reqLen, SkipTag());

    static const std::size_t NextIdx =
        (TFromIdx <= TLenFieldIdx) ? TLenFieldIdx + 1 : TFromIdx;

    if ((std::tuple_size<ValueType>::value <= TUntilIdx) && (len < reqLen)) {
      return comms::ErrorStatus::NotEnoughData;
    }

    auto remLen = std::min(len, reqLen);
    es = BaseImpl::template readFromUntilAndUpdateLen<NextIdx, TUntilIdx>(
        iter, remLen);

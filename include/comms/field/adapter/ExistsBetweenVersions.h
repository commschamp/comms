...
public:
  ...
  explicit ExistsBetweenVersions(const ValueType &val) : BaseImpl(val) {}

  explicit ExistsBetweenVersions(ValueType &&val) : BaseImpl(std::move(val)) {}
  ...
  static constexpr bool isVersionDependent() { return true; }
...
private:
  ...
  template <typename... TParams> using CompareTag = comms::details::tag::Tag2<>;

  static bool aboveFrom(VersionType version) {
    using Tag = typename comms::util::LazyShallowConditional<
        TFrom == 0>::template Type<AlwaysTrueTag, CompareTag>;
  ...
  static bool aboveFrom(VersionType version, CompareTag<TParams...>) {
    static const VersionType MinVersion = static_cast<VersionType>(std::min(
  ...
  static bool belowUntil(VersionType version) {
    using Tag = typename comms::util::LazyShallowConditional<
        static_cast<decltype(TUntil)>(
            std::numeric_limits<VersionType>::max()) <=
        TUntil>::template Type<AlwaysTrueTag, CompareTag>;
  ...
  static bool belowUntil(VersionType version, CompareTag<TParams...>) {
    static const VersionType MaxVersion = static_cast<VersionType>(std::min(

...
class FixedBitLength : public TBase {
  ...
  using BaseUnsignedSerialisedType =
...
public:
  ...
  using SerialisedType = typename comms::util::
      Conditional<(Length < sizeof(BaseSerialisedType))>::template Type<
          typename comms::util::SizeToType<
              Length, std::is_signed<BaseSerialisedType>::value>::Type,
          BaseSerialisedType>;
  ...
  explicit FixedBitLength(const ValueType &val) : BaseImpl(val) {}
...
private:
  ...
  using HasSignTag = typename comms::util::LazyShallowConditional<
      ...
      template Type<SignedTag, UnsignedTag>;
  ...
  static SerialisedType adjustToSerialised(BaseSerialisedType val,
                                           UnsignedTag<TParams...>) {
    return static_cast<SerialisedType>(
  ...
  static SerialisedType adjustToSerialised(BaseSerialisedType val,
                                           SignedTag<TParams...>) {
    auto valueTmp = static_cast<UnsignedSerialisedType>(
        static_cast<BaseUnsignedSerialisedType>(val) & UnsignedValueMask);
  ...
  static BaseSerialisedType adjustFromSerialised(SerialisedType val,
    ...
    auto valueTmp = static_cast<UnsignedSerialisedType>(
        static_cast<UnsignedSerialisedType>(val) & UnsignedValueMask);
    return static_cast<BaseSerialisedType>(signExtUnsignedSerialised(valueTmp));
  }

  static SerialisedType signExtUnsignedSerialised(UnsignedSerialisedType val) {
    using SignExtTag =
        typename comms::util::LazyShallowConditional <
        BitLength<static_cast<std::size_t>(
            std::numeric_limits<UnsignedSerialisedType>::digits)>::
            template Type<MustSignExtTag, NoSignExtTag>;
    return signExtUnsignedSerialisedInternal(val, SignExtTag());
  }

  template <typename... TParams>
  static SerialisedType
  signExtUnsignedSerialisedInternal(UnsignedSerialisedType val,
                                    MustSignExtTag<TParams...>) {
    static_assert(BitLength <
                      std::numeric_limits<UnsignedSerialisedType>::digits,
                  "BitLength is expected to be less than number of bits in the "
                  "value type");

    static const auto SignExtMask = static_cast<UnsignedSerialisedType>(
        ~((static_cast<UnsignedSerialisedType>(1U) << BitLength) - 1));

    static const auto SignMask = static_cast<UnsignedSerialisedType>(
        static_cast<UnsignedSerialisedType>(1U) << (BitLength - 1));

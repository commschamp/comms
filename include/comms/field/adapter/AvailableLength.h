...
template <typename TBase> class AvailableLength : public TBase {
...
public:
  ...
  explicit AvailableLength(const ValueType &val) : BaseImpl(val) {}

  explicit AvailableLength(ValueType &&val) : BaseImpl(std::move(val)) {}
  ...
  void setForcedLength(int len) { m_forcedLength = len; }

  int getForcedLength() const { return m_forcedLength; }
  ...
  std::size_t length() const {
    ...
    if (0 < m_forcedLength) {
      return std::min(BaseImpl::length(),
                      static_cast<std::size_t>(m_forcedLength));
    }
  ...
  static constexpr std::size_t minLength() { return 1U; }
  ...
  comms::ErrorStatus read(TIter &iter, std::size_t size) {
    ...
    auto fromIter = iter;
    auto unsignedSerialized = util::readData<UnsignedSerialisedType>(
        iter, std::min(size, BaseImpl::maxLength()), Endian());
    auto len = static_cast<std::size_t>(std::distance(fromIter, iter));
    BaseImpl::setValue(fromSerialised(
        signExtUnsignedSerialised(unsignedSerialized, len, HasSignTag())));
...
private:
  ...
  template <typename... TParams>
  static SerialisedType getMinLimitedValue(std::size_t len,
                                           UnsignedTag<TParams...>) {
  ...
  static SerialisedType getMinLimitedValue(std::size_t len,
    ...
    auto mask =
        ((static_cast<UnsignedSerialisedType>(1U) << ((len * BitsInByte) - 1)) -
         1U);
  ...
  template <typename... TParams>
  static SerialisedType getMaxLimitedValue(std::size_t len,
                                           UnsignedTag<TParams...>) {
    COMMS_ASSERT(len < sizeof(UnsignedSerialisedType));
    auto value =

...
template <std::size_t TLen> struct UnsignedValueMaskWrap {
  static const std::uintmax_t Value = static_cast<std::uintmax_t>(
      (static_cast<std::uintmax_t>(1U)
       << (TLen * std::numeric_limits<std::uint8_t>::digits)) -
      1U);
};

template <> struct UnsignedValueMaskWrap<sizeof(std::uintmax_t)> {
...
public:
  ...
  using SerialisedType = typename comms::util::Conditional<(
      TLen < sizeof(BaseSerialisedType))>::
      template Type<typename comms::util::SizeToType<
                        TLen, std::is_signed<BaseSerialisedType>::value>::Type,
                    BaseSerialisedType>;
  ...
  explicit FixedLength(const ValueType &val) : BaseImpl(val) {}
  ...
  static constexpr ValueType fromSerialised(SerialisedType val) {
    return BaseImpl::fromSerialised(
        adjustFromSerialised(val, ConversionTag<>()));
  }
  ...
  template <typename TIter> void writeNoStatus(TIter &iter) const {
    BaseImpl::template writeData<Length>(toSerialised(BaseImpl::getValue()),
                                         iter);
  }
...
private:
  ...
  template <typename... TParams> using SignedTag = comms::details::tag::Tag4<>;

  template <typename...>
  using ConversionTag = typename comms::util::LazyShallowConditional<(
      TLen < sizeof(SerialisedType))>::template Type<SignExtendTag,
                                                     JustCastTag>;

  template <typename...>
  using HasSignTag = typename comms::util::LazyShallowConditional<
      std::is_signed<SerialisedType>::value &&
      TSignExtend>::template Type<SignedTag, UnsignedTag>;

  using UnsignedSerialisedType =
      typename std::make_unsigned<SerialisedType>::type;

  template <typename... TParams>
  static constexpr SerialisedType adjustToSerialised(BaseSerialisedType val,
                                                     JustCastTag<TParams...>) {
    return static_cast<SerialisedType>(val);
  }

  template <typename... TParams>
  static SerialisedType adjustToSerialised(BaseSerialisedType val,
                                           SignExtendTag<TParams...>) {
    auto valueTmp =
  ...
  template <typename... TParams>
  static constexpr BaseSerialisedType
  adjustFromSerialised(SerialisedType val, JustCastTag<TParams...>) {
    return castToBaseSerializedType(val, HasSignTag<>());
  }

  template <typename... TParams>
  static BaseSerialisedType adjustFromSerialised(SerialisedType val,
                                                 SignExtendTag<TParams...>) {
    auto valueTmp =
        static_cast<UnsignedSerialisedType>(val) & UnsignedValueMask;
    return castToBaseSerializedType(
        signExtUnsignedSerialised(valueTmp, HasSignTag<>()), HasSignTag<>());
  }

  template <typename... TParams>
  static constexpr SerialisedType
  signExtUnsignedSerialised(UnsignedSerialisedType val,
                            UnsignedTag<TParams...>) {
    return static_cast<SerialisedType>(val);
  }
  ...
  template <typename... TParams>
  static constexpr BaseSerialisedType
  castToBaseSerializedType(SerialisedType val, UnsignedTag<TParams...>) {
    return static_cast<BaseSerialisedType>(
        static_cast<UnsignedSerialisedType>(val));
  }

  template <typename... TParams>
  static constexpr BaseSerialisedType
  castToBaseSerializedType(SerialisedType val, SignedTag<TParams...>) {

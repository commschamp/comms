...
template <typename TFieldBase,
          comms::field::details::MembersVersionDependency TVersionDependency,
          typename... TMembers>
class Bundle<TFieldBase, TVersionDependency, std::tuple<TMembers...>>
    : public TFieldBase {
public:
  ...
  Bundle() = default;
  explicit Bundle(const ValueType &val) : m_members(val) {}

  explicit Bundle(ValueType &&val) : m_members(std::move(val)) {}
  ...
  const ValueType &value() const { return m_members; }

  ValueType &value() { return m_members; }
  ...
  template <std::size_t TFromIdx> constexpr std::size_t lengthFrom() const {
    return comms::util::tupleAccumulateFromUntil<
        TFromIdx, std::tuple_size<ValueType>::value>(
        value(), std::size_t(0),
        comms::field::details::FieldLengthSumCalcHelper<>());
  }

  template <std::size_t TUntilIdx> constexpr std::size_t lengthUntil() const {
    return comms::util::tupleAccumulateFromUntil<0, TUntilIdx>(
        value(), std::size_t(0),
        comms::field::details::FieldLengthSumCalcHelper<>());
  }
  ...
  constexpr std::size_t lengthFromUntil() const {
    return comms::util::tupleAccumulateFromUntil<TFromIdx, TUntilIdx>(
        value(), std::size_t(0),
        comms::field::details::FieldLengthSumCalcHelper<>());
  }
  ...
  template <std::size_t TFromIdx> static constexpr std::size_t minLengthFrom() {
    return comms::util::tupleTypeAccumulateFromUntil<
        TFromIdx, std::tuple_size<ValueType>::value, Members>(
        std::size_t(0), comms::field::details::FieldMinLengthSumCalcHelper<>());
  }

  template <std::size_t TUntilIdx>
  static constexpr std::size_t minLengthUntil() {
    return comms::util::tupleTypeAccumulateFromUntil<0, TUntilIdx, Members>(
        std::size_t(0), comms::field::details::FieldMinLengthSumCalcHelper<>());
  }
  ...
  constexpr bool valid() const {
    return comms::util::tupleAccumulate(
        value(), true, comms::field::details::FieldValidCheckHelper<>());
  }

  bool refresh() {
    return comms::util::tupleAccumulate(
        value(), false, comms::field::details::FieldRefreshHelper<>());
  }
  ...
  ErrorStatus readFromAndUpdateLen(TIter &iter, std::size_t &len) {
    auto es = ErrorStatus::Success;
    comms::util::template tupleForEachFrom<TFromIdx>(
        value(), makeReadHelper(es, iter, len));
    return es;
  }
  ...
  ErrorStatus readUntilAndUpdateLen(TIter &iter, std::size_t &len) {
    auto es = ErrorStatus::Success;
    comms::util::template tupleForEachUntil<TUntilIdx>(
        value(), makeReadHelper(es, iter, len));
    return es;
  }
  ...
  ErrorStatus readFromUntilAndUpdateLen(TIter &iter, std::size_t &len) {
    auto es = ErrorStatus::Success;
    comms::util::template tupleForEachFromUntil<TFromIdx, TUntilIdx>(
        value(), makeReadHelper(es, iter, len));
    return es;
  }
  ...
  void readFromNoStatus(TIter &iter) {
    comms::util::template tupleForEachFrom<TFromIdx>(
        value(), makeReadNoStatusHelper(iter));
  }
  ...
  ErrorStatus writeFrom(TIter &iter, std::size_t len) const {
    auto es = ErrorStatus::Success;
    comms::util::template tupleForEachFrom<TFromIdx>(
        value(), makeWriteHelper(es, iter, len));
    return es;
  }
  ...
  void writeFromNoStatus(TIter &iter) const {
    comms::util::template tupleForEachFrom<TFromIdx>(
        value(), makeWriteNoStatusHelper(iter));
  }
  ...
  static constexpr bool isVersionDependent() {
    return details::BundleVersionDependencyDetectHelper<TVersionDependency,
                                                        TMembers...>::Value;
  }

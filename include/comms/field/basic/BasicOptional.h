...
public:
    ...
    explicit BasicOptional(const Field& fieldSrc, Mode mode = Mode::Tentative)
      : m_field(fieldSrc),
        m_mode(mode)
    {
    }

    explicit BasicOptional(Field&& fieldSrc, Mode mode = Mode::Tentative)
      : m_field(std::move(fieldSrc)),
        m_mode(mode)
    {
    }

    BasicOptional(const BasicOptional&) = default;

    BasicOptional(BasicOptional&&) = default;

    ~BasicOptional() noexcept = default;

    BasicOptional& operator=(const BasicOptional&) = default;

    BasicOptional& operator=(BasicOptional&&) = default;

...
template <typename TSerializedType> class BitfieldReadHelper {
public:
...
template <typename TSerializedType> class BitfieldReadNoStatusHelper {
public:
  BitfieldReadNoStatusHelper(TSerializedType val) : m_value(val) {}
...
template <typename TSerializedType> class BitfieldWriteHelper {
public:
  ...
  void operator()(TFieldParam &&field) {
    ...
    using FieldEndian = typename FieldType::Endian;
    const auto *readIter = &buf[0];
    auto fieldSerValue = comms::util::readData<TSerializedType, MaxLength>(
    ...
    static const auto ClearMask =
        static_cast<TSerializedType>(~(Mask << m_pos));

    auto valueMask = static_cast<TSerializedType>(
...
template <typename TSerializedType> class BitfieldWriteNoStatusHelper {
public:
  BitfieldWriteNoStatusHelper(TSerializedType &val) : m_value(val) {}
...
template <typename TFieldBase,
          comms::field::details::MembersVersionDependency TVersionDependency,
          typename... TMembers>
class Bitfield<TFieldBase, TVersionDependency, std::tuple<TMembers...>>
    : public TFieldBase {
  using BaseImpl = TFieldBase;
  using Members = std::tuple<TMembers...>;

  static_assert(1U < std::tuple_size<Members>::value,
                "Number of members is expected to be at least 2.");

  static const std::size_t TotalBits =
      CommonFuncs::FieldSumTotalBitLengthIntType<TMembers...>::value;
  static_assert((TotalBits % std::numeric_limits<std::uint8_t>::digits) == 0,
                "Wrong number of total bits");

  static const std::size_t Length =
      TotalBits / std::numeric_limits<std::uint8_t>::digits;
  static_assert(0U < Length,
  ...
  using FixedIntValueField =
      comms::field::IntValue<TFieldBase, SerializedType,
  ...
  using IntValueField =
      typename comms::util::Conditional<((Length & (Length - 1)) == 0)>::
          template Type<SimpleIntValueField, FixedIntValueField>;
...
public:
  ...
  Bitfield() = default;
  explicit Bitfield(const ValueType &val) : m_members(val) {}

  explicit Bitfield(ValueType &&val) : m_members(std::move(val)) {}
  ...
  static constexpr std::size_t length() { return Length; }

  static constexpr std::size_t minLength() { return length(); }

  static constexpr std::size_t maxLength() { return length(); }
  ...
  template <typename TIter> void readNoStatus(TIter &iter) {
    ...
    comms::util::tupleForEachWithTemplateParamIdx(
        m_members,
        details::BitfieldReadNoStatusHelper<SerializedType>(serValue));
  }
  ...
  template <typename TIter> void writeNoStatus(TIter &iter) const {
    ...
    comms::util::tupleForEachWithTemplateParamIdx(
        m_members,
        details::BitfieldWriteNoStatusHelper<SerializedType>(serValue));
    comms::util::writeData<Length>(serValue, iter, Endian());
  }
  ...
  template <std::size_t TIdx> static constexpr std::size_t memberBitLength() {
    static_assert(TIdx < std::tuple_size<ValueType>::value,
                  "Index exceeds number of fields");

    using FieldType = typename std::tuple_element<TIdx, ValueType>::type;
    return comms::util::FieldBitLengthIntType<>::template Type<
        FieldType>::value;
  }

  static constexpr bool isVersionDependent() {
    return details::BitfieldVersionDependencyDetectHelper<TVersionDependency,
                                                          TMembers...>::Value;
  }

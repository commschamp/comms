...
public:
  ...
  explicit Optional(const Field &fieldSrc, Mode mode = Mode::Tentative)
      : m_field(fieldSrc), m_mode(mode) {}

  explicit Optional(Field &&fieldSrc, Mode mode = Mode::Tentative)
      : m_field(std::move(fieldSrc)), m_mode(mode) {}
  ...
  ValueType &value() { return field(); }

  const ValueType &value() const { return field(); }
  ...
  static constexpr std::size_t maxLength() { return Field::maxLength(); }
  ...
  static constexpr bool hasWriteNoStatus() { return Field::hasWriteNoStatus(); }
  ...
  bool setVersion(VersionType version) {
    return m_field.setVersion(
        static_cast<typename Field::VersionType>(version));
  }

...
public:
  ...
  using SerialisedType =
      typename comms::util::SizeToType<sizeof(ValueType), false>::Type;
  using ScalingRatio = std::ratio<1, 1>;
  using CommsTag = comms::field::tag::Float;

  FloatValue() = default;

  explicit FloatValue(ValueType val) : m_value(val) {}
  ...
  static constexpr std::size_t length() { return sizeof(SerialisedType); }
  ...
  template <typename TIter> void readNoStatus(TIter &iter) {
    auto serialisedValue = BaseImpl::template readData<SerialisedType>(iter);
...
private:
  template <typename...> union CastUnion {
    ValueType m_value;

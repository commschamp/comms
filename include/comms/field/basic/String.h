...
template <typename TStorage> struct StringMaxLengthRetrieveHelper {
...
public:
  ...
  static_assert(sizeof(ElementType) == sizeof(char),
                "Single byte charactes only supported");

  String() = default;

  explicit String(const ValueType &val) : m_value(val) {}

  explicit String(ValueType &&val) : m_value(std::move(val)) {}
  ...
  static constexpr std::size_t maxLength() {
    return details::StringMaxLengthRetrieveHelper<TStorage>::Value *
           sizeof(ElementType);
  }
  ...
  static constexpr std::size_t maxElementLength() { return minElementLength(); }
...
private:
  ...
  static void doAdvance(TIter &iter, std::size_t len) {
    ...
    using Tag = typename comms::util::LazyShallowConditional<
        InputIter>::template Type<AdvancableTag, NotAdvancableTag>;
    doAdvance(iter, len, Tag());
  }

  template <typename TIter, typename... TParams>
  static void doAdvance(TIter &iter, std::size_t len,
                        AdvancableTag<TParams...>) {
    std::advance(iter, len);
  }

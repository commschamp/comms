...
template <typename TStorage> struct ArrayListMaxLengthRetrieveHelper {
...
using ArrayListFieldHasVarLengthBoolType =
    typename comms::util::LazyDeepConditional<std::is_integral<TElem>::value>::
        template Type<comms::util::FalseType, comms::util::FieldCheckVarLength,
                      TElem>;

template <typename TElem>
using HasArrayListElemNonDefaultRefreshBoolType =
    typename comms::util::LazyDeepConditional<std::is_integral<TElem>::value>::
        template Type<comms::util::FalseType,
                      comms::util::FieldCheckNonDefaultRefresh, TElem>;

template <typename TElem>
using IsArrayListElemVersionDependentBoolType =
    typename comms::util::LazyDeepConditional<std::is_integral<TElem>::value>::
        template Type<comms::util::FalseType,
                      comms::util::FieldCheckVersionDependent, TElem>;

template <typename TFieldBase, typename TStorage>
using ArrayListVersionStorageBase =
    typename comms::util::LazyShallowConditional<
        IsArrayListElemVersionDependentBoolType<typename TStorage::value_type>::
            value>::template Type<comms::field::details::VersionStorage,
...
class ArrayList
  ...
  using VersionBaseImpl =
      details::ArrayListVersionStorageBase<TFieldBase, TStorage>;
...
public:
  ...
  using CommsTag =
      typename comms::util::Conditional<std::is_integral<ElementType>::value>::
          template Type<comms::field::tag::RawArrayList,
                        comms::field::tag::ArrayList>;

  ArrayList() = default;

  explicit ArrayList(const ValueType &val) : m_value(val) {}

  explicit ArrayList(ValueType &&val) : m_value(std::move(val)) {}
  ...
  void clear() {
    static_assert(comms::util::detect::hasClearFunc<ValueType>(),
                  "The used storage type for ArrayList must have clear() "
                  "member function");

    m_value.clear();
  }

  constexpr std::size_t length() const { return lengthInternal(ElemTag<>()); }

  static constexpr std::size_t minLength() { return 0U; }

  static constexpr std::size_t maxLength() {
    return details::ArrayListMaxLengthRetrieveHelper<TStorage>::Value *
           maxLengthInternal(ElemTag<>());
  }

  constexpr bool valid() const { return validInternal(ElemTag<>()); }

  bool refresh() { return refreshInternal(ElemTag<>()); }
  ...
  static ErrorStatus readElement(ElementType &elem, TIter &iter,
                                 std::size_t &len) {
  ...
  template <typename TIter> ErrorStatus read(TIter &iter, std::size_t len) {
    ...
    static const bool IsRawData = std::is_integral<ElementType>::value &&
                                  (sizeof(ElementType) == sizeof(std::uint8_t));

    using Tag = typename comms::util::LazyShallowConditional<
        IsRandomAccessIter && IsRawData>::template Type<RawDataTag,
                                                        FieldElemTag>;
  ...
  template <typename TIter> void readNoStatusN(std::size_t count, TIter &iter) {
    using IterType = typename std::decay<decltype(iter)>::type;
    using IterCategory =
        typename std::iterator_traits<IterType>::iterator_category;
    static const bool IsRandomAccessIter =
  ...
  template <typename TIter>
  static ErrorStatus writeElement(const ElementType &elem, TIter &iter,
                                  std::size_t &len) {
  ...
  bool canWrite() const { return CommonFuncs::canWriteSequence(*this); }
  ...
  static constexpr bool hasNonDefaultRefresh() {
    return details::HasArrayListElemNonDefaultRefreshBoolType<
        ElementType>::value;
  }
...
private:
  ...
  template <typename... TParams> using RawDataTag = comms::details::tag::Tag5<>;
  ...
  using ElemTag =
      typename comms::util::Conditional<std::is_integral<ElementType>::value>::
          template Type<IntegralElemTag<TParams...>, FieldElemTag<TParams...>>;

  template <typename... TParams>
  using VersionTag = typename comms::util::Conditional<
      details::IsArrayListElemVersionDependentBoolType<ElementType>::value>::
      template Type<VersionDependentTag<TParams...>,
  ...
  constexpr std::size_t lengthInternal(FieldElemTag<TParams...>) const {
    using Tag = typename comms::util::LazyShallowConditional<
        ...
        template Type<VarLengthTag, FixedLengthTag>;
  ...
  std::size_t fieldLength(VarLengthTag<TParams...>) const {
    return std::accumulate(
        m_value.begin(), m_value.end(), std::size_t(0),
        [](std::size_t sum, typename ValueType::const_reference e)
            -> std::size_t { return sum + e.length(); });
  }
  ...
  template <typename TIter>
  static ErrorStatus readFieldElement(ElementType &elem, TIter &iter,
                                      std::size_t &len) {
    auto fromIter = iter;
  ...
  template <typename TIter>
  static ErrorStatus readIntegralElement(ElementType &elem, TIter &iter,
                                         std::size_t &len) {
  ...
  static ErrorStatus readElementInternal(ElementType &elem, TIter &iter,
                                         std::size_t &len,
  ...
  template <typename TIter, typename... TParams>
  static void readElementNoStatusInternal(ElementType &elem, TIter &iter,
                                          FieldElemTag<TParams...>) {
  ...
  template <typename TIter>
  static ErrorStatus writeFieldElement(const ElementType &elem, TIter &iter,
                                       std::size_t &len) {
  ...
  template <typename TIter>
  static ErrorStatus writeIntegralElement(const ElementType &elem, TIter &iter,
                                          std::size_t &len) {
    if (len < sizeof(ElementType)) {
  ...
  template <typename TIter, typename... TParams>
  static ErrorStatus writeElementInternal(const ElementType &elem, TIter &iter,
                                          std::size_t &len,
                                          FieldElemTag<TParams...>) {
  ...
  template <typename TIter>
  static void writeNoStatusIntegralElement(const ElementType &elem,
                                           TIter &iter) {
    BaseImpl::writeData(elem, iter);
  }

  template <typename TIter, typename... TParams>
  static void writeElementNoStatusInternal(const ElementType &elem, TIter &iter,
                                           FieldElemTag<TParams...>) {
  ...
  constexpr bool validInternal(FieldElemTag<TParams...>) const {
    return std::all_of(m_value.begin(), m_value.end(),
                       [](const ElementType &e) -> bool { return e.valid(); });
  }
  ...
  minElemLengthInternal(IntegralElemTag<TParams...>) {
    return sizeof(ElementType);
  }
  ...
  template <typename... TParams>
  static constexpr std::size_t
  maxElemLengthInternal(IntegralElemTag<TParams...>) {
    return sizeof(ElementType);
  }
  ...
  template <typename... TParams>
  static constexpr std::size_t
  elementLengthInternal(const ElementType &, IntegralElemTag<TParams...>) {
    return sizeof(ElementType);
  }

  template <typename... TParams>
  static constexpr std::size_t elementLengthInternal(const ElementType &elem,
                                                     FieldElemTag<TParams...>) {
    return elem.length();
  }

  template <typename TIter, typename... TParams>
  ErrorStatus readInternal(TIter &iter, std::size_t len,
                           FieldElemTag<TParams...>) {
    static_assert(comms::util::detect::hasClearFunc<ValueType>(),
                  "The used storage type for ArrayList must have clear() "
                  "member function");
  ...
  ErrorStatus readInternal(TIter &iter, std::size_t len,
                           RawDataTag<TParams...>) {
    comms::util::assign(value(), iter,
                        iter + std::min(len, comms::util::maxSizeOf(value())));
    std::advance(iter, len);
    return ErrorStatus::Success;
  }

  template <typename TIter, typename... TParams>
  ErrorStatus readInternalN(std::size_t count, TIter &iter, std::size_t len,
                            FieldElemTag<TParams...>) {
    clear();
    while (0 < count) {
  ...
  template <typename TIter, typename... TParams>
  void readNoStatusInternalN(std::size_t count, TIter &iter,
                             FieldElemTag<TParams...>) {
    clear();
    while (0 < count) {
  ...
  template <typename... TParams>
  static constexpr bool updateElemVersion(ElementType &,
                                          NoVersionDependencyTag<TParams...>) {
    return false;
  }
  ...
  template <typename... TParams>
  static constexpr bool setVersionInternal(VersionType,
                                           NoVersionDependencyTag<TParams...>) {
    return false;
  }

  template <typename... TParams>
  static bool canWriteElementInternal(const ElementType &elem,
                                      FieldElemTag<TParams...>) {
  ...
  template <typename TIter>
  comms::ErrorStatus createAndReadNextElementInternal(TIter &iter,
                                                      std::size_t &len) {
    if (m_value.size() < m_value.max_size()) {
      auto &elem = createBack();

...
struct BasicArrayListMaxLengthRetrieveHelper
{
    static const std::size_t Value = BasicCommonFuncs::maxSupportedLength();
};

template <typename T, std::size_t TSize>
struct BasicArrayListMaxLengthRetrieveHelper<comms::util::StaticVector<T, TSize> >
{
    static const std::size_t Value = TSize;
};

template <std::size_t TSize>
struct BasicArrayListMaxLengthRetrieveHelper<comms::util::StaticString<TSize> >
{
    static const std::size_t Value = TSize - 1;
};

template <typename TElem>
using BasicArrayListFieldHasVarLengthBoolType =
...
template <typename TElem>
using HasBasicArrayListElemNonDefaultRefreshBoolType =
    typename comms::util::LazyDeepConditional<
        std::is_integral<TElem>::value
    >::template Type<
        comms::util::FalseType,
...
template <typename TElem>
using IsBasicArrayListElemVersionDependentBoolType =
    typename comms::util::LazyDeepConditional<
        std::is_integral<TElem>::value
    >::template Type<
        comms::util::FalseType,
...
using BasicArrayListVersionStorageBase =
    typename comms::util::LazyShallowConditional<
        IsBasicArrayListElemVersionDependentBoolType<typename TStorage::value_type>::value
    >::template Type<
...
class BasicArrayList :
        public TFieldBase,
        public details::BasicArrayListVersionStorageBase<TFieldBase, TStorage>
{
    using BaseImpl = TFieldBase;
    using VersionBaseImpl = details::BasicArrayListVersionStorageBase<TFieldBase, TStorage>;
...
public:
    ...
    BasicArrayList() = default;

    explicit BasicArrayList(const ValueType& val)
      : m_value(val)
    {
    }

    explicit BasicArrayList(ValueType&& val)
      : m_value(std::move(val))
    {
    }

    BasicArrayList(const BasicArrayList&) = default;
    BasicArrayList(BasicArrayList&&) = default;
    BasicArrayList& operator=(const BasicArrayList&) = default;
    BasicArrayList& operator=(BasicArrayList&&) = default;
    ~BasicArrayList() noexcept = default;
    ...
    {
        return
            details::BasicArrayListMaxLengthRetrieveHelper<TStorage>::Value *
    ...
    bool canWrite() const
    {
        return BasicCommonFuncs::canWriteSequence(*this);
    }

    template <typename TIter>
    ErrorStatus write(TIter& iter, std::size_t len) const
    {
        return BasicCommonFuncs::writeSequence(*this, iter, len);
    }
    ...
    {
        BasicCommonFuncs::writeSequenceNoStatus(*this, iter);
    }

    template <typename TIter>
    ErrorStatus writeN(std::size_t count, TIter& iter, std::size_t& len) const
    {
        return BasicCommonFuncs::writeSequenceN(*this, count, iter, len);
    }

    template <typename TIter>
    void writeNoStatusN(std::size_t count, TIter& iter) const
    {
        BasicCommonFuncs::writeSequenceNoStatusN(*this, count, iter);
    }

    static constexpr bool isVersionDependent()
    {
        return details::IsBasicArrayListElemVersionDependentBoolType<ElementType>::value;
    }

    static constexpr bool hasNonDefaultRefresh()
    {
        return details::HasBasicArrayListElemNonDefaultRefreshBoolType<ElementType>::value;
    }
...
private:
    ...
    using VersionTag =
        typename comms::util::Conditional<
            details::IsBasicArrayListElemVersionDependentBoolType<ElementType>::value
        >::template Type<
    ...
    constexpr std::size_t lengthInternal(FieldElemTag<TParams...>) const
    {
        using Tag =
            typename comms::util::LazyShallowConditional<
                details::BasicArrayListFieldHasVarLengthBoolType<ElementType>::value
            >::template Type<
                VarLengthTag,
                FixedLengthTag
            >;

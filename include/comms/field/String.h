...
template <bool THasOrigDataViewStorage> struct StringOrigDataViewStorageType;

template <> struct StringOrigDataViewStorageType<true> {
#if COMMS_HAS_CPP17_STRING_VIEW
...
template <> struct StringOrigDataViewStorageType<false> {
...
template <> struct StringFixedSizeUseFixedSizeStorageType<true> {
  template <typename TOpt>
  using Type = comms::util::StaticString<TOpt::SequenceFixedSize>;
};

template <> struct StringFixedSizeUseFixedSizeStorageType<false> {
  template <typename TOpt>
  using Type =
      typename StringOrigDataViewStorageType<TOpt::HasOrigDataView>::Type;
};

template <bool THasFixedSizeStorage> struct StringFixedSizeStorageType;

template <> struct StringFixedSizeStorageType<true> {
  template <typename TOpt>
  using Type = comms::util::StaticString<TOpt::FixedSizeStorage>;
};

template <> struct StringFixedSizeStorageType<false> {
  template <typename TOpt>
  using Type = typename StringFixedSizeUseFixedSizeStorageType<
      TOpt::HasSequenceFixedSizeUseFixedSizeStorage>::template Type<TOpt>;
};

template <bool THasCustomStorage> struct StringCustomStringStorageType;

template <> struct StringCustomStringStorageType<true> {
  template <typename TOpt> using Type = typename TOpt::CustomStorageType;
};

template <> struct StringCustomStringStorageType<false> {
  template <typename TOpt>
  using Type = typename StringFixedSizeStorageType<
      TOpt::HasFixedSizeStorage>::template Type<TOpt>;
};

template <typename TOpt>
using StringStorageTypeT = typename StringCustomStringStorageType<
    TOpt::HasCustomStorageType>::template Type<TOpt>;

template <typename TFieldBase, typename... TOptions>
using StringBase = AdaptBasicFieldT<
...
///     option is used, which forces usage of comms::util::StaticString instead.
...
public:
  ...
  /// @brief Constructor
  explicit String(const ValueType &val) : BaseImpl(val) {}

  /// @brief Constructor
  explicit String(ValueType &&val) : BaseImpl(std::move(val)) {}

  /// @brief Constructor
  explicit String(const char *str) { setValue(str); }
  ...
  template <typename TIter> ErrorStatus read(TIter &iter, std::size_t len) {
    ...
    using TagTmp = typename comms::util::LazyShallowConditional<
        ParsedOptions::HasSequenceFixedSize>::template Type<AdjustmentNeededTag,
                                                            NoAdjustmentTag>;
  ...
  std::size_t length() const { return BaseImpl::length(); }
  ...
  ///     characters the field contains. If @ref
  ///     comms::option::def::SequenceFixedSize option is used, the number of
  ///     characters, that is going to be written, is exactly as the option
  ///     specifies. If underlying string storage doesn't contain enough data,
  ///     the '\0' characters will be appended to the written sequence until the
  ///     required amount of elements is reached.
  ...
  /// comms::option::def::SequenceSizeForcingEnabled option has been
  ///     used.
  void clearReadElemCount() { BaseImpl::clearReadElemCount(); }
...
private:
  ...
  void adjustValue(AdjustmentNeededTag<TParams...>) {
    std::size_t count = 0;
    for (auto iter = BaseImpl::value().begin(); iter != BaseImpl::value().end();
         ++iter) {
      if (*iter == 0) {
        break;
      }
  ...
  void doResize(std::size_t count) {
    using TagTmp = typename comms::util::Conditional<
        comms::util::detect::hasResizeFunc<ValueType>()>::
        template Type<
            HasResizeTag<>,
            typename comms::util::Conditional<
                comms::util::detect::hasRemoveSuffixFunc<ValueType>()>::
                template Type<HasRemoveSuffixTag<>, void>>;

    static_assert(!std::is_void<TagTmp>::value,
                  "The string storage value type must have either resize() or "
                  "remove_suffix() "
                  "member functions");
  ...
  static_assert(!ParsedOptions::HasSequenceElemFixedSerLengthFieldPrefix,
                ...
                "option is not applicable to String field");
...
/// comms::field::String type
///     in order to have access to its internal types.
/// @related comms::field::String
template <typename TFieldBase, typename... TOptions>
inline String<TFieldBase, TOptions...> &
...
/// @related comms::field::String
template <typename TFieldBase, typename... TOptions>
inline const String<TFieldBase, TOptions...> &

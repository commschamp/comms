...
///     of length(), read() and write() operations depends on the current
///     field's mode.
/// @tparam TField Proper type of the field that needs to be optional.
/// @tparam TOptions Zero or more options that modify/refine default behaviour
///     of the field.@n
///     Supported options are:
///     @li @ref comms::option::def::DefaultValueInitialiser, @ref
///     comms::option::def::DefaultOptionalMode,
///         @ref comms::option::def::OptionalMissingByDefault, or @ref
///         comms::option::def::OptionalExistsByDefault.
...
class Optional
  ...
  using BaseImpl =
      details::AdaptBasicFieldT<basic::Optional<TField>, TOptions...>;
...
public:
  ...
  explicit Optional(const Field &fieldSrc) : BaseImpl(fieldSrc) {}

  /// @brief Construct the field.
  /// @param[in] fieldSrc Field to be moved from during construction.
  explicit Optional(Field &&fieldSrc) : BaseImpl(std::move(fieldSrc)) {}
  ...
  Mode getMode() const { return BaseImpl::getMode(); }

  /// @brief Get optional mode
  void setMode(Mode val) { BaseImpl::setMode(val); }
  ...
  ///     function returns comms::ErrorStatus::Success without advancing
  ///     iterator.@n If field is marked as existing (mode is
  ///     OptionalMode::Exists) the read() member function of the wrapped field
  ///     object is invoked.@n If field is marked to be tentative (mode is
  ///     OptionalMode::Tentative), the call redirected to wrapped field's
  ///     read() member function if value of the "len" parameter is greater than
  ///     0, i.e. there are still bytes available for reading, and field itself
  ///     is marked as existing.@n Otherwise, field is marked as missing and
  ...
  ///     function returns comms::ErrorStatus::Success without advancing
  ///     iterator.@n If field is marked as existing (mode is
  ///     OptionalMode::Exists) the write() member function of the wrapped field
  ///     object is invoked.
...
/// @related comms::field::Optional
template <typename TField, typename... TOptions>
inline const Optional<TField, TOptions...> &

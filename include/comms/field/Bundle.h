...
public:
  ...
  /// @brief Constructor
  explicit Bundle(const ValueType &val) : BaseImpl(val) {}

  /// @brief Constructor
  explicit Bundle(ValueType &&val) : BaseImpl(std::move(val)) {}
  ...
  /// @tparam TFromIdx Index of the field (included) from which the counting
  /// must start.
  /// @return Number of bytes it will take to serialise the specified member
  /// fields.
  /// @pre TFromIdx < std::tuple_size<ValueType>::value
  template <std::size_t TFromIdx> std::size_t lengthFrom() const {
  ...
  /// @details Summarises all the results returned by the call to length() for
  ///     every specified field in the bundle.
  /// @tparam TUntilIdx Index of the field (not included) until which the
  /// counting must be performed.
  /// @return Number of bytes it will take to serialise the specified member
  /// fields.
  /// @pre TUntilIdx <= std::tuple_size<ValueType>::value
  template <std::size_t TUntilIdx> std::size_t lengthUntil() const {
  ...
  /// @brief Get minimal length that is required to serialise all bundled
  /// fields.
  ...
  /// @brief Get minimal length that is required to serialise specified bundled
  /// fields.
  /// @tparam TFromIdx Index of the field (included) from which the counting
  /// must start.
  /// @return Minimal number of bytes required to serialise the specified member
  /// fields.
  /// @pre TFromIdx < std::tuple_size<ValueType>::value
  template <std::size_t TFromIdx> static constexpr std::size_t minLengthFrom() {
  ...
  /// @brief Get maximal length that is required to serialise all bundled
  /// fields.
  /// @return Maximal number of bytes required serialise the field value.
  static constexpr std::size_t maxLength() { return BaseImpl::maxLength(); }

  /// @brief Get maximal length that is required to serialise specified bundled
  /// fields.
  /// @tparam TFromIdx Index of the field (included) from which the counting
  /// must start.
  /// @return Minimal number of bytes required to serialise the specified member
  /// fields.
  /// @pre TFromIdx < std::tuple_size<ValueType>::value
  template <std::size_t TFromIdx> static constexpr std::size_t maxLengthFrom() {
  ...
  /// @details Similar to @ref readFrom(), but updates provided length
  /// information.
  ...
  ///     updating remaining length information.
  /// @details Similar to @ref readUntil(), but updates provided length
  /// information.
  ///     Number of consumed bytes are subsctruced from provided length value.
  /// @tparam TUntilIdx Index of the member field (NOT included) until which the
  ///     "read" operation continues.
  ...
  /// while
  ///     updating remaining length information.
  /// @details Similar to @ref readFromUntil(), but updates provided length
  /// information.
  ///     Number of consumed bytes are subsctruced from provided length value.
  /// @tparam TFromIdx Index of the member field (included) from which the
  ///     "read" operation starts
  ...
  ErrorStatus readFromUntilAndUpdateLen(TIter &iter, std::size_t &len) {
    return BaseImpl::template readFromUntilAndUpdateLen<TFromIdx, TUntilIdx>(
        iter, len);
  }
...
private:
  ...
  static_assert(!ParsedOptions::HasSequenceElemLengthForcing,
                ...
                "is not applicable to Bundle field");
  ...
  static_assert(!ParsedOptions::HasSequenceLengthForcing,
                ...
                "not applicable to Bundle field");
  ...
  static_assert(!ParsedOptions::HasSequenceFixedSizeUseFixedSizeStorage,
                ...
                "option is not applicable to Bundle field");
